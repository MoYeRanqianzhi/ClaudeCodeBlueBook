...
答：因为弹窗只能在动作发生时拦截，不能强迫系统先探索再设计，更不能在并行实施前建立共享 plan 对象。

### 问：为什么不直接只用 Git branch，而还要 worktree

答：branch 解决历史指针，worktree 解决并行执行时的物理工作目录隔离。

### 问：为什么还需要计划文件落盘

答：因为只有落成对象，审批、恢复、远程同步、teammate 协调才有共同引用面。
