...
- `122-127` remote recovery / ownership / terminality / compaction
...
- owner-side recovery、viewer ownership、transport terminality、compaction
- surface presence、front-state consumer topology、schema-store consumer path、shared interaction shell
- remote-session presence ledger、gray runtime、global remote bit、session pane、command shell、headless remote memory

但不想再从 `100-149` 或主 `README` 里逐条找文件，
...
- 哪些记忆负责把 `122-127` 收成 recovery 双主干与 zoom
- 哪些记忆负责把 `128-138` 收成 surface presence 到 interaction shell 的接力线
- 哪些记忆负责把 `139-149` 收成 remote truth、shell pair 与 env-driven persistence 的后继结构

所以这个目录的作用只是：

- 把现有结构页、scope guard 与子区间接力关系收成一个更窄的家族入口
...
### 1. 我还在 remote recovery / ownership / terminality（`122-127`）
...
### 2. 我已经进入 surface presence / interaction shell（`128-138`）
