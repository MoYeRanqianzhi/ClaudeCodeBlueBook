...
- `docs/` 与 `bluebook/` 并列协同，负责方法附录、证据归档与目录治理记录，不承载正文

总索引只保留一级路由和稳定阅读路径；更深的跨目录编号链统一下沉到各子目录 README，尤其是 `navigation/README.md`。如果问题已经变成方法附录、版本锚点、清理记录与目录治理历史，就不要继续留在 `bluebook/`，直接去 `docs/README.md`。
