# 如何把Prompt宿主修复稳态纠偏再纠偏执行失真压回compiled request truth：固定纠偏顺序、拒收升级路径与recorrection、protocol、threshold改写模板骨架

这一章不再解释 Prompt 宿主修复稳态纠偏再纠偏执行最常怎样失真，而是把 Claude Code 式 Prompt correction-of-correction-of-correction 压成一张可执行的 builder-facing 手册。

它主要回答五个问题：

1. 为什么 Prompt correction-of-correction-of-correction 真正要救回的不是一张更严谨的 `recorrection card`，而是同一个 `compiled request truth`。
2. 怎样把假 `recorrection card`、假 `protocol repair`、假 `lawful forgetting reseal` 与假 `threshold liability` 压回固定纠偏顺序。
...
Prompt correction-of-correction-of-correction 真正要救回的不是：
...
Prompt 的魔力显得像“咒语”，恰恰是因为它先把世界编译成同一个 request object、同一个 protocol transcript、同一个 lawful forgetting boundary 与同一个 continuation qualification；一旦 recorrection 退回 prose，魔力就会从编译链退回安抚链。
...
2. 先把 `compiled request truth` 与 `protocol transcript` 从 UI 历史与整理过的对话里救出来。
3. 先把 `lawful forgetting boundary` 与 `preserved segment` 从礼貌 summary 里救出来。
4. 先把 `stable prefix` 与 `cache break threshold` 从安静曲线与好运气里救出来。
5. 先把 `continue qualification` 与 `threshold liability` 从经验式继续里救出来。

## 2. 固定纠偏顺序

### 2.1 先冻结假 recorrection
...
### 2.4 再恢复 `stable prefix reseal` 与 `lawful forgetting reseal`

第四步要把共享前缀资产与合法遗忘边界从好运气与 summary 礼貌感里救回：

1. `stable_prefix_boundary`
2. `cache_break_budget`
3. `compaction_lineage`
4. `lawful_forgetting_boundary`
5. `preserved_segment_ref`
6. `summary_override_blocked`
7. `reactivation_trigger_registered`

没有这一步，Prompt 世界仍会退回“目前还能读”的安静叙事。

### 2.5 再恢复 `continuation requalification rebinding`
...
### 2.6 最后恢复 `threshold liability reinstatement`
...
4. `lawful_forgetting_boundary_missing`
5. `stable_prefix_reseal_missing`
6. `continue_qualification_unbound`
...
5. `truth_continuity_restitution_ref`
6. `protocol_transcript_repair_ref`
7. `stable_prefix_reseal_ref`
8. `lawful_forgetting_reseal_ref`
9. `continue_qualification_ref`
10. `threshold_liability_ref`
11. `reject_verdict`
12. `verdict_reason`
...
3. 我现在保护的是 `lawful forgetting boundary`，还是一份更体面的 summary。
4. 我现在允许的是 `continue qualification`，还是一种“先继续再说”的默认冲动。
5. `threshold liability` 还在不在；如果不在，我是在恢复 recorrection，还是在删除未来反对当前状态的能力。

## 6. 一句话总结

真正成熟的 Prompt 宿主修复稳态纠偏再纠偏再纠偏，不是把 transcript 与 summary 写得更顺，而是把假 `recorrection card`、假 `protocol repair`、假 `lawful forgetting reseal` 与假 `threshold liability` 重新压回同一个 `compiled request truth`。
