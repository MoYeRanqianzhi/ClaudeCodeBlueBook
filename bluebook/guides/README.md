...
`guides/` 当前有 95 篇编号文档，范围 `01-95`。本目录把源码研究翻成可执行的方法、模板、审读清单和构建手册，不重复产品说明文案。
...
- `81-95`: 再纠偏、改写纠偏、精修与 refinement correction 执行模板。
...
- 想处理宿主迁移、验收、修复和后续纠偏：从 `48-95`
- 想做跨目录跳转：回到 [../navigation/README.md](../navigation/README.md)

## 维护约定

- README 只保留编号段和稳定起点，不再逐条镜像 `57-95` 这类高频重命名段。
