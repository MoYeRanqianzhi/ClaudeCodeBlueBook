...
- 想直接抓 Prompt 对象链的高阶审读模板：从 `99`
- 想直接抓 `governance key / decision window / continuation pricing` 的高阶审读模板：从 `100`
- 想直接抓 `event-stream-vs-state-writeback / freshness gate / stale worldview / ghost capability` 的高阶审读模板：从 `101`
...
- 想把 `09` 落成当前审读顺序：本目录只保留三个目录内起点，Prompt 从 `99`，治理从 `100`，结构从 `101`；凡是需要补桥接或接到长期验证，统一回 [../navigation/README.md](../navigation/README.md)
- 想处理 Prompt 迁移 / 验收 / 长期验证尾部：默认走 `57 -> 35 -> 77`
- 想处理治理迁移 / 验收 / 长期验证尾部：默认走 `58 -> 36 -> 78`
- 想处理结构迁移 / 验收 / 长期验证尾部：默认走 `59 -> 37 -> 79`
- 想把高阶审读一路接到长期验证：Prompt 走 `99 -> 77`，治理走 `100 -> 78`，结构走 `101 -> 79`
...
- README 只保留编号段和稳定起点，不再逐条镜像后段编号链。
