...
本 README 只说明 `guides/` 目录的稳定入口与编号段职责；旧词汇桥接、跨目录链路和迁移期映射统一由 [../navigation/README.md](../navigation/README.md) 承担。
...
- 想把 `09` 落成 Builder 审读顺序：本目录只保留三个目录内起点，Prompt 从 `99`，治理从 `100`，结构从 `101`；凡是需要补桥接或接到长期验证，统一回 [../navigation/README.md](../navigation/README.md)
