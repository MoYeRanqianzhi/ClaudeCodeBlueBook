...
1. 先回 `../philosophy/63` 与 `../philosophy/60`，判断自己改坏的是主语、前缀、边界还是合法遗忘。
2. 再回 `18` 与 `24`，检查共享前缀网络和 Prompt Constitution 是否已被重新拆散。
3. 再回 `27`，把问题写进 section card、invalidation ledger 与 triage runbook。
4. 最后才决定是修正文案、重画 boundary，还是修改 compact / recovery 协议。
