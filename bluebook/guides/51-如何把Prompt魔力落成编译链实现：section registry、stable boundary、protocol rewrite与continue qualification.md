...
`stable boundary` 首先决定的也不是缓存命中，而是继承法：哪些字节可免重审继承，哪些变化一旦越界就必须重入重审。`cache break` 不是性能提示，而是继承资格失效信号。
...
`protocol rewrite` 保护的首先不是 transcript 整洁，而是行动因果作者链；display layer、summary 与 orphan pairing 一旦混入协议面，就会把只配解释现场的 consumer 偷写成 shadow author。
...
`continue qualification` 首先也不是恢复便利，而是时间续租；时间本身就是扩张面，继续前必须重新证明当前世界仍配沿同一条资格链续租。
...
这六步按 `reject cost` 从低到高排序，不按模块名排序；越便宜、越靠前的 veto 越该先发生，别让模型先在非法世界里做昂贵推理。
