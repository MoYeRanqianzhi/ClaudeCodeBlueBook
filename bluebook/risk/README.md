...
1. `governance key`
   - 谁配签发恢复、谁配冻结边界
2. `decision window + product promise`
   - 当前误伤、恢复与入口差异到底落在用户侧哪一层
...
更稳的 first reject signal 也应先记两条：
...
- 当你已经知道统一定价治理成立，准备判断它怎样在误伤、恢复、支持链路与入口语义差上显形。
...
- 本目录只回答统一定价治理怎样在误伤、恢复、支持链路与入口语义差上对用户显形。
