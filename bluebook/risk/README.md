...
`risk/` 只看统一治理收费链在 cleanup 之后怎样被用户侧读回成 `product promise readback / reopen qualification / evidence binding`，不把风控写成账号、入口、恢复或地区场景的并列主题。
...
- 本目录不是在补充另一套规则堆，而是在看同一条治理收费链怎样在 cleanup 之后被用户侧读成 `product promise readback / reopen eligibility readback / evidence receipt binding`；想先抓高阶判断，先回 `10 -> philosophy/85 -> philosophy/61 -> security`，再回本目录看误伤、恢复与入口差异怎样落到用户侧。
...
- 风控不是第二套安全规则；它只解释 `product promise readback / reopen eligibility readback / evidence receipt binding` 这三类用户侧对象。治理机制 speaking right 仍回 `10 -> philosophy/85 -> philosophy/61 -> security`。
...
如果把本页压成最短顺序，也只剩三步，但它们都只是 post-cleanup readback，不是新的入口流程：

1. `product promise readback`
   - 已知你在看用户侧 admission / capability / continuity wording 时，再来这里补读回证据
2. `reopen eligibility readback`
   - 已知你在看 cleanup 之后“还能不能重开”的用户侧资格时，再来这里补尾部读回
3. `evidence receipt binding`
   - 已知你在看申诉、追责或收口后还能绑定什么 receipt 时，再来这里补证据读回
...
- `risk/` 只在 post-cleanup readback 已经成立后开始；若还要回头重判治理链，先退回 `10 -> philosophy/85 -> philosophy/61 -> security`，不在本页自己重放治理前门。

因此风控入口说明也不该自己回放事件去猜当前真相，而应沿已外化的 readback、receipt 与 reopen 条件去回读。通用弱读回面总声明与治理 why 统一回 `10 -> philosophy/85 -> philosophy/61`；机制口径统一回 `security`；只要问题已经进入验收、修复或 reopen drill，就统一回 `playbooks/README`。
...
- 只按对象 handoff 继续：还在判治理主键、host truth 或 liability evidence，先回 `09 / philosophy/85 / security/README`；已经进入执行动作，统一回 [../playbooks/README.md](../playbooks/README.md)。
