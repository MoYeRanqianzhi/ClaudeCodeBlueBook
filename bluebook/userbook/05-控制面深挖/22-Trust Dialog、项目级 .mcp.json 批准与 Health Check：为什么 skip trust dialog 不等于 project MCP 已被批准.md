...
1. 先认 workspace trust
   - 当前入口有没有真的通过交互工作区信任边界。
2. 再认 project server approval
   - 这些 project-scope `.mcp.json` server 现在是否被本地启用。
3. 再认 health-check runtime
   - 当前 `doctor`、`mcp list|get` 做的是批准流，还是探活/诊断流。
4. 最后才判断为什么某条 project MCP 现在“看起来像能用”。

更短的前门公式就是：

- `workspace trust -> project approval -> health-check runtime -> fallback`

## 进入本页前的 first reject signal

看到下面迹象时，应先回到三层对象链，而不是继续把 health check 写成批准：

- 你把 `skip trust dialog` 写成“这个目录已经被信任”。
- 你把 `mcp list` 或 `mcp get` 碰到 server 写成“这个 project server 已经过批准”。
- 你把 `reset-project-choices` 写成重置 workspace trust。
- 你把 bypass / non-interactive fallback 写成 health-check 的另一种叫法。
...
- `skip trust dialog` 只说明当前入口没走交互 trust UI。
- 它不替你回答 workspace trust、project approval 或 health-check 三个对象里的任何一个最终 verdict。
