...
1. 先认配置层
   - 这条 server 是不是已经进入全局可见配置集合。
2. 再认解析层
   - 它是不是属于按名可引用集合。
3. 再认连接层
   - 当前 session 是不是真的连上了对应 client。
4. 再认 agent attach 路径
   - 当前 agent 走的是 string 引用，还是 inline 定义。
5. 最后才问为什么 `/mcp` 里看得到、Agent 却挂不上。

更短的前门公式就是：

- `config set -> name resolution -> client connection -> agent attach -> runtime gate`

## 进入本页前的 first reject signal

看到下面迹象时，应先回到 MCP 对象层，而不是继续把 `/mcp` 当唯一真相：

- 你把 `/mcp` 菜单里的可见 server 直接当成 Agent 可按名引用的 server。
- 你把全局配置总览和按名解析集合写成同一张表。
- 你把 string 引用和 inline 定义写成同一条路径。
- 你把“agent 有 `mcpServers`”直接等同于“运行时一定会挂上”。
...
- `/mcp` 看见了，不等于按名可引用。
- 按名可引用了，不等于当前 client 已连上。
- client 已连上了，也不等于当前 agent 还能把它挂进自己的运行面。
