...
Claude Code 不是为“一问一答”设计的。它服务的是长时间、可中断、可续接的软件工作。
...
如果继续把这组顺序压成治理链，也只该再补一句：

- `Compact / Resume / Memory` 都只是 `decision window -> continuation pricing -> durable-transient cleanup` 的 continuation consumer，不是独立控制面；`Export` 只配把结果移交 `Outside`，不是会内 continuation consumer。
- 相邻的 `/status / /doctor / /usage` 只配做 runtime projection；真正 verdict 仍留在 `truth-surface -> decision window -> durable-transient cleanup`，用户侧恢复仍留在 `signer + evidence + reopen`。
- 更稳的 fail-route handoff 也只记一句：continuation consumer 只处理续接动作；一旦 `resume` 资格已经滑到责任、证据与恢复签发，就该转回用户侧 `signer + evidence + reopen` 链，而不是继续把 `/resume` 当万能续接按钮。
- Prompt 线的正式 owner 仍只认 `philosophy/84 -> guides/51`；本页只消费 `continuation_object_ref + continue_qualification_verdict`，不再另造一条 continuity frontdoor。
...
- 你把 `pending_action / task_summary / summary prose` 当成恢复主链，而不是把它们看成观察面或辅助恢复资产。

## 这些入口都不配重签世界

更硬一点说，这四个入口都不配重签世界：

- `Compact` 只能重写表示，不能改判唯一 `world-definition source`。
- `Resume` 只能恢复已具资格的现场，不能把旧 summary、旧标题或旧恢复资产升级成 current truth。
- `Memory` 只能外置 stable constraints，不能把 transient authority 或整个 working set 直接沉成长期真相。
- `Export` 只能把结果送到 `Outside`，不能替会内继续资格补签缺失的 `durable-transient cleanup`。
- `Usage`、status 条或 compact 成功提示都只配帮助观察这条链走到哪一段；runtime projection 不配改判，continuation consumer 也不配代签 verdict。
- 如果这里开始靠 `/resume`、summary prose 或 compact 成功提示重签 Prompt ABI，本页就已经从 consumer 长成第二 Prompt 前门。

如果继续把这组边界压得更硬，还应再多记一句：

- progress、duplicate control_response、旧 title/tag/mode 这类对象都只配帮助恢复或解释，不配逆流写回 transcript 主链，更不配重签 current truth。
...
真正成熟的 Compact 还要允许 cache break、stale witness 与 reload failure 被正式点名；一旦系统只能给出“故事还差不多”，就已经从 continuation consumer 退化成了摘要器。
...
如果旧 worktree、旧 summary 或旧恢复资产已经失效，正确动作是降级、重绑或拒绝继续，而不是假装恢复仍然成功。
如果问题已经从“还能不能续接当前工作对象”滑到“谁来重新签发恢复、证据和 reopen 责任”，就应转回 [../../risk/README.md](../../risk/README.md) 与相关 reopen playbook，而不是继续停在 `/resume` 的命令说明里。
...
若答案已经变成“需要谁来重新签发恢复或 reopen”，就不要继续停在本页，转回 [../../risk/README.md](../../risk/README.md)。
