...
2. 扩张或权限判断不要先看弹窗、`status`、`usage`、继续入口或摘要；这些都只算 `weak readback / lease checkpoint`。除非它们在同一条 `authority lease` 上真的带来新的 `decision delta`，否则最多也只是 `receipt-grade` 证据。先找这次动作、可见性或继续请求里最早那条还没被定价的 `unpaid expansion`，再看它有没有补齐 `repricing proof / lease checkpoint / explicit cleanup trigger`。
...
- Prompt 线只翻 user-facing witness，不替 Prompt owner 页重判 `compiled world verdict`；owner 级 ABI 统一回 `philosophy/84`
- 治理线只翻用户可见的 weak readback surface，并显式降格 continuation consumer 与 reopen tail evidence；弹窗、`status`、`usage`、继续入口与摘要都只算 `weak readback / lease checkpoint`，没有新增 `decision delta` 时也只配留在 `receipt-grade`；治理速记与 why 统一回 `10 -> philosophy/85 -> philosophy/61`，机制、用户侧恢复与现场执行分别回 `security / risk / playbooks`
...
- 不确定自己现在该走哪条正式路线：
  [00-阅读路径.md](./00-%E9%98%85%E8%AF%BB%E8%B7%AF%E5%BE%84.md)
- 想先看稳定使用主线，而不是直接挑专题或控制面：
  [01-主线使用/README.md](./01-%E4%B8%BB%E7%BA%BF%E4%BD%BF%E7%94%A8/README.md)
- 想围绕一个真实工作目标完整走一遍：
  [04-专题深潜/README.md](./04-%E4%B8%93%E9%A2%98%E6%B7%B1%E6%BD%9C/README.md)
- 想判断为什么不能把相邻入口、状态投影和继续判断混着用：
  [05-控制面深挖/README.md](./05-%E6%8E%A7%E5%88%B6%E9%9D%A2%E6%B7%B1%E6%8C%96/README.md)
- 只想按名字、入口和合同速查：
  [03-参考索引/README.md](./03-%E5%8F%82%E8%80%83%E7%B4%A2%E5%BC%95/README.md)
- 想建立运行时总图，而不是先学操作：
...
- root flags / root commands / slash commands、自动化、长任务恢复、多前端接续、扩展运维：
  先回 [00-阅读路径.md](./00-%E9%98%85%E8%AF%BB%E8%B7%AF%E5%BE%84.md) 选正式路径。
- 某个专题目标里的具体下一跳：
  先回 [04-专题深潜/README.md](./04-%E4%B8%93%E9%A2%98%E6%B7%B1%E6%BD%9C/README.md) 选对应专题。
- 某个控制面、host/viewer/remote、命令可见性或继续资格对照：
  先回 [05-控制面深挖/README.md](./05-%E6%8E%A7%E5%88%B6%E9%9D%A2%E6%B7%B1%E6%8C%96/README.md) 选对应子树。

根前门不再继续库存另一份 `04 / 05 / 03 / 02` 的叶子页总表。
