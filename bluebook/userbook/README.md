...
- 治理线只翻用户可见的 readback、恢复与继续信号；弹窗、`status`、`usage`、继续入口与摘要都只算 `weak readback / lease checkpoint`，没有新增 `decision delta` 时也只配留在 `receipt-grade`；why、mechanism 与 execution 由目标页继续分流，不在根页写死
