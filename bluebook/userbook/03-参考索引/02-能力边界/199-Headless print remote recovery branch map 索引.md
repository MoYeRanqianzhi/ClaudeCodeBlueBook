# Headless print remote recovery branch map 索引

这页只做速查，不替代长文专题。长文解释请结合：

- `05-控制面深挖/212-remote recovery、viewer ownership、transport terminality 与 compaction contract：为什么 122、123、124、125、126、127 不是一条 recovery 后续链.md`
...
- 这页保护的是 owner-side recovery、ownership、proof、transport、terminality 与 compaction 的主语分裂，不把常量、gate 名或某条 surface 是否出现直接升级成稳定公共合同。

## 1. 分叉图总表

| 节点 | 作用 | 在图上的位置 |
| --- | --- | --- |
| 212 | 解释 122-127 的双根 + 降层主干结构 | 结构收束页 |
| 122 | 先分 watchdog / warning / reconnect / reconnecting / disconnected | recovery 根页 |
| 123 | 先分 `viewerOnly` 为什么不是 session owner | ownership 侧枝 |
| 124 | 继续分 warning / authority / action / projection / absence 这些 signer | signer zoom |
| 125 | 从 signer / proof 层降到 transport authority 层 | 降层根页 |
...
| 错误写法 | 更准确的写法 |
| --- | --- |
| 122-127 都在讲 recovery，所以顺着编号一路读就行 | 更稳的是：`122/123` 双根，`124` signer zoom，`125` 降层根页，`126/127` 双枝 |
| 123 只是 122 的 viewer 附录 | 123 在问 owner scope / gate，不在继续列 recovery edge |
...
| attach viewer 没 warning / remote pill，就说明没有 remote recovery | absence 先要问 signer / surface 是否本来就没挂上 |

## 3. stable / conditional / internal

| 类型 | 对象 |
| --- | --- |
| 稳定可见 | warning、reconnect action、reconnecting、disconnected 不是同一状态；`viewerOnly` 是 non-owning interactive client；`remoteConnectionStatus` 更接近 shared durable authority；`handleClose(...)` 是 transport authority path；terminality 至少分三桶；compaction 至少分五类信号 |
...
| 内部/灰度层 | timeout / retry / delay 常量、`KAIROS`、`remoteSessionUrl` 挂载条件、`compact_boundary.preserved_segment`、具体 cadence 与具体 UI 文案 |
...
- 我有没有把 `122/123/124/125/126/127` 又写回成一条线性恢复链？
