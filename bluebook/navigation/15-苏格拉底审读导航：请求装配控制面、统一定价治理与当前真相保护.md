...
- 目录前门在这里仍只复用 `Authority -> Boundary -> Transcript -> Lineage -> Continuation -> Explainability` 这组 `first-reject order`；对象级 canonical witness order 继续只回 `philosophy/84` 与 `guides/51`。
