...
- 如果你已经开始点名 `message_lineage_ref / section_registry_ref / stable_prefix_ref / protocol_transcript_ref / continuation_object_ref / continue_qualification_verdict`，说明你缺的已不是问题梯子，而是 owner page。
...
- 如果你已经开始点 `pricing-right / truth-surface / asset-rollback ABI / reopen qualification`，说明你缺的已不是问题梯子，而是 owner page

这一刀区分也要先钉死：mode、审批、usage、default continue 与 compact 都只是投影或 consumer，不是治理真相。

如果排查已经进入 failure order、shared reject verdict 或 reopen qualification，就立刻离开 `15`，统一回 `10 -> philosophy/61 -> security/README`；`15` 只负责暴露第一条反证，不负责发布治理故障顺序。
...
如果排查已经开始给 `shim / snapshot / pointer / authority / seam / recovery / eviction` 排 later-maintainer reject 顺序，说明你缺的已不是苏格拉底问题，而是 current-truth owner chain；统一回 `guides/102 -> philosophy/86 -> philosophy/87 -> architecture/README`。
...
6. 我有没有把 owner page 的 failure order 讲成了本页的制度顺序，而不是只保留失稳前问题。
