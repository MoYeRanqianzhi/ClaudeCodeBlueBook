...
## 1. 世界是否沿同一 witness chain 合法进入模型

关键追问：

- 为什么同一条 `Authority -> Boundary -> Transcript -> Lineage -> Continuation` witness chain 必须同时解释当前、下一步与接手后的合法进入。
- 为什么 instruction 更长，却未必更能通过 `same-world test`。
- 为什么一旦这条 witness chain 不能在失稳前被审读，维护者就会把 Prompt 误写成文案效果。
...
- Prompt 线唯一 canonical object chain 是 `message lineage -> section registry / stable prefix -> protocol transcript -> continuation object -> continue qualification verdict`
