...
最值钱的纪律也只该再多记一句：`navigation/` 只管 route，不签 truth；一旦 route words 开始代替 `governance key / typed ask / decision window / current-truth surface` 说话，它就在制造第二前门。
...
- `09`
  宪法总图，先定义你在问哪一张控制面。
- `05`
  模仿对象校正，先防止 Prompt 魔力、安全定价和源码先进性被读成表象技巧。
- `15`
