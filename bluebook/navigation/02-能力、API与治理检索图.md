...
6. 治理层：账号、组织、GrowthBook、策略、allowlist、trusted device、remote managed settings、风控恢复共同约束运行时。
...
1. 命令层、工具层、任务层、团队层的统一手册。
2. SDK/control/session/remote 的统一接入矩阵。
3. prompt 魔力、安全分层、token 经济三者的同构关系。
4. 源码先进性与热点大文件债务如何同时成立。
