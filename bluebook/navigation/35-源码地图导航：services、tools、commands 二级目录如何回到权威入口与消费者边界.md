# 源码地图导航：services、tools、commands 二级目录如何回到权威入口与消费者边界
...
1. 问 canonical ladder、rung 定义与降格规则
   - 先回 `01 + guides/102`
2. 问 repo-specific 地图正文、authority file、consumer subset 与 seam type locator
   - 先回 `api/46-50`
...
4. reject path：
   - 一旦某个 service 看起来像权威真相，却只是在投影，先回 `../api/46` 的 authority file，再回 `../01-源码结构地图.md` 与 `../architecture/84-权威面与反僵尸图谱：single-writer surfaces、409 adoption、bridge pointer freshness与release shaping.md` 的 seam / object owner。
...
4. reject path：
   - 一旦命令看起来像公开能力，却更像内部命令面，先回 `commands.ts` 与 `../api/48` 的 authority 段，再回 `../01-源码结构地图.md` 的 seam type locator。
