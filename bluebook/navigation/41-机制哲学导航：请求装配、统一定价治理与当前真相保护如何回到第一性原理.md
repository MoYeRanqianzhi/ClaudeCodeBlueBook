...
Prompt `66 -> 69 -> 72 -> 78 -> 47 -> 50` 与结构 `guides/83 / 92 -> playbooks/61 / 67 -> casebooks/54 / 63` 因而都只配做专题侧门：前者承接 same-world compiler 的继续资格、residual reopen 与 threshold liability，后者承接 builder-facing template 与 later reject path；它们都不负责第一次定义 Prompt `same-world test`、统一定价治理或 `current-truth protection`。
...
- 更短的前门校准句是：先看 `Authority / Boundary / Transcript / Lineage / Continuation`，再谈 Prompt 前门；如果这五环里有一环答不出来，解释就还停在文案层
