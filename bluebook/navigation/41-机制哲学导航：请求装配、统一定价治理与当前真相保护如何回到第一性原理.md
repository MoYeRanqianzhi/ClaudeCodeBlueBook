...
- 为什么同一条 `message lineage -> section registry -> protocol transcript -> continuation qualification` witness chain 能同时约束当前、下一步与接手后的合法进入。
- 为什么一旦离开 `message lineage`、`projection consumer`、`protocol transcript`、`continuation qualification` 与 `cache-safe forks`，这条 witness chain 就会立刻失真。
