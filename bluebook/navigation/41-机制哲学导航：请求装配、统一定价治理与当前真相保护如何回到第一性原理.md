...
- 把世界合法进入模型压回“先有 same-world witness，再谈为什么它能维持同一世界”的第一性原理
- 如果必须压成一句，只写：先确认谁定义世界、模型实际消费什么、同一身份如何延续，以及继续资格由谁裁定；`Explainability` 只负责给失稳命名。更细的 `Authority -> Boundary -> Transcript -> Lineage -> Continuation -> Explainability` 仍回 `../philosophy/84`
