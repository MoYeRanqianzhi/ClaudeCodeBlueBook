...
Anthropic 官方 engineering 还把这条线从 production 角度补硬了一层；本页因此只保留一条验收问句：`same-world compiler` 若真成立，它是否同时让 verify、delegate、tool choice 与 feedback loop 共享同一 witness 与 effort budget；更完整的 why 统一回 `../philosophy/84`，不在本页重造 heuristics 清单或 object chain。
