...
- `41` 在 Prompt 线上只回答“为什么必须先有 same-world compiler contract”，不再重写 `84 / 51` 已拥有的公式、witness order 或实现顺序。
- 如果必须压成一句，只写：先确认谁定义世界、模型实际消费什么、同一身份如何延续，以及继续资格由谁裁定；更细的 `first-reject order` 与 object-level witness 继续回 `../philosophy/84` 与 `../guides/51`。
