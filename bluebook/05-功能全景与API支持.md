...
### 2.8 状态回写与宿主 readback / lease-checkpoint 面
...
这说明它给宿主暴露的不是“只读消息流”，而是“消息流 + 可查询状态快照 / lease-checkpoint”的组合 API。
但 `worker_status / pending_action / permission_mode / model` 默认仍只是 snapshot / projection：它们最多帮助宿主复核这段 authority lease 当前走到哪一步，不单独签发 host truth。
...
- `continuation / recovery carrier` 面

但 `resume / continue / rewind` 暴露的是恢复载体，不是旧 authority 的自动续租。只有同一条 authority lease 仍然闭合、且 `cleanup trigger` 已满足时，durable asset 才配继续带入；否则这些接口只负责把需要重审的东西重新带回现场。
...
1. 入口是否存在且语义明确
2. 当前公开实现是否已足够支持 `current-truth claim state`；若还不能 promotion，就只配写成 `provisional claim + unresolved-authority note`
3. 是否进入主路径或稳定 `consumer subset`
4. 是否已经形成对外 `product promise`

只有四问过关，才允许把某个入口写成“支持”。若第二问还答不出 promotion 证据，就只配写成“可见实现 / provisional claim / 声明痕迹”，不借 taxonomy 代签 current truth。
