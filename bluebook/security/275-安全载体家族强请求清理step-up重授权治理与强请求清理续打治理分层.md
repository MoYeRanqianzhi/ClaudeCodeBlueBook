...
## 1. 为什么在 `274` 之后还必须继续写 `275`

`274-安全载体家族强请求清理用时重验证治理与强请求清理step-up重授权治理分层`
已经回答了：
...
3. `src/tools/McpAuthTool/McpAuthTool.ts:57-60,134-196` 的 OAuth 完成后 reconnect / swap real tools path
4. `src/components/mcp/MCPRemoteServerMenu.tsx:263-289` 的 `preserveStepUpState: true` 与 auth success copy
5. `src/components/mcp/MCPAgentServerMenu.tsx:65-71` 与 `src/cli/print.ts:3310-3508` 的 future-run / control-plane choreography
...
2. `stronger-request continuation governance`
   负责决定先前那个被挡下来的 stronger request，是否仍应被视为“同一个 request”的合法续打，以及应由谁、在什么条件下、以什么预算继续它
...
4. `MCPRemoteServerMenu` 在 re-auth 前会显式 `preserveStepUpState: true`，说明它愿意保留 higher-authority intent，但 auth 完成后的 UI 文案仍只签 `Connected` / `Reconnected` / `still requires authentication` / `reconnection failed`
5. `print.ts` 在 `mcp_authenticate` / `mcp_oauth_callback_url` 里主要签 `authUrl`、`requiresUserAction`、auth completion 与 reconnect / dynamic registration 更新；这仍不是“原 stronger cleanup request 现在已被当作同一请求继续”
6. `MCPAgentServerMenu` 甚至直接写 `The server will connect when the agent runs.`；这说明 auth success 也可以只签 future readiness，而完全不签 immediate continuation
...
2. step-up-state-means-same-request illusion
   只要更高 scope 被保留，就误以为 old request identity 也一并被保留
3. tools-available-means-old-call-completed illusion
   只要工具重新可见，就误以为先前被挡下的调用已经被代表性完成
4. reconnect-success-means-causal-link-preserved illusion
   只要 reconnect 成功，就误以为旧 request 的因果链仍然完好无损
5. higher-authority-means-retry-budget-approved illusion
   只要 scope 已升够，就误以为 retry 次数、重试方式与 consent 也一并被批准
6. auth-done-means-same-request-done illusion
...
2. 如果 `preserveStepUpState: true` 已经等于 old request continuity，为什么 `MCPRemoteServerMenu` 在 OAuth 成功后仍只说 `Connected` / `Reconnected`，而不说 “the blocked request is now resumed”？
   因为 higher-authority intent continuity 不拥有替 old request replay 签字的资格。
3. 如果工具 `should now be available` 已经等于原请求继续成功，为什么 `toolExecution.ts` 还要再明确说 `then retry this call` 或 `You may retry it if you would like`？
   因为 availability truth 不拥有替 old call continuation 签字的资格。
4. 如果 auth 完成已经自动恢复原 stronger cleanup request，为什么 `MCPAgentServerMenu` 还会说 `The server will connect when the agent runs`？
...
`toolExecution.ts:586-606,1068-1105`
再给出第二组强证据。
...
`MCPRemoteServerMenu.tsx:263-289`
同样如此。

这里最值钱的新细节是：

1. re-auth 前显式 `revokeServerTokens(..., { preserveStepUpState: true })`
2. `auth.ts:578-617` 会把 `stepUpScope` 与 `discoveryState` 跨 revoke 保留下来
3. `performMCPOAuthFlow()` 又会在下一次 flow 里读回 cached `stepUpScope`

这说明 UI 愿意保留的是：

`higher-authority target continuity`
...
5. 在 manual callback path 明确把 reconnect 留给另一条 control choreography
...
## 7. 为什么这层不等于 `274` 的 step-up gate

这里必须单独讲清楚，
否则很容易把 `274` 和 `275` 写成一篇。
...
5. `state continuity as ceiling control`
   step-up scope 的跨 revoke / re-auth 保留，只证明更强授权目标被保留，不证明旧 request identity 也被保留
6. `layered actor design`
...
repo 已经在 `callMCPToolWithUrlElicitationRetry()` 的 bounded retry + two-phase waiting grammar、`toolExecution.ts` 的 explicit retry wording、`McpAuthTool` / `MCPRemoteServerMenu` / `MCPAgentServerMenu` 的 availability-vs-readiness ceiling、`print.ts` 的 auth/reconnect control choreography，以及 `preserveStepUpState` 只保留 higher-authority target 而不保留 old request replay decision 的分层设计上，清楚展示了 stronger-request continuation governance 的独立存在；因此 `artifact-family cleanup stronger-request cleanup-step-up reauthorization-governor signer` 仍不能越级冒充 `artifact-family cleanup stronger-request continuation-governor signer`。
