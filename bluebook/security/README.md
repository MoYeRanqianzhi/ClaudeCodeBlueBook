...
`security/` 当前有 351 篇正文，范围 `00-350`；`appendix/` 当前有 334 篇速查文档；`source-notes/` 当前有 201 篇源码剖面。
...
- `147-350` 这一整段已经证明：弱层只能说明局部事实，强层才有权宣布更高阶治理结果；任何弱层都不能越级冒充强层。
...
- 如果你只先判断“cleanup 与 forgetting 为什么不能混成一个结果词”，从 `147-350` 进入。失败信号：还在把完成、终局、遗忘、清理写成一个“已经没事了”的总结果。
...
- 想直看治理链主干：先读 `147-350 -> appendix/131-334 -> source-notes/01-201`。
