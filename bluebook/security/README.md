...
`security/` 当前有 288 篇正文，范围 `00-287`；`appendix/` 当前有 271 篇速查文档；`source-notes/` 当前有 138 篇源码剖面。
...
- 任何 user-facing 状态、诊断、压缩或导出入口，都只配读取已外化的 truth-surface、decision window 与 cleanup verdict，不配定义治理真相或继续资格；用户侧恢复读法统一回 `risk/`。
...
- `147-287` 这一整段已经证明：弱层只能说明局部事实，强层才有权宣布更高阶治理结果；任何弱层都不能越级冒充强层。
...
- 如果你只先判断“cleanup 与 forgetting 为什么不能混成一个结果词”，从 `147-287` 进入。失败信号：还在把完成、终局、遗忘、清理写成一个“已经没事了”的总结果。
...
- 想看 stronger-request cleanup ladder 与更高阶 continuation / completion / finality / forgetting / constitution governor：从 `197-287` 进入。
...
- 想直看治理链主干：先读 `147-287 -> appendix/131-271 -> source-notes/01-138`。
