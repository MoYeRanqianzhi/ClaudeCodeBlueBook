...
`security/` 当前有 301 篇正文，范围 `00-300`；`appendix/` 当前有 284 篇速查文档；`source-notes/` 当前有 151 篇源码剖面。
...
- `147-300` 这一整段已经证明：弱层只能说明局部事实，强层才有权宣布更高阶治理结果；任何弱层都不能越级冒充强层。
...
- 如果你只先判断“cleanup 与 forgetting 为什么不能混成一个结果词”，从 `147-300` 进入。失败信号：还在把完成、终局、遗忘、清理写成一个“已经没事了”的总结果。
...
- 想看 stronger-request cleanup ladder 与更高阶 continuation / completion / finality / forgetting / constitution governor：从 `197-300` 进入。
...
- 想直看治理链主干：先读 `147-300 -> appendix/131-284 -> source-notes/01-151`。
