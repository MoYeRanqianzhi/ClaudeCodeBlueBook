...
- 想看为什么即使缺席层级已经分清，想把结论说得更强的人仍必须自己补齐缺失层证明，而不能把默认推定外包给读者：`122` -> `123`
- 想把 `123` 的长文压成一张举证责任矩阵，快速看出不同 stronger claim 缺哪一层证明、责任在谁、以及哪些 burden shift 被禁止：`123` -> `appendix/107`
- 想看为什么即使举证责任已经明确，一旦补证失败，系统也不能继续用模糊词硬撑 stronger claim，而必须降回真实层级：`123` -> `124`
- 想把 `124` 的长文压成一张降级语法矩阵，快速看出不同 failed stronger claim 应如何改口、保留什么真相，以及哪些模糊词必须被禁止：`124` -> `appendix/108`
- 想看为什么即使降级语法已经明确，系统仍不能让不同层级继续共用近义词，而必须把 observed、wired、hosted、verified、equivalent 固定成协议词：`124` -> `125`
...
- 想看为什么即使能力协商与版本地板已经存在，Claude Code 仍不把所有代际错位压成同一句“版本太旧”，而是继续区分 app 可见性偏斜、compat tag 错位、grammar 错版与握手代际竞争：`141` -> `142`
