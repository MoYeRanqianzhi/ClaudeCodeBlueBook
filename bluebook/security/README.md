...
`security/` 当前有 270 篇正文，范围 `00-269`；`appendix/` 当前有 253 篇速查文档；`source-notes/` 当前有 120 篇源码剖面。
...
- 如果你只先判断“cleanup 与 forgetting 为什么不能混成一个结果词”，从 `147-269` 进入。失败信号：还在把完成、终局、遗忘、清理写成一个“已经没事了”的总结果。
...
- 想看 stronger-request cleanup ladder 与更高阶 continuation / completion / finality / forgetting / constitution governor：从 `197-269` 进入。
...
- 想直看治理链主干：先读 `147-269 -> appendix/131-253 -> source-notes/01-120`。
