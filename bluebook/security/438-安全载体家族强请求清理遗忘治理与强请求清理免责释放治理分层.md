...
## 1. 为什么在 `437` 之后还必须继续写 `438`

`437-安全载体家族强请求清理终局治理与强请求清理遗忘治理分层`
已经回答了：
...
1. `src/cli/structuredIO.ts:374-398` 的 duplicate `control_response` ignore path 会在 `resolvedToolUseIds` 命中时明确忽略重复回包
2. `src/cli/structuredIO.ts:395-398` 的 `unexpectedResponseCallback` / unknown-response ignore path 会把 unknown old echo 分流成 compensate-or-refuse
3. `src/cli/print.ts:5241-5304` 的 `handleOrphanedPermissionResponse()` 会把 orphaned permission 明确分成 re-enqueue 与 `return false`
4. `src/cli/print.ts:5272-5284` 的 `already handled` / `already resolved in transcript` 会把 stronger truth 明确写成 no-more-duty 理由
5. `src/cli/structuredIO.ts:401-409` 的 `onControlRequestResolved(...)` 会主动取消 claude.ai 侧 stale permission prompt
...
`safe enough to forget，不等于 no longer owing duty。`
