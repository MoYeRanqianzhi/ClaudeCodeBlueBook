...
## 1. 为什么在 `213` 之后还必须继续写 `214`

`213-安全载体家族强请求清理完成治理与强请求清理终局治理分层` 已经回答了：
...
## 9. 为什么这层不等于 `213` 的 stronger finality gate

这里必须单独讲清楚，
否则容易把 `214` 误读成 `213` 的尾注。
...
1. `213` 的典型形态是 `tool_result`、`files_persisted`、`idle`、`state_restored`
2. `214` 的典型形态是 `resolvedToolUseIds`、`handledOrphanedToolUseIds`、duplicate ignore、bounded eviction
