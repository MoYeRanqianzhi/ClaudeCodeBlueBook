...
| 主题 | 关键源码 | 这里回答的问题 |
| --- | --- | --- |
| message tombstone control signal | `src/query.ts:712-719`; `src/utils/messages.ts:2954-2957`; `src/QueryEngine.ts:758-760` | 为什么 orphaned message 不会只静默消失，而会留下 machine-consumable tombstone |
| tombstone execution chain | `src/screens/REPL.tsx:2646-2648`; `src/utils/sessionStorage.ts:1470-1473,925-945` | 为什么 tombstone 不是描述性文字，而是驱动 UI / transcript 删除的正式控制信号 |
| plugin orphan marker | `src/utils/plugins/cacheUtils.ts:56-72,145-171`; `src/services/plugins/pluginOperations.ts:523-547` | 为什么 `.orphaned_at` 是对象退役后的最小残留标记，而不是单纯垃圾文件 |
| marker-driven visibility control | `src/utils/plugins/orphanedPluginFilter.ts:1-15,35-38`; `src/main.tsx:2544-2568` | 为什么 marker 还会继续塑造 future search / visibility world |
...
`REPL.tsx:2646-2648`
收到 tombstoned message 时会：

1. 从 `messages` state 里过滤掉对应消息
2. 调 `removeTranscriptMessage(uuid)`

而 `sessionStorage.ts:1470-1473`
又把这个动作落到 transcript 存储层。
甚至 `sessionStorage.ts:925-945`
还专门处理 tombstone slow path、truncate 与大文件 rewrite。
...
更重要的是 `processOrphanedPluginVersion()` 所在的
...
所以 stronger-request cleanup 线真正缺的不是抽象上“承认旧世界会退役”，
而是把：
...
`marker future effect`

`marker expiry owner`

明确制度化。

## 10. 苏格拉底式边界自校：什么才算真正的墓碑治理

### 诘问一

`如果只有 retirement decision，没有 residual marker，这算 tombstone governance 吗？`

不算。
这最多是 sunset governance。
墓碑治理至少还要回答：
...
`如果只有 marker，没有后续 consumer，这算 tombstone governance 吗？`

也不稳。
没有 consumer，
marker 很容易退化成普通元数据。
真正的 tombstone grammar 必须能被后续系统继续读取并采取动作。

### 诘问三

`如果只有一次 notification，没有 future effect，这算 tombstone governance 吗？`

仍然不完整。
notification 最多是 residual surface 之一，
还不足以回答搜索、恢复、审计与误复活防线如何被约束。
...
`退役治理回答“何时结束”，墓碑治理回答“结束后留下什么最小残留事实，并由谁继续消费”。`

但源码同样也提醒我们不要越级：

`别把 repo 某些子系统已经存在的 tombstone-like grammar，直接误写成 stronger-request cleanup 线已经拥有同等完备的 cleanup tombstone constitution。`
