...
`diskOutput.ts:33-55` 的价值不只是在“换了路径”。
...
`fileHistory.ts:951-957` 与 `sessionEnvironment.ts:15-21` 说明：
...
`debug.ts:230-249` 与 `cleanup.ts:391-428` 又说明：
...
1. 高风险 live runtime carrier 值得被抬进专门的 temp constitution，而不是一律回收进统一历史桶
2. shared-readable surface 最容易诱发误统，必须把“可读”和“可删”分法单独讲清
3. project-session persistence world 与 home-root operational world 的差异，不只是路径差异，而是 owner model 与 cleanup gate 的差异
4. path override 一旦存在，cleanup constitution 就不能只靠默认目录假定维持
5. env-selected external carrier 会逼迫系统承认：不是所有 cleanup constitution 都位于 repo-local plane 内
