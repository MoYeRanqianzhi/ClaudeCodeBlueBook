# savePluginOptions、saveMcpServerUserConfig 与 PluginOptionsFlow 的重配置治理边界
...
`cleanup 线未来谁来决定旧对象回来后还配不配拿 seat，`

而是：

`cleanup 线即便让旧对象重新拿回了 seat，谁来决定它现在应按哪组 options、哪组 secrets、哪组 channel config 重新工作。`
...
`Claude Code 已经在 plugin 线上明确展示：对象重新获得 authority seat，并不自动回答它现在该按哪组 current config truth 重新工作；cleanup 线当前缺的不是这种思想，而是这套 reconfiguration governance 还没被正式接到旧 path、旧 promise 与旧 receipt 世界上。`
...
`Claude Code 在 plugin 线已经明确把“对象拿回 seat”和“对象按哪组 current config 重新工作”拆成两层；cleanup 线当前缺的不是文化，而是这套 reconfiguration governance 还没被正式接到旧 cleanup artifact family 上。`
...
从 cleanup 研究角度看，  
这给出的启示不是“以后也去搞 keychain”，  
而是：

`一旦 restored object 的配置真相横跨多个 store，谁来决定写入顺序、scrub 规则与 fallback honesty，本身就是 reconfiguration governance。`
...
这对 cleanup 线极其重要。  
因为将来旧 path、旧 promise、旧 receipt 即便重新被允许出现，  
也仍不自动说明：

`它们现在已经带着足够 current declaration work-ready。`
...
它把“还缺哪些当前声明”正式提升成了一类 machine-readable state，  
而不是把它留在作者脑内或注释里。
...
这不是 UX 修辞。  
这是治理层级已经长到 UI protocol surface 的证据。

## 8. 这篇源码剖面给主线带来的四条技术启示

### 启示一

repo 已经在 top-level options 线上明确展示：

`seat return != option truth return`

### 启示二

repo 已经在 per-server payload 线上明确展示：

`seat return != channel config truth return`

### 启示三

repo 已经在 `needs-config` 与 `unconfigured` 判定里明确展示：

`current object visibility != current config validity`

### 启示四

repo 已经在 `configured` / `saved` / `take effect` 句法里明确展示：

`configuration persistence != active application`

这四句合起来，  
正好说明为什么 cleanup 线未来不能把 re-entitlement governance 直接偷写成 complete reconfiguration。
...
`repo 在 savePluginOptions、saveMcpServerUserConfig、getUnconfiguredOptions、getUnconfiguredChannels、PluginOptionsFlow 与 ManagePlugins 的 outcome grammar 上已经明确展示了 reconfiguration governance 的存在；因此 artifact-family cleanup re-entitlement-governor signer 仍不能越级冒充 artifact-family cleanup reconfiguration-governor signer。`
