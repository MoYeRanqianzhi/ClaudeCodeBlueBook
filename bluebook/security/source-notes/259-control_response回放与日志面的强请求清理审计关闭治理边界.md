...
## 7. 为什么这层不等于 `258` 的 archive-close gate

这里必须单独讲清楚，
否则容易把 `259` 误读成 `258` 的尾注。
...
1. `258` 的典型形态是 lifecycle close、stale prompt teardown、queue non-admission、inline completed contract
2. `259` 的典型形态是 replay propagation、replay enqueue、debug trace、diagnostics parse
