...
## 8. 为什么这一层不等于 `100` 的 audit-close gate

这里必须单独讲清楚，
否则最容易把 `101` 误读成 `100` 的尾注。
...
1. `100` 的典型形态是 replay propagation、debug trace、diagnostics parse
2. `101` 的典型形态是 debug logfile materialization、generic retention delete、transcript truncate、workspace unlink
...
`stronger-request cleanup 线真正缺的，不只是“谁来宣布 old echo 现在已退出 replay / debug / diagnostics 审计世界”，还包括“谁来宣布它现在也已从 debug log、diagnostic logfile、transcript 与 workspace 这些 carrier 世界被真正销毁”。`
