# duplicate 与 orphan response 分流中的强请求免责释放治理边界
...
`cleanup 线未来谁来决定 old request identity 什么时候开始可以安全淡出，`

而是：

`cleanup 线如果未来已经进入 forgetting 阶段，谁来决定旧 duplicate/orphan echo 什么时候已经不再值得补偿接回。`
...
| orphaned replay refusal | `src/cli/print.ts:5238-5305` | 为什么 orphaned echo 有时值得补偿接回，有时又被正式拒绝 |
| transcript-resolved amnesty | `src/cli/print.ts:5278-5284` | 为什么 transcript-level stronger truth 可以释放 further compensation duty |
...
## 9. 为什么这层不等于 `182` 的 forgetting gate
...
1. `182` 的典型形态是 retained IDs、bounded eviction、anti-duplicate memory
2. `183` 的典型形态是 duplicate ignore、orphan compensate-or-refuse、transcript-resolved amnesty、stale prompt teardown
...
`只要补出 old-request memory 的 safe forgetting grammar，cleanup 线就已经知道什么时候不再对旧 echo 负有责任。`
...
`cleanup 线真正缺的，不只是“谁来宣布 old request identity 什么时候开始可以忘掉”，还包括“谁来宣布旧 duplicate/orphan echo 现在已经不再对系统构成任何责任线程”。`
