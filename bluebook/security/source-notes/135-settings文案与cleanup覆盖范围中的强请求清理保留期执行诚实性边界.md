# settings文案与cleanup覆盖范围中的强请求清理保留期执行诚实性边界
...
| wider visible cleanup coverage | `src/utils/cleanup.ts:33-45,155-257,300-347,350-428,575-595` | 实际 executor 当前 visibly 覆盖了哪些 carrier family，以及为什么没有统一 receipt |
...
## 9. 为什么这一层不等于 `134` 的 retention-governance gate

这里必须单独讲清楚，
否则最容易把 `135` 误读成 `134` 的尾注。
...
1. `134` 的典型形态是 `cleanupPeriodDays`、`rawSettingsContainsKey()`、background scheduler、visible debug cleanup
2. `135` 的典型形态是 transcript-centered wording、startup-delete wording、future suppression、wider executor coverage、missing unified receipt、diagnostics opacity
