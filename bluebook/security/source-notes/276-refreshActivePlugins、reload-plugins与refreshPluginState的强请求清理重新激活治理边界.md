...
| 主题 | 关键源码 | 这里回答的问题 |
| --- | --- | --- |
| Layer-3 reactivation primitive | `src/utils/plugins/refresh.ts:1-18,59-145` | 为什么 active world swap 是单独主权，不是 config save 的自然尾气 |
| user-initiated take-effect contract | `src/commands/reload-plugins/index.ts:1-16`; `src/commands/reload-plugins/reload-plugins.ts:10-56` | 为什么 `/reload-plugins` 是正式 reactivation command，而不是多余包装 |
...
`src/commands/plugin/ManagePlugins.tsx:1645-1707`
很硬。
...
而
`src/services/plugins/PluginInstallationManager.ts:52-59,135-180`
又把制度写得很清楚：
...
`src/cli/print.ts:1749-1769,3071-3080`
很值钱。

这里写得很清楚：

1. `refreshPluginState()` 显式调用 `refreshActivePlugins()`
2. 注释直接写出它会重载 commands / agents / hooks 并 bump `pluginReconnectKey`
3. remote/headless reload 路径会先 `redownloadUserSettings()`
4. 再 `notifyChange('userSettings')`
5. 然后才跑 `refreshActivePlugins()`

而
`src/state/AppStateStore.ts:209-215`
更直接把制度压成一句：
...
5. 它把 stale active world 视为治理事实，而不是界面噪音
...
2. repo 已经给 active-world swap 单独命名 `Layer-3 refresh primitive`，说明 reactivation 不是实现细节。
3. repo 已经在 `needsRefresh` 上明确展示：stale active world 应被显化，而不是被偷偷自动覆盖。
4. repo 已经在 new install / update split 上明确展示：同类磁盘变化也可以采用不同 activation rule。
5. repo 已经在 headless `refreshPluginState()` 上明确展示：自动接管也必须是被制度化的显式路径。
6. stronger-request cleanup 如果未来要长出 reactivation-governor plane，关键不只是“保存了新 truth”，而是明确这组 truth 何时、以什么 mode policy、通过哪个 trigger 接管 running session。
...
1. 如果 reconfiguration grammar 已经足够强，为什么还要再拆 reactivation？
   因为 truth 已声明，不等于 truth 已 live。
2. 如果 UI 已显示 `Configuration saved`，是不是就说明当前 session 一定在用它？
   不是。源码明确把 `saved` 与 `take effect` 分开。
3. 如果 `/reload-plugins` 存在，是不是说明所有模式都必须手动触发？
   也不对。headless mode 可以 auto-consume，但这是另一条被明说的制度路径。
4. 如果 new install 可 auto-refresh，是不是说明 update 也应自动接管？
   不能这样推。repo 明确把同属 disk change family 的两类场景分配给不同 policy。
5. 如果 stale signal 只是通知文本，是不是说明它不属于安全治理？
   恰恰相反。`needsRefresh` 被写进 AppState，并有明确 consume 语义，本身就是治理事实。
6. 如果 active swap 已有 Layer-3 primitive，是不是说明它只是实现细节？
   不是。正因为它被命名、被复用、被分配 mode policy，它才是治理层对象，而不是细节。
7. 如果 cleanup 线还没有显式 reactivation 代码，是不是说明这层还不值得写？
   恰恰相反。越是缺这层 grammar，越容易把“配置已闭合”误说成“当前运行世界已接管”。
8. 如果我继续把 reactivation 当成 reconfiguration 的副产品，会漏掉什么？
   会漏掉 stale signal、mode-sensitive policy、take-effect trigger 与 active-world swap 这些真正决定 truth 何时 live 的制度对象。

这一串反问最终逼出一句更稳的判断：

`reactivation 的关键，不在配置有没有保存，而在系统能不能正式决定这组 persisted truth 何时变成 current running truth。`
...
`cleanup 线真正缺的不是“当前该按哪组 truth 工作”，而是“谁来决定这组 truth 何时真正接管当前运行世界”。`
