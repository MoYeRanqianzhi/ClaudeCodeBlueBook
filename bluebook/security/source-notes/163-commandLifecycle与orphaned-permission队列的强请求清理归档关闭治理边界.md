...
| 主题 | 关键源码 | 这里回答的问题 |
| --- | --- | --- |
| lifecycle close on every `control_response` | `src/cli/structuredIO.ts:372-409` | 为什么 duplicates / orphans 即使已 no-more-duty 也要被正式 completed |
...
## 8. 为什么这层不等于 `162` 的 liability-release gate

这里必须单独讲清楚，
否则容易把 `163` 误读成 `162` 的尾注。
...
1. `162` 的典型形态是 duplicate ignore、unknown-response no-op、transcript-resolved amnesty、compensate-or-refuse
2. `163` 的典型形态是 lifecycle completed、stale prompt teardown、queue non-admission、inline completed contract
