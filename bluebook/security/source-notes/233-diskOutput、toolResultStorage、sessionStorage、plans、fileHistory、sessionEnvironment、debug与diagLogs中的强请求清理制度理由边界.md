...
| persisted inspection rationale | `src/utils/toolResultStorage.ts:1-5,94-124` | 为什么 tool-results 需要被持久化而不是只留在 live runtime |
...
| restore / replay rationale | `src/utils/fileHistory.ts:951-957`; `src/utils/sessionEnvironment.ts:15-22,60-143`; `src/utils/cleanup.ts:305-387` | 为什么 file-history 与 session-env 虽同居 home-root/session-dir world，但服务不同 duty |
| operational debugging rationale | `src/utils/debug.ts:30-68,91-111,216-235`; `src/utils/cleanup.ts:391-428` | 为什么 debug logs 不是 generic retained artifact，而是 operational surface |
