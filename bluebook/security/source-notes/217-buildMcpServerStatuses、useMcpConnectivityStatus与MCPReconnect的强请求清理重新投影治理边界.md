...
| 主题 | 关键源码 | 这里回答的问题 |
| --- | --- | --- |
| control projection | `src/cli/print.ts:1612-1698,2957-2960,3127-3128` | 为什么 control readers 读到的是结构化 current truth，而不是 raw reintegration state |
| manager / panel status grammar | `src/commands/plugin/ManagePlugins.tsx:511-519`; `src/components/mcp/MCPListPanel.tsx:306-337` | 为什么 UI status surface 会重新压缩同一 truth |
| selective notification grammar | `src/hooks/notifs/useMcpConnectivityStatus.tsx:13-83` | 为什么 notification surface 会选择性重讲某些 truth、沉默另一些 truth |
...
`src/commands/plugin/ManagePlugins.tsx:511-519`
的 `getMcpStatus()`
把 truth 压成：
...
`src/hooks/notifs/useMcpConnectivityStatus.tsx:13-83`
特别值钱。
...
## 10. 苏格拉底式自反诘问：我是不是又把“已经并回当前世界”误认成了“所有读者已经被正确告知”
