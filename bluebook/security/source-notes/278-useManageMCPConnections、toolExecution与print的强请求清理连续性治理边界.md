...
`continuity 的关键，不在对象此刻有没有 ready，而在系统能不能正式决定 ready truth 中断之后这条可用性时间线还值不值得继续维持。`

如果再追问“怎样做得更好”，
更好的方法也不是简单给 retry 多加几个参数，
而是继续把四个问题拆开：
...
`Claude Code 在 continuity budget、disconnect retry lifecycle、stale retry line revocation、operator stop/restart control、runtime downgrade 与 pool-level repair 上已经明确展示了 continuity governance 的存在；因此 artifact-family cleanup stronger-request cleanup-readiness-governor signer 仍不能越级冒充 artifact-family cleanup stronger-request cleanup-continuity-governor signer。`

再压成一句：

`cleanup 线真正缺的不是“对象此刻能不能用”，而是“这份可用性能不能在时间里继续被制度负责”。`
