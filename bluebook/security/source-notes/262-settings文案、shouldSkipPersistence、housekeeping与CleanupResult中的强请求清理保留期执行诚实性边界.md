...
| immediate future suppression | `src/utils/sessionStorage.ts:954-970` | 哪部分 truth 会立刻生效 |
...
| wider visible cleanup coverage | `src/utils/cleanup.ts:155-190,300-303,305-428,575-598` | 实际 executor 当前 visibly 覆盖了哪些 carrier family |
...
但 `cleanup.ts:155-190,300-303,305-428,575-598`
...
## 9. 为什么这一层不等于 `261` 的 retention-governance gate

这里必须单独讲清楚，
否则最容易把 `262` 误读成 `261` 的尾注。
...
1. `261` 的典型形态是 `cleanupPeriodDays`、validation veto、housekeeping admission、coverage declaration
2. `262` 的典型形态是 transcript-centered wording、startup wording、future-suppression truth、local-only `CleanupResult` 与 diagnostics uncovered honesty path
...
`只要 stronger-request retention policy 已经被定义，当前系统就已经诚实说明这条 policy 现在做到哪一步。`

而是：

`repo 已经在 transcript-centered declaration wording、future suppression、delayed runtime admission、wider executor coverage、local receipt gap 与 diagnostics uncovered honesty path 上，清楚展示了 stronger-request retention-enforcement honesty governance 的独立存在；因此 artifact-family cleanup stronger-request retention-governor signer 仍不能越级冒充 artifact-family cleanup stronger-request retention-enforcement-honesty-governor signer。`
