...
`Claude Code 已经把 continuation 与 completion 明确拆成不同 ceiling：前者处理 retry permission、accept/decline、budget 与 could-not-complete，后者处理 completed progress、normalized result、abort-not-complete discipline 与 explicit success tool_result。`
...
| outer completion progress | `src/services/mcp/client.ts:1848-1892` | 为什么 `status: 'completed'` 只在 `mcpResult` 返回后才发 |
...
5. 若是 `AbortError`，就只返回 `{ content: undefined }`
...
2. `127` 的典型形态是 could-not-complete、completed progress、completed successfully、normalized result、success tool_result
