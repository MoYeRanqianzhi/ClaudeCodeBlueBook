...
## 9. 为什么这层不等于 `288` 的 forgetting gate

这里必须单独讲清楚，
否则容易把 `289` 误读成 `288` 的尾注。
...
1. `288` 的典型形态是 retained IDs、bounded eviction、anti-duplicate memory
2. `289` 的典型形态是 duplicate ignore、orphan compensate-or-refuse、transcript-resolved amnesty、stale prompt teardown
