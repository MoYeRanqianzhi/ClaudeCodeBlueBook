...
repo 已经在 structured `McpServerStatus[]` 上明确展示：

`machine-readable current truth`

本身也是 reader-specific projection。

### 启示二

repo 已经在 manager / panel enum 与 label 上明确展示：

`UI grammar`

不是 raw reintegration state 的简单回放。

### 启示三

repo 已经在 notification 选择性沉默上明确展示：

`not every positive truth should be proactively published`

### 启示四

repo 已经在 health glyph grammar 上明确展示：

`narrow probe readers`

只应拿到 narrow projection。

### 启示五

repo 已经在 control success / error envelope 上明确展示：

`payload strength`

本身也是 truth governance 的一部分。
...
`Claude Code 在 structured control payload、manager/panel status grammar、selective notification policy、health glyph grammar 与 reconnect dialog copy 上已经明确展示了 reprojection governance 的存在；因此 artifact-family cleanup stronger-request cleanup-reintegration-governor signer 仍不能越级冒充 artifact-family cleanup stronger-request cleanup-reprojection-governor signer。`

再压成一句：

`cleanup 线真正缺的不是“谁来决定对象何时重新进入当前世界”，而是“谁来决定这件事怎样被不同读者重新讲述”。`
