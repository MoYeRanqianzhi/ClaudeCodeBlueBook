...
## 9. 为什么这层不等于 `256` 的 forgetting gate

这里必须单独讲清楚，
否则容易把 `257` 误读成 `256` 的尾注。
...
1. `256` 的典型形态是 retained IDs、bounded eviction、anti-duplicate memory
2. `257` 的典型形态是 duplicate ignore、orphan compensate-or-refuse、transcript-resolved amnesty、stale prompt teardown
...
## 10. 苏格拉底式自反诘问：我是不是又把“现在可以开始忘掉它”误认成了“现在已经彻底不再欠它任何责任”
...
`stronger-request cleanup 线真正缺的，不只是“谁来宣布 old request identity 什么时候开始可以忘掉”，还包括“谁来宣布旧 duplicate/orphan echo 现在已经不再对系统构成任何责任线程”。`
