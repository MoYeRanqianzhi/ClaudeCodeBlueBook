...
## 7. 为什么这层不等于 `131` 的 archive-close gate

这里必须单独讲清楚，
否则容易把 `132` 误读成 `131` 的尾注。
...
1. `131` 的典型形态是 lifecycle close、stale prompt teardown、queue non-admission、inline completed contract
2. `132` 的典型形态是 replay propagation、replay enqueue、debug trace、diagnostics parse
...
## 8. 技术启示：成熟安全系统为什么要把“退出现场”与“退出证据世界”拆开

这组源码给出的设计启示至少有四条。

第一条：

`operator-world close` 与 `auditor-world close` 必须是两个独立 verdict。
...
debug trace 不是工程噪音，
而是 refusal / ignore / compensate 这些判断的解释资产。

第四条：

diagnostics parse 说明“这条消息仍被看见”，
而“仍被看见”本身就是 audit-close 尚未成立的证据。
