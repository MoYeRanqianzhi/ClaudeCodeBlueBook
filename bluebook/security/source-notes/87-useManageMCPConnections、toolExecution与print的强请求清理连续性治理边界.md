...
| 主题 | 关键源码 | 这里回答的问题 |
| --- | --- | --- |
| continuity constitution | `src/services/mcp/useManageMCPConnections.ts:87-90` | 为什么 repo 把“继续争取可用性多久”建模成正式制度参数 |
...
| operator continuity control | `src/services/mcp/useManageMCPConnections.ts:1043-1128` | 为什么 manual reconnect、disable 与 re-enable 都在改写 continuity authority |
| runtime readiness revocation | `src/services/tools/toolExecution.ts:1599-1628` | 为什么 ready truth 会被运行时证据持续撤销 |
| SDK pool continuity repair | `src/cli/print.ts:1389-1428` | 为什么 continuity 不只是单连接问题，而是 current usable pool 的持续维持问题 |
...
`useManageMCPConnections.ts:87-90`
很值钱。
...
4. 如果 pool repair 从不发生，系统又凭什么声称自己理解“整体可用性”？
