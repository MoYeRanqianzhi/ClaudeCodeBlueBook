# control_response回放与日志面的强请求清理审计关闭治理边界
...
| replay enqueue | `src/cli/print.ts:4030-4035` | 为什么 old echo 即使不再 active handling，也还能进入 replay output |
| duplicate trace logging | `src/cli/structuredIO.ts:386-398` | 为什么 duplicate old echo 仍留下 debug trace |
| orphaned trace logging | `src/cli/print.ts:5263-5290` | 为什么 orphaned old echo 仍留下 explainability trace |
| diagnostics parse surface | `src/cli/structuredIO.ts:232-236` | 为什么 returned message 仍会进入 diagnostics parsing |
...
## 7. 为什么这层不等于 `290` 的 archive-close gate

这里必须单独讲清楚，
否则容易把 `291` 误读成 `290` 的尾注。
...
1. `290` 的典型形态是 lifecycle close、stale prompt teardown、queue non-admission、inline completed contract
2. `291` 的典型形态是 replay propagation、replay enqueue、debug trace、diagnostics parse
