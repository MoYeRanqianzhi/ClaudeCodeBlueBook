# diskOutput、sessionStorage、toolResultStorage、plans、fileHistory、sessionEnvironment、debug与diagLogs中的强请求清理制度理由边界
...
| operational debugging rationale | `src/utils/debug.ts:30-68,91-111,229-249`; `src/utils/cleanup.ts:391-428` | 为什么 debug logs 不是 generic retained artifact，而是 operational surface |
...
`debug.ts:30-68,91-111,229-249` 说明 debug 首先服务：
...
1. 高级安全设计会让 risk object 直接改写存储法，而不是先定一个统一容器再让所有 family 迁就它
2. inspection、continuity、planning、restore、replay、operational debugging 与 host monitoring 不是同一种 duty，不能被 generic retention sentence 糊平
3. 同一 constitution world 内部也可能容纳不同理由，所以“住在一起”不等于“为了同一个 why”
4. configurable storage 一旦出现，cleanup reason 若不跟进，就会最先暴露 rationale drift
5. host-visible no-PII telemetry 证明：有些 cleanup family 的制度理由天生就跨出 repo-local persistence 哲学
