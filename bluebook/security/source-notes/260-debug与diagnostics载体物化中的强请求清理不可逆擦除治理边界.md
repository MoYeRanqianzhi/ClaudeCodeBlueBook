...
## 8. 为什么这一层不等于 `259` 的 audit-close gate

这里必须单独讲清楚，
否则最容易把 `260` 误读成 `259` 的尾注。
...
1. `259` 的典型形态是 replay propagation、debug trace、diagnostics parse
2. `260` 的典型形态是 debug logfile materialization、generic retention delete、transcript truncate、workspace unlink
