...
## 7. 为什么这层不等于 `286` 的 current completion gate

这里必须单独讲清楚，
否则容易把 `287` 误读成 `286` 的尾注。
...
1. `286` 的典型形态是 `tool_result`、`completed progress`、`completed successfully`
2. `287` 的典型形态是 `sendResult()`、`files_persisted`、`idle`、`flush()` disclaimer、`state_restored`
