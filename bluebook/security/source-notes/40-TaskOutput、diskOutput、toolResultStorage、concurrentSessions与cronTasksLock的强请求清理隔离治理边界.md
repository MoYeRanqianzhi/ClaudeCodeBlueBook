# TaskOutput、diskOutput、toolResultStorage、concurrentSessions与cronTasksLock的强请求清理隔离治理边界
...
`谁来诚实说明 stronger-request retention law 当前执行到哪里，`

而是：

`谁来证明这条 law 在执行时不会误伤同项目里仍在运行、仍在读取或仍在写入的 live peer。`
...
把历史 side effect、局部 isolation repair、共享读面、旧 cleanup world、live-session ledger 与 single-owner positive control 并排，
逼出一句更硬的结论：

`Claude Code 已经明确展示：知道 cleanup 执行了，不等于已经证明 cleanup 对 live peers 无害；非干扰性需要自己的治理层。`
...
| 主题 | 关键源码 | 这里回答的问题 |
| --- | --- | --- |
| historical live-peer side effect | `src/utils/task/TaskOutput.ts:313-325` | cleanup 过去怎样伤到同项目 live peer |
| repaired task-output isolation | `src/utils/task/diskOutput.ts:33-55` | 哪个 artifact family 已被提升成 session-scoped isolation architecture |
...
| visible project/session sweep | `src/utils/cleanup.ts:155-257,300-347,350-428,575-595` | cleanup 怎样继续进入更广的 artifact families |
| live-session ledger | `src/utils/concurrentSessions.ts:49-109,163-204` | 系统其实已经有哪些 “谁还活着” 的账本 |
| single-owner positive control | `src/utils/cronTasksLock.ts:3-4,49-57,109-111`; `src/utils/cronScheduler.ts:34-44` | repo 在别的 same-project multi-session 子系统里怎样正式证明 noninterference |

## 4. `TaskOutput` 先把伤痕留住：cleanup 真可能伤到 same-project live peer

`TaskOutput.ts:313-325` 不是模糊猜测，
而是直接告诉我们：
...
## 5. `diskOutput.ts` 再证明：repo 已经在一个 artifact family 上把 isolation 升级成 architecture repair

`diskOutput.ts:33-55` 很硬。

这里 repo 明确写：

1. task output dir 现在走 `getProjectTempDir()/sessionId/tasks`
2. 之所以把 `sessionId` 放进路径
3. 是因为 concurrent sessions in the same project previously clobbered each other's output files
...
`permissions/filesystem.ts:1688-1700` 把问题推进得更深。
...
`toolResultStorage.ts:94-118` 说明：

1. persisted tool results 仍落在 `projectDir/sessionId/tool-results`
2. 这是 projects-tree world，而不是 task-output family 那种 temp-dir isolation repair world
...
也就是说，
task-output family 和 tool-results family 当前活在两种不同的 cleanup constitution 里。

## 8. `concurrentSessions` 让缺口更具体：repo 知道谁活着，但 cleanup 没把这份知识 visibly 用起来

`concurrentSessions.ts:49-109,163-204` 已经把 live-session ledger 搭好了：
...
这说明 stronger-request cleanup path 当前缺的绝不是：
...
这就把 stronger-request cleanup isolation 的问题从抽象风险推进成了更硬的结构判断：

`live-peer knowledge exists, but deletion constitution does not visibly consume it`

## 9. `cronTasksLock` 与 `cronScheduler` 再给一条正对照：repo 明明知道怎样把 same-project noninterference 做成正式治理原语

`cronTasksLock.ts:3-4,49-57,109-111` 与 `cronScheduler.ts:34-44` 给出了更强的正控制面：
