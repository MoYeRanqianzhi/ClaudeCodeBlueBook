...
| runtime contradiction revocation | `src/services/mcp/client.ts:3196-3230` | 为什么 401 / expired session 会即时撤销旧 reassurance |
| tool roster freshness | `src/services/mcp/useManageMCPConnections.ts:618-656` | 为什么 tool list 本身也必须在 runtime 保持 current |
| current-tool hard filter | `src/services/tools/toolExecution.ts:337-394` | 为什么历史 tool 名不拥有继续停留在 current execution world 的资格 |
...
`src/services/mcp/client.ts:3196-3230`
又进一步把这层写硬了。
...
`src/services/mcp/useManageMCPConnections.ts:618-656`
特别值钱。
...
`src/services/tools/toolExecution.ts:337-394`
则把这层钉得更死。
...
## 7. 更深一层的技术先进性：Claude Code 把“真正使用前的验票权”当成一等安全对象

这组源码给出的技术启示至少有六条：

1. 正向 reassurance 不能直接转成 actual-use authorization，二者之间必须插入 fresh-proof primitive
2. roster freshness 与 connection freshness 一样，都属于 current-use proof 的组成部分
3. runtime contradiction 必须结构化为 revocation signal，而不应退化成一般异常
4. consumer edge 与 agent edge 都必须保有独立的最终验票权
5. current execution context 的辅佐信息也必须服从 live connected truth，而不是服从历史印象
6. 真正成熟的安全系统不仅会说“现在看起来没问题”，还会在你真的伸手时重新验票

## 8. 这篇源码剖面给主线带来的六条技术启示

1. repo 已经在 `ensureConnectedClient()` 上明确展示：fresh-at-use proof 必须有 canonical primitive。
2. repo 已经在 `MCPTool.call()` 上明确展示：真正 tool use 会在每次调用前重回 fresh-proof path。
3. repo 已经在 `callMCPTool()` 上明确展示：runtime contradiction 会即时撤销旧 reassurance，并切换回 recovery signal。
4. repo 已经在 `tools/list_changed` 上明确展示：当前 roster 也是安全事实，不能让旧 roster 残留。
5. repo 已经在 `runToolUse()` 上明确展示：当前工具世界只认 current pool，不认历史名字。
6. stronger-request cleanup 如果未来要长出 use-time-revalidation-governor plane，关键不只是“谁来决定现在可以说多放心”，还包括“谁来决定真正消费这一刻这份放心话是否仍配被当真”。

## 9. 苏格拉底式自反诘问：我是不是又把“它刚才看起来能用”误认成了“它现在真的仍然属于可依赖的当前世界”
