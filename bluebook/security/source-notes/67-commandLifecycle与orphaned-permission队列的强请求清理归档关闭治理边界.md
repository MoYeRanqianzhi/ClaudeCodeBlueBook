...
`stronger-request cleanup 线如果未来已经宣布 no-more-duty，谁来决定这条 old echo 什么时候已经从 active lifecycle / waiting / queue 表面正式关闭。`
...
## 8. 为什么这层不等于 `66` 的 liability-release gate
...
1. `66` 的典型形态是 duplicate ignore、unknown-response no-op、transcript-resolved amnesty、compensate-or-refuse
2. `67` 的典型形态是 lifecycle completed、stale prompt teardown、queue non-admission、inline completed contract
...
`queue admission and observability close are separate archive-close questions, not footnotes to release`
