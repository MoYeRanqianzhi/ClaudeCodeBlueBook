...
`print.ts:2768-2782,5241-5304` 很硬。
...
## 8. 为什么这层不等于 `287` 的 stronger finality gate

这里必须单独讲清楚，
否则容易把 `288` 误读成 `287` 的尾注。
...
1. `287` 的典型形态是 `tool_result`、`sendResult()`、`files_persisted`、`idle`、`state_restored`
2. `288` 的典型形态是 `resolvedToolUseIds`、`handledOrphanedToolUseIds`、duplicate ignore、bounded eviction
...
## 9. 苏格拉底式自反诘问：我是不是又把“它以后回来时仍然成立”误认成了“现在已经可以不再记住它”
