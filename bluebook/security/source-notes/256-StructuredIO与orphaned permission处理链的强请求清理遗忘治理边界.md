...
## 8. 为什么这层不等于 `255` 的 stronger finality gate

这里必须单独讲清楚，
否则容易把 `256` 误读成 `255` 的尾注。
...
1. `255` 的典型形态是 `tool_result`、`files_persisted`、`idle`、`state_restored`
2. `256` 的典型形态是 `resolvedToolUseIds`、`handledOrphanedToolUseIds`、duplicate ignore、bounded eviction
