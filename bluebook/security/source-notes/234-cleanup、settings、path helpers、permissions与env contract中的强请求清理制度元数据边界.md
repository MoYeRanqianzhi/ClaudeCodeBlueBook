...
| local retention knobs | `src/utils/settings/types.ts:325-332,824-830` | 为什么当前 schema 公开的是零散 knobs，而不是完整 family grammar |
...
| cross-file puzzle truth | `src/utils/toolResultStorage.ts:1-5,94-124`; `src/utils/sessionStorage.ts:198-224`; `src/utils/cleanup.ts:155-257`; `src/utils/permissions/filesystem.ts:1656-1672` | 为什么 tool-results / transcripts 的 truth 已经是跨模块拼图，而不是单点 descriptor |
| cross-plane propagation stress test | `src/utils/plans.ts:79-110,164-233`; `src/utils/settings/types.ts:824-830`; `src/utils/permissions/filesystem.ts:244-255,1487-1495,1644-1652`; `src/utils/cleanup.ts:300-303` | 为什么 plans family 最能暴露没有 metadata plane 时的传播失灵 |
| stable but still local truth | `src/utils/fileHistory.ts:951-957`; `src/utils/sessionEnvironment.ts:15-22,25-31,60-143`; `src/utils/debug.ts:30-68,91-111,216-235` | 为什么有些 family 暂时没失灵，也仍不等于已经 metadata 化 |
...
`toolResultStorage.ts`、`sessionStorage.ts`、`permissions/filesystem.ts` 与 `cleanup.ts`
...
`system-readable contract`

## 11. 从技术先进性看：成熟安全系统为什么要把“知道为什么”与“把为什么做成元数据”拆开

这组源码给出的设计启示至少有六条：

1. family truth can exist without family metadata
2. local explicitness is weaker than canonical self-description
3. cross-plane propagation is the best metadata stress test
4. config knobs are exposure surfaces, not the grammar itself
5. permission projections show consumption, not canonical truth ownership
6. metadata is the layer that turns rationale into a transportable anti-drift contract

它的哲学本质是：

`成熟安全系统不仅要知道为什么这样设计，还要把这个为什么正式写进系统自己能够携带和传递的元数据里。`
