...
| 主题 | 关键源码 | 这里回答的问题 |
| --- | --- | --- |
| Layer-3 reactivation primitive | `src/utils/plugins/refresh.ts:1-176` | 为什么 active world swap 是单独主权，不是 config save 的自然尾气 |
...
| scenario-sensitive activation policy | `src/services/plugins/PluginInstallationManager.ts:52-180` | 为什么 new install 与 update 不共享同一 reactivation rule |
...
| headless auto-consume path | `src/cli/print.ts:1733-1768,3072-3092`; `src/state/AppStateStore.ts:209-214` | 为什么 headless mode 可自动 reactivation，而 interactive mode 不行 |
