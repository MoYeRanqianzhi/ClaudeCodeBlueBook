...
| control-plane reintegration | `src/cli/print.ts:3153-3197,3256-3288,3391-3441` | 为什么 print mode 里 recovery 之后还要单独写回 state 与重绑 handlers |
...
`src/services/mcp/useManageMCPConnections.ts:310-332`
很值钱。
...
1. 写回 `appState.mcp`
2. 更新 `dynamicMcpState`
3. 只有 `connected` 时才 `registerElicitationHandlers([result.client])`
4. 只有 `connected` 时才 `reregisterChannelHandlerAfterReconnect(result.client)`

`src/cli/print.ts:3256-3288`
的 `mcp_toggle` 也是同样制度。

`src/cli/print.ts:3391-3441`
...
`src/cli/print.ts:1610-1627`
又说明 `buildMcpServerStatuses()` 会把：
...
`src/cli/print.ts:1253-1280`
的 `registerElicitationHandlers()` 很硬。
...
的 `reregisterChannelHandlerAfterReconnect()` 也同样严格：
...
而
`src/tools/McpAuthTool/McpAuthTool.ts:134-165`
又把 tool plane 的 reintegration 写得很清楚：
...
如果再追问“怎样做得更好”，
更好的方法也不是只多写几个 `connected` 的成功提示，
而是继续把四个问题拆开：

1. 谁签字说 raw recovery result 已够强
2. 谁签字说 state plane 已回席
3. 谁签字说 tool/status plane 已回席
4. 谁签字说 live event plane 已回席
