...
## 8. 为什么这层不等于 `289` 的 liability-release gate

这里必须单独讲清楚，
否则容易把 `290` 误读成 `289` 的尾注。
...
1. `289` 的典型形态是 duplicate ignore、unknown-response no-op、transcript-resolved amnesty、compensate-or-refuse
2. `290` 的典型形态是 lifecycle completed、stale prompt teardown、queue non-admission、inline completed contract
