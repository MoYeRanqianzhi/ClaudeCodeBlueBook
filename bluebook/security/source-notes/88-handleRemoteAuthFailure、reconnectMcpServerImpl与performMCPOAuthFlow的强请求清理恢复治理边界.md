...
| 主题 | 关键源码 | 这里回答的问题 |
| --- | --- | --- |
| auth-failure demotion | `src/services/mcp/client.ts:340-360,1079-1137,2290-2345` | 为什么 continuity 断裂后会正式转入 `needs-auth` / auth-tool grammar |
...
1. 先读 cached step-up scope / discovery state
2. 清掉旧的本地 token
...
repo 已经在 pseudo-tool grammar 上明确展示：

`恢复入口本身也应被建模成一等对象，而不是让真实能力在未恢复时继续裸奔`
