...
| 主题 | 关键源码 | 这里回答的问题 |
| --- | --- | --- |
| lifecycle close on every `control_response` | `src/cli/structuredIO.ts:362-398` | 为什么 duplicates / orphans 即使已 no-more-duty 也要被正式 completed |
| lifecycle surface contract | `src/utils/commandLifecycle.ts:1-20` | 为什么 started / completed 本身是一条独立 active surface |
| stale prompt teardown | `src/cli/structuredIO.ts:402-409` | 为什么 stronger signer 还要主动撤掉旧 waiting prompt |
| orphan queue mode | `src/cli/print.ts:1936-1948` | 为什么 orphaned echo 是否进入 active queue 是单独问题 |
| compensate-or-refuse branch | `src/cli/print.ts:5241-5301` | 为什么 release 之后仍要继续回答 active re-entry surface 该怎么关 |
| inline completed observability | `src/cli/print.ts:2816-2828` | 为什么 operator-facing close 不是 release judgement 的附带物 |

## 4. `notifyCommandLifecycle(uuid, 'completed')` 先证明：archive-close 的第一层是 lifecycle close，不是语义比喻

`structuredIO.ts:362-398`
很硬。
...
## 8. 为什么这层不等于 `257` 的 liability-release gate

这里必须单独讲清楚，
否则容易把 `258` 误读成 `257` 的尾注。
...
1. `257` 的典型形态是 duplicate ignore、unknown-response no-op、transcript-resolved amnesty、compensate-or-refuse
2. `258` 的典型形态是 lifecycle completed、stale prompt teardown、queue non-admission、inline completed contract
...
`repo 已经在 StructuredIO 的 lifecycle completed、stale permission prompt cancellation、print.ts 的 orphaned-permission queue mode 与 compensate-or-refuse / non-admission 分流，以及 commandLifecycle 的独立 listener contract 与 non-user inline completed observability 上，清楚展示了 stronger-request archive-close governance 的独立存在；因此 artifact-family cleanup stronger-request liability-release-governor signer 仍不能越级冒充 artifact-family cleanup stronger-request archive-close-governor signer。`
