# control_response 回放与日志面的强请求审计关闭治理边界
...
`cleanup 线未来谁来决定 old echo 何时退出 active operational surface，`

而是：

`cleanup 线如果未来已经让 old echo 退出活跃表面，谁来决定它什么时候也退出 replay、debug 与 diagnostics 这些审计面。`
...
`Claude Code 已经在 old echo world 里明确展示：退出活跃表面只解决 operator surface；只要 replay/debug/diag 还在继续承载它，这条 old echo 就仍然活在 audit world。`

## 2. 最短判断

这组代码最重要的技术事实不是：

`Claude Code 有一些 debug log 和 replay 选项。`

而是：

`Claude Code 明确把 operational closure 与 evidentiary closure 分成两层：前者让 old echo 离开 active surface，后者才决定它是否也离开 replay/debug/diagnostic audit surface。`
...
| duplicate trace logging | `src/cli/structuredIO.ts:390-398` | 为什么 duplicate old echo 仍留下 debug trace |
...
`即便不再 active，也要让后来的审计者知道它发生了什么、为何被跳过、为何仍被接回`
...
## 7. 为什么这层不等于 `184` 的 archive-close gate
...
`即便它已退出这些 active 表面，它是否也已经退出 replay/debug/diag 这些 evidentiary 表面。`

所以：

1. `184` 的典型形态是 lifecycle close、stale prompt teardown、queue non-admission
2. `185` 的典型形态是 replay propagation、debug trace、diagnostics parse
...
这四句合起来，
正好说明为什么 cleanup 线未来不能把 stronger-request archive close 直接偷写成 stronger-request audit close。

## 9. 一条硬结论

这组源码真正说明的不是：

`只要 old echo 已经离开 active handling world，cleanup 线就已经知道何时让它退出全部审计世界。`
...
`cleanup 线真正缺的，不只是“谁来宣布 old echo 现在已退出活跃操作表面”，还包括“谁来宣布它现在也已退出 replay、debug 与 diagnostics 这些审计世界”。`
