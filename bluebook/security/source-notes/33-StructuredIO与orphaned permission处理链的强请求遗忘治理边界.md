# StructuredIO 与 orphaned permission 处理链的强请求遗忘治理边界
...
`cleanup 线未来谁来决定 resumed stronger request 的结果以后回来时仍然成立，`

而是：

`cleanup 线如果未来已经确认这份结果 future-readable enough，谁来决定与这条 request 相关的 old identity 什么时候才配安全遗忘。`
...
把 `resolvedToolUseIds`、`trackResolvedToolUseId()`、bounded eviction 与 `handledToolUseIds` / orphaned permission dedupe 并排，
逼出一句更硬的结论：

`Claude Code 已经在 resumed stronger-request world 里明确展示：final truth 建立后，系统仍会保留一段 bounded anti-duplicate memory；真正的 forgetting 要等 duplicate/orphan replay 风险被单独压住之后才成立。`
...
| orphaned replay suppression | `src/cli/print.ts:5238-5305` | 为什么 orphaned permission 也需要 handled-ID memory |
...
1. 若 `handledToolUseIds.has(toolUseID)`，duplicate orphaned permission 直接跳过
2. 若 `findUnresolvedToolUse(toolUseID)` 找不到，说明 transcript 已 resolved，也拒绝重新接回
...
## 8. 为什么这层不等于 `181` 的 stronger finality gate
...
2. `182` 的典型形态是 `resolvedToolUseIds`、`handledToolUseIds`、duplicate ignore、bounded eviction
...
这四句合起来，
正好说明为什么 cleanup 线未来不能把 resumed stronger-request finality 直接偷写成 resumed stronger-request forgetting。

## 10. 一条硬结论

这组源码真正说明的不是：

`只要补出 resumed stronger-request 的 stronger finality，cleanup 线就已经知道何时可以忘掉与它相关的 old request identity。`
...
`cleanup 线真正缺的，不只是“谁来宣布 resumed stronger-request 的结果以后仍然成立”，还包括“谁来宣布与它相关的 old identity 现在已经安全到可以忘掉”。`
