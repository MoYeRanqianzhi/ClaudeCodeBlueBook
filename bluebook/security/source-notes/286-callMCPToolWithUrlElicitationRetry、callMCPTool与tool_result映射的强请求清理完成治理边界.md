...
| settled tool-call result | `src/services/mcp/client.ts:3029-3171` | 为什么真正 completion 要等到底层 `callTool` success / error 决出结果 |
...
`src/services/mcp/client.ts:3029-3171`
更硬。
...
## 8. 为什么这层不等于 `285` 的 continuation 剖面

这里必须单独讲清楚，
否则容易把 `286` 误读成 `285` 的尾注。
...
1. `285` 的典型形态是 retry loop、accept/decline/cancel、retry budget、future readiness ceiling
2. `286` 的典型形态是 could-not-complete、completed progress、completed successfully、normalized result、success tool_result
...
## 9. 苏格拉底式自反诘问：我是不是又把“它现在已经合法继续”误认成了“它现在已经完成”
...
`the blocked stronger cleanup request may try again`

而不是：

`the blocked stronger cleanup request has already been settled as complete`
