# savePluginOptions、saveMcpServerUserConfig、PluginOptionsFlow与ManagePlugins的强请求清理重配置治理边界
...
4. repo 已经在 `PluginOptionsFlow` 上明确展示：closure 需要被显式收尾，而不是由存在状态自动担保。
5. repo 已经在 `ManagePlugins` 上明确展示：保存、配置完成与运行时生效是三种不同事实。
6. stronger-request cleanup 如果未来要长出 reconfiguration-governor plane，关键不只是“returned seat 还能不能继续工作”，而是“returned seat 当前按哪组 truth、以哪种 closure 和何时生效继续工作”。

## 10. 为什么这些正对照会反过来暴露 stronger-request cleanup 的当前缺口

和这些成熟 reconfiguration grammar 对照，
stronger-request cleanup 线当前真正缺的已经不是：

`旧对象回来后还配不配占原来的 authority seat`

而是：

`它回来后究竟按哪组 current options、哪组 current payload 与哪组 current config truth 重新工作。`

更具体地说，
现在还没有哪一层正式决定：

1. 旧 startup wording comeback 后按哪组 current option truth 继续生效
2. 旧 cleanup law comeback 后哪些旧 secrets 仍算被承认、哪些必须 scrub
3. 旧 promise vocabulary comeback 后哪些 payload / channel truth 必须重填
4. 旧 receipt objects comeback 后只是 saved truth，还是已经 apply 到 running world
5. 哪些 returned carriers 只是 visible，哪些已经 configured，哪些已真正 take effect

这意味着 stronger-request cleanup 当前虽然已经开始显露 re-entitlement-governance，
却还没有谁正式回答：

`谁来决定 returned seat 现在究竟按哪组 current input truth 运转。`

## 11. 苏格拉底式自反诘问：我是不是又把“对象重新拿回了 seat”误认成了“它现在也已经按正确配置工作”
