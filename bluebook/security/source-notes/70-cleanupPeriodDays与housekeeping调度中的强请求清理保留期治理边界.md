...
## 8. 为什么这一层不等于 `69` 的 irreversible-erasure gate

这里必须单独讲清楚，
否则最容易把 `70` 误读成 `69` 的尾注。
...
1. `69` 的典型形态是 `truncate`、`unlink(filePath)`、old debug log delete
2. `70` 的典型形态是 `cleanupPeriodDays`、validation skip、delay/activity gate、carrier coverage gap
