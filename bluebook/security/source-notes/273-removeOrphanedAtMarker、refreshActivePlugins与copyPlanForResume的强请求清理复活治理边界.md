...
| explicit and mode-sensitive re-entry gate | `src/services/plugins/PluginInstallationManager.ts:53-59,135-179`; `src/hooks/useManagePlugins.ts:28-35,287-303` | 为什么 repo 会继续区分 auto-refresh、manual `/reload-plugins` 与 not-yet-admitted worlds |
...
| deletion-only tombstone path | `src/utils/sessionStorage.ts:925-945,1468-1474` | 为什么 tombstone consumption 本身不自动携带 resurrection grammar |
...
`PluginInstallationManager.ts:53-59`
先给出高层策略：
...
再看
`useManagePlugins.ts:28-35,287-303`：
...
6. 如果 comeback 只是 refresh 问题，为什么还要追问 evidence？
   因为 `copyPlanForResume()` 已经证明，没有 lineage 与 evidence，就不配回到 current world。
7. 如果 cleanup 线还没有显式 resurrection 代码，是不是说明这层还不值得写？
   恰恰相反。越是缺这层明确 grammar，越容易把“还能回来吗”偷写成“反正 marker 清掉就算回来”。
8. 如果我继续把 resurrection 当成 tombstone 的副产品，会漏掉什么？
   会漏掉 authoritative clearing、explicit admission、identity policy 与 active-layer gate 这些真正决定“谁配回来”的制度对象。
...
`Claude Code 在 authoritative marker clearing、Layer-3 refresh、mode-sensitive re-entry gate、lineage-based plan recovery 与 deletion-only tombstone path 上已经明确展示了 resurrection governance 的存在；因此 artifact-family cleanup stronger-request cleanup-tombstone-governor signer 仍不能越级冒充 artifact-family cleanup stronger-request cleanup-resurrection-governor signer。`

再压成一句：

`cleanup 线真正缺的不是“谁来留下退役痕迹”，而是“谁来重新签发回到当前世界的资格”。`
