...
| plugin verify/demote split | `src/utils/plugins/dependencyResolver.ts:172-233`; `src/utils/plugins/pluginLoader.ts:3189-3195` | 为什么 repo 已显式拥有 verify + governance consequence + scope grammar |
| cleanup repair gap | `src/utils/cleanup.ts:33-45,300-303,575-598`; `src/utils/plans.ts:79-110`; `src/utils/diagLogs.ts:14-60` | 为什么 stronger-request cleanup 线当前还没有对等 repair-governor plane |

## 4. auto-mode 先证明：repo 已经知道 verifier 只配“发现问题”，不配自己偷偷改世界

`permissionSetup.ts` 的 `verifyAutoModeGateAccess()` 已经很强，
它会：
...
## 9. 苏格拉底式自反诘问：我是不是又把“知道哪里错了”误认成了“已经知道该怎么修”
