...
| stronger-request cleanup conformance cluster | `src/utils/sessionStorage.ts:953-969`; `src/utils/backgroundHousekeeping.ts:31-80`; `src/utils/cleanup.ts:33-45,575-597`; `src/utils/plans.ts:79-110`; `src/utils/cleanup.ts:300-303` | 为什么 cleanup 线当前仍主要是在表达 conformance symptom，而不是 verifier grammar |
...
这条启示非常重要，
因为今天 cleanup 的 metadata、scheduler、executor 与 receipt 仍可半独立变化，
所以 drift 仍有表达机会。
...
`policy 已进入某些 plane，但没有 verifier 持续检查 executor plane 是否仍与 policy 同步。`
