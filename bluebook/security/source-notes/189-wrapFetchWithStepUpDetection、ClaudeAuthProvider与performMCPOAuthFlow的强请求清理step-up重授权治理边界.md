...
| 主题 | 关键源码 | 这里回答的问题 |
| --- | --- | --- |
| insufficient-scope detection | `src/services/mcp/auth.ts:1345-1368` | 为什么 transport fetch 会在最前线把 `403 insufficient_scope` 识别成 authority-level 问题 |
| step-up pending state | `src/services/mcp/auth.ts:1468-1469,1625-1690` | 为什么当前 token fresh 不等于当前 scope 足够 |
| step-up continuation | `src/services/mcp/auth.ts:582-598,909,1896` | 为什么更高授权要保留自己的 state across revoke / re-auth |
| transport integration | `src/services/mcp/client.ts:633,827` | 为什么 step-up detection 是 live transport path 的正式部分 |
| completion semantics | `src/services/mcp/auth.ts:1705` | 为什么 higher-authority pending state 拥有单独的完成条件 |
