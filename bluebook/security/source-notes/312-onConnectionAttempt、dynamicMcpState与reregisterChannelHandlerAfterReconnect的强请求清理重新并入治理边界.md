...
| standard reintegration path | `src/services/mcp/useManageMCPConnections.ts:310-332` | 为什么 connection result 还要经过 state update 与 handler attach |
| control-plane reintegration | `src/cli/print.ts:3153-3204,3255-3288,3391-3441` | 为什么 print mode 里 recovery 之后还要单独写回 state 与重绑 handlers |
...
`useManageMCPConnections.ts:310-332` 很值钱。
...
`print.ts:3153-3204` 的 `mcp_reconnect` 很硬。
...
## 9. 更深一层的技术先进性：Claude Code 把“回到当前世界”当成一等安全对象

这组源码给出的技术启示至少有六条：

1. 安全系统不能只问“对象好没好”，还要问“当前世界有没有正式接纳它”
2. raw result、current state 与 next-turn state 需要显式分层，否则恢复成功会在投影过程中被默默吞掉
3. live handlers 本身也是 reintegration 对象，而不是状态更新的附属动作
4. placeholder tool 与 real tool 的替换是一种 current-world truth 迁移，而不是 UI 细节
5. consumer 看到的 world 必须从 reintegrated state 推导，而不能从内部实现返回值推导
6. current-world membership 应该被显式治理，否则“已经恢复”会被错误放大成“已经重新并入”

## 10. 这篇源码剖面给主线带来的六条技术启示

1. repo 已经在 `reconnectMcpServerImpl()` 上明确展示：恢复结果本身只是 reintegration 的输入材料，不是完成态。
2. repo 已经在 `onConnectionAttempt()` 上明确展示：standard path 里也存在 state plane 与 event plane 的分层回归。
3. repo 已经在 `mcp_reconnect` / `mcp_toggle` / auth-complete background path 上明确展示：control plane 会把 reintegration 拆成 writeback、dynamic projection 与 handler rebind 三层。
4. repo 已经在 `buildAllTools()` 与 `buildMcpServerStatuses()` 上明确展示：consumer 真正读到的是 reintegrated state，而不是 internal raw result。
5. repo 已经在 `reregisterChannelHandlerAfterReconnect()` 与 `registerElicitationHandlers()` 上明确展示：live interaction 也有自己的 reintegration gate。
6. stronger-request cleanup 如果未来要长出 reintegration-governor plane，关键不只是“它已经恢复了吗”，而是“它现在何时正式重新成为当前世界的一部分”。

## 11. 苏格拉底式自反诘问：我是不是又把“恢复成立”误认成了“当前世界已经重新接回它”
