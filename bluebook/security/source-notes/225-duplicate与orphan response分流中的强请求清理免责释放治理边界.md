...
| 主题 | 关键源码 | 这里回答的问题 |
| --- | --- | --- |
| duplicate refusal | `src/cli/structuredIO.ts:374-393` | 为什么 duplicate control_response 被忽略不是因为系统忘了，而是因为它已不再欠第二次补偿 |
| unknown-response no-op | `src/cli/structuredIO.ts:395-398` | 为什么没有 live pending basis 时也能形成 no-more-duty judgment |
| orphaned compensate-or-refuse | `src/cli/print.ts:5241-5301` | 为什么 orphan echo 不是统一忽略，而要重新判断当前是否仍欠它责任 |
| already handled / transcript resolved | `src/cli/print.ts:5272-5284` | 为什么 refusal 要踩在 handled fact 或 stronger transcript truth 上 |
| stale prompt cancellation | `src/cli/structuredIO.ts:400-408` | 为什么免责释放不只是 return false，而还要主动关闭旧 waiting surface |
...
## 10. 苏格拉底式自反诘问：我是不是又把“开始遗忘”误认成了“已经免责”
