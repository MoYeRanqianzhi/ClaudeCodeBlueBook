...
## 7. 为什么这层不等于 `163` 的 archive-close gate

这里必须单独讲清楚，
否则容易把 `164` 误读成 `163` 的尾注。
...
1. `163` 的典型形态是 lifecycle close、stale prompt teardown、queue non-admission、inline completed contract
2. `164` 的典型形态是 replay propagation、replay enqueue、debug trace、diagnostics parse
