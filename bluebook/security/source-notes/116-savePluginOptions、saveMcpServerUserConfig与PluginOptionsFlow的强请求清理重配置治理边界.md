...
## 7. `PluginOptionsFlow` 与 `ManagePlugins` 再证明：`enabled`、`configured`、`applied` 是三层不同事实
...
repo 已经在 option storage 线上明确展示：

`seat truth != config truth`

### 启示二

repo 已经在 `needs-config` grammar 上明确展示：

`config truth != ready truth`

### 启示三

repo 已经在 `configured / skipped` outcome 上明确展示：

`editable != closed`

### 启示四

repo 已经在 `Configuration saved. Run /reload-plugins...` 句法上明确展示：

`saved truth != applied truth`

这四句合起来，
正好说明为什么 cleanup 线未来不能把 re-entitlement governance 直接偷写成 complete work-ready configuration。

## 10. 苏格拉底式边界自校：什么才算真正的重配置治理
