...
| visible project/session sweep | `src/utils/cleanup.ts:155-257,300-347,350-428,575-595` | cleanup 怎样继续进入更广的 carrier families |
| live-session ledger | `src/utils/concurrentSessions.ts:49-109,163-204` | 系统其实已经有哪些“谁还活着”的账本 |
| single-owner positive control | `src/utils/cronTasksLock.ts:1-15,49-57,95-163`; `src/utils/cronScheduler.ts:34-44` | repo 在别的 same-project multi-session 子系统里怎样正式证明 noninterference |

## 4. `TaskOutput` 先把事故痕迹留下：cleanup 真可能伤到 same-project live peer

`TaskOutput.ts:313-325` 不是抽象猜测，
而是直接告诉我们：
...
`cleanup.ts:300-347,350-428,575-595` 继续说明：
...
## 9. `concurrentSessions` 让缺口更具体：repo 明明知道谁还活着，但 cleanup 没把这份知识 visibly 用起来
...
## 10. `cronTasksLock` 与 `cronScheduler` 再给一条正对照：repo 明明知道怎样把 same-project noninterference 做成 formal primitive

`cronTasksLock.ts:1-15,49-57,95-163` 与 `cronScheduler.ts:34-44` 给出了更强的正控制面：
...
6. non-owner session 还会按 probe interval 重试
...
1. 历史事故必须被制度化保留，否则系统会失去继续分层治理的动机
2. 真正有效的安全修复不是“加一行文案”，而是把高风险 family 升级成路径与 owner boundary 的 architecture repair
3. shared-readable surface 是安全设计里最容易偷换概念的地方，它要求更强 delete proof，而不是更弱
4. 账本型知识只有在被 destructive path 消费时才构成治理；只存在于观测层的 knowledge 仍不足以证明 noninterference
5. 一个 repo 若已在别处展示 single-owner lease、PID liveness 与 stale takeover，就已经证明它知道怎样把 concurrency safety 做成 formal primitive
