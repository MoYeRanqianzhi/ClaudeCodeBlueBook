...
把 repo 里现成的配置删除、重新 enable、policy blocking、scope legality、settings divergence、closure write 与 new-slug fork policy 并排，
...
| closure-level entitlement | `src/utils/plugins/pluginInstallationHelpers.ts:330-339,361-367,414-437` | 为什么旧 relation bundle 的资格恢复不一定是单对象问题 |
...
## 7. closure-level write 再证明：资格恢复有时不是单对象回归，而是关系束重签

`pluginInstallationHelpers.ts:330-339`
把安装逻辑写得很清楚：

1. 先解析 transitive dependency closure
2. 再把整个 closure 一次性写入 `enabledPlugins`
3. 然后再缓存 closure members

继续看
`pluginInstallationHelpers.ts:361-367`
与
`pluginInstallationHelpers.ts:414-437`：

1. root plugin 可能被 org policy 挡住
2. transitive dependency 也可能被 policy 挡住
3. 只有整组 closure 都合法，才会统一写回 settings

这说明 entitlement 有时不是：
...
这对 stronger-request cleanup 很关键。
因为 cleanup artifact family 的很多资格本来就带有 relation / family 属性，
而不是单载体属性。
...
6. relation bundle entitlement 说明“回来之后还能做什么”常常是 closure-level 问题，而不是 object-level 问题。
7. stronger-request cleanup 如果未来要长出 re-entitlement-governor plane，关键不只是“让对象回来”，而是明确哪些旧资格根本不应该跟着回来。

## 10. 苏格拉底式自反诘问：我是不是又把“对象回来了”误认成了“它原来的全部资格也回来了”
...
8. 如果我继续把 re-entitlement 当成 resurrection 的副产品，会漏掉什么？
   会漏掉 config/secrets trust、enabled truth、policy veto、scope legality、closure write 与 identity fallback 这些真正决定“回来之后还能做什么”的制度对象。
