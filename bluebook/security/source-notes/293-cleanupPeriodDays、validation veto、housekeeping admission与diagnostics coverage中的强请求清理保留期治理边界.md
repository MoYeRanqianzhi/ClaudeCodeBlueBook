...
## 10. 为什么这一层不等于 `292` 的 irreversible-erasure gate

这里必须单独讲清楚，
否则最容易把 `293` 误读成 `292` 的尾注。
...
1. `292` 的典型形态是 debug logfile materialization、generic retention delete、transcript truncate、workspace unlink
2. `293` 的典型形态是 time law declaration、validation veto、future suppression、runtime admission、covered-vs-uncovered carrier governance
