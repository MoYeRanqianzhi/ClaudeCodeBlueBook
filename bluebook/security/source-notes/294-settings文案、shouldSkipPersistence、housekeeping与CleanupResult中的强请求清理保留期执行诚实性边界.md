...
## 9. 为什么这一层不等于 `293` 的 retention-governance gate

这里必须单独讲清楚，
否则最容易把 `294` 误读成 `293` 的尾注。
...
1. `293` 的典型形态是 `cleanupPeriodDays`、validation veto、housekeeping admission、coverage declaration
2. `294` 的典型形态是 transcript-centered wording、startup wording、future-suppression truth、local-only `CleanupResult` 与 diagnostics uncovered honesty path
