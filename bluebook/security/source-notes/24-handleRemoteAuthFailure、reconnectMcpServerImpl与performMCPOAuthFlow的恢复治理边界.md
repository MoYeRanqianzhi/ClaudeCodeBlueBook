# handleRemoteAuthFailure、reconnectMcpServerImpl 与 performMCPOAuthFlow 的恢复治理边界
...
`cleanup 线未来谁来维持旧的可用性线，`

而是：

`cleanup 线一旦 continuity 已经断裂，谁来决定什么新的凭据、什么新的连接结果、什么新的工具世界，才配正式宣布对象已经恢复。`
...
`Claude Code 已经在 MCP 线上明确展示：继续抢救旧的可用性线，并不自动回答对象已经恢复；cleanup 线当前缺的不是这种思想，而是这套 recovery governance 还没被正式接到旧 path、旧 promise 与旧 receipt 世界上。`
...
`Claude Code 已经在 MCP 线上明确把“旧线还要不要继续”和“什么新的证据足以宣布恢复成立”拆成两层；cleanup 线当前缺的不是文化，而是这套 recovery governance 还没被正式接到旧 cleanup artifact family 上。`
...
`1548-1614` 与 `1755-1778` 更进一步把恢复路径拆成两类：
...
这四句合起来，
正好说明为什么 cleanup 线未来不能把 continuity governance 直接偷写成 complete recovery。
