# cleanupPeriodDays 与 housekeeping 调度中的强请求清理保留期治理边界
...
`谁来定义这类 stronger-request old echo carriers 本来该保留多久、validation 出错时谁有权停手、何时允许真正执行 cleanup，以及当前哪些 carriers actually 被纳入这条 retention 栈。`
...
| intent honesty guard | `src/utils/settings/settings.ts:871-875,984-1004`; `src/utils/cleanup.ts:575-584` | validation 出错时谁有权阻止默认 retention 接管 |
| runtime admission / delay | `src/utils/backgroundHousekeeping.ts:43-60,77-80` | cleanup 何时真正被允许运行 |
| covered retention execution | `src/utils/cleanup.ts:23-30,396-428,575-593` | 哪些 visible carriers 当前已经纳入 cleanup horizon |
...
`settings.ts:871-875,984-1004` 与 `cleanup.ts:575-584`
...
`src/utils/cleanup.ts:23-30,396-428,575-593`
给出了很硬的正控制面。
...
所以 `283` 的真正技术启示不是“retention horizon 已经统一”，
而是：

`retention governance is partially explicit and partially coverage-limited`

## 8. 为什么这一层不等于 `133` 的 irreversible-erasure gate

这里必须单独讲清楚，
否则最容易把 `134` 误读成 `133` 的尾注。
...
1. `133` 的典型形态是 `truncate`、`unlink(filePath)`、old debug log delete
2. `134` 的典型形态是 `cleanupPeriodDays`、validation skip、delay/activity gate、carrier coverage gap
...
## 9. 技术启示：成熟安全系统为什么要把“能删载体”与“何时该删载体”拆开

这组源码给出的设计启示至少有四条。

第一条：

`destruction capability` 与 `retention law` 必须是两个独立 verdict。

第二条：

validation honesty 必须高于默认 policy，
否则“默认值”会变成偷删用户数据的借口。

第三条：

runtime admission 不是性能优化，
而是 retention governance 的一部分。

第四条：

carrier coverage 本身就是治理问题，
不能因为 debug 被覆盖就假定 diagnostics 也被同层法律覆盖。

## 10. 一条硬结论

这组源码真正说明的不是：

`只要 stronger-request old echo carrier 已经能被 delete 或已经被 delete，stronger-request cleanup 线就已经知道这类 carriers 本来该保留多久。`
