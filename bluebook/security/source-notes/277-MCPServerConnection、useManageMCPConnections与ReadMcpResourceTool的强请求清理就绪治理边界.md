...
`stronger-request cleanup 旧对象即便已经接管当前世界，谁来决定这个世界现在到底配不配继续被消费。`
...
把五态联合类型、`pluginReconnectKey` 触发后的 `pending`、显式 enable 仍回到 `pending`、surface honesty、consumer hard gate 与 runtime revocation 并排，
...
`Claude Code 在 MCP 线上已经明确把“对象进入 current world”和“对象当前是否真的配被继续消费”拆成两层；stronger-request cleanup 线当前缺的不是这种文化，而是这套 readiness governance 还没被正式接到旧 cleanup artifact family 上。`
...
| explicit enable still pending | `src/services/mcp/useManageMCPConnections.ts:1109-1123` | 为什么 even explicit enabling 也只是重新开启 readiness 流程 |
...
1. 先剔除 stale plugin MCP servers
2. 清掉旧 reconnect timer 与 stale cleanup hazard
3. 对新 client 按 `disabled / pending` 重新建档
4. 让连接流程重新开始

这里最硬的点在于：

`src/services/mcp/useManageMCPConnections.ts:817-825`
并没有把新 client 直接写成 `connected`，
而是：
...
4. 最后把结果交给 `onConnectionAttempt(result)`
...
3. consumer 已经成功使用过一次，也不等于当前仍然可继续使用
...
`ready until silence proves otherwise`
...
1. repo 已经在 `MCPServerConnection` 联合类型上明确展示：`present` 与 `ready` 必须被分开治理。
...
## 11. 苏格拉底式自反诘问：我是不是又把“对象已经接管当前世界”误认成了“它现在一定能用”
...
3. 如果 explicit enable 已经足以表达 operator will，为什么 enable path 还要先 `pending -> reconnect`？
   因为 operator intent 只能开启证明流程，不能替 runtime 直接签字。
4. 如果 readiness 是一次性 verdict，为什么 `toolExecution.ts` 还会把一个原本 `connected` 的 client 降成 `needs-auth`？
   因为 readiness 会被运行时证据持续撤销，它不是永久通行证。
5. 如果用户能在 UI 里看见对象，是不是就说明它已经足够可信？
   不是。visible 只是 current-world presence，不是 capability proof。
6. 如果 cleanup 线现在还没有显式 readiness 代码，是不是说明这层还不值得写？
   恰恰相反。越是缺这层 formal grammar，越容易把“已经接回 current world”偷写成“现在可以继续信”。
