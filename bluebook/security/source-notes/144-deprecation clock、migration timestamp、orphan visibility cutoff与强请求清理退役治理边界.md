...
| 主题 | 关键源码 | 这里回答的问题 |
| --- | --- | --- |
| migration registry | `src/main.tsx:323-345` | 为什么 migration version 只回答要不要重跑迁移动作，不回答旧世界何时正式结束 |
| retirement clock | `src/utils/model/deprecation.ts:21-25,33-61,66-100` | 为什么 repo 已把 retirement date 当成 provider-specific truth |
...
| deprecation warning surface | `src/hooks/notifs/useDeprecationWarningNotification.tsx:6-27,29-42`; `src/main.tsx:2872-2895` | 为什么 sunset warning 是单独 runtime surface，而不是 migration 副产物 |
| compatibility still alive | `src/utils/model/model.ts:549-554` | 为什么 migration 已发生时，旧世界仍可能继续被 remap |
| orphan grace window | `src/utils/plugins/cacheUtils.ts:23-24,66-74,89-105,149-176` | 为什么旧 plugin world 先进入 grace period，再进入 deletion |
