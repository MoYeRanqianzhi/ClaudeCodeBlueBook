...
| auth success restoration | `src/tools/McpAuthTool/McpAuthTool.ts:60,187,195` | 为什么 auth success 主要签 tools/resources become available，而不是 old blocked stronger cleanup request replay |
| preserved step-up intent | `src/components/mcp/MCPRemoteServerMenu.tsx:267`; `src/services/mcp/auth.ts:582-598,909,1896` | 为什么保留更高 scope / state 只说明 higher-authority target continuity，仍不等于 same-request continuation |
...
| control-plane auth choreography | `src/cli/print.ts:3362,3366` | 为什么 control path 主要签 auth completion / action-state change，而不是旧 stronger cleanup request replay |
