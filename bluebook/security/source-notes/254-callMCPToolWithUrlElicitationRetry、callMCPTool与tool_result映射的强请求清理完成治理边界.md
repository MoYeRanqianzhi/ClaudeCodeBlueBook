...
| result normalization | `src/services/mcp/client.ts:2662-2690,2725` | 为什么 completion 关心的是 result settlement，而不是 retry permission |
...
`client.ts:2662-2690,2725` 再给出第二组强证据。
...
`FileWriteTool.ts:424-430` 与 `TaskCreateTool.ts:135` 是很好的正对照。
...
## 8. 为什么这层不等于 `253` 的 continuation 剖面

这里必须单独讲清楚，
否则容易把 `254` 误读成 `253` 的尾注。
...
1. `253` 的典型形态是 retry loop、accept/decline/cancel、retry budget、future readiness ceiling
2. `254` 的典型形态是 could-not-complete、completed progress、completed successfully、normalized result、success tool_result
