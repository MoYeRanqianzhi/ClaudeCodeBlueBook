...
`Claude Code 已经在 stronger-request old cleanup surface 上公开展示至少八种不同制度理由，而且其中 plans family 已经显露出 rationale drift。`

## 3. 关键源码锚点

| 主题 | 关键源码 | 这里回答的问题 |
| --- | --- | --- |
| live non-clobber rationale | `src/utils/task/diskOutput.ts:33-49`; `src/utils/permissions/filesystem.ts:1688-1700` | 为什么 task-output family 被推向 repaired temp law |
| persisted inspection rationale | `src/utils/toolResultStorage.ts:1-5,94-105` | 为什么 tool-results 需要被持久化而不是只留在 live runtime |
| continuity / replay rationale | `src/utils/sessionStorage.ts:198-224` | 为什么 transcripts / casts 继续住在 session persistence tree |
| planning recovery rationale + drift | `src/utils/plans.ts:79-110,164-233`; `src/utils/cleanup.ts:300-303` | 为什么 plans family 既要自治又要恢复，以及为什么 cleanup reason 还没有完全跟上 |
| restore backup rationale | `src/utils/fileHistory.ts:951-957`; `src/utils/cleanup.ts:305-342` | 为什么 file-history family 要按 per-session-dir world 保留与删除 |
| environment replay rationale | `src/utils/sessionEnvironment.ts:15-22,61-143`; `src/utils/cleanup.ts:346-378` | 为什么 session-env family 需要 deterministic replay world |
...
| host monitoring rationale | `src/utils/diagLogs.ts:14-20,27-60` | 为什么 diagnostics family 更接近 host-visible monitoring world |
| family-by-family execution without rationale plane | `src/utils/cleanup.ts:575-598` | 为什么 visible cleanup plane 仍未等强度显式签署 plural rationales |
...
再加上 `permissions/filesystem.ts:1688-1700`：

1. project temp dir 被明确允许 cross-session read
2. 这意味着 task outputs 不是因为“完全隔绝”才安全
3. 而是因为 shared-readable world 里仍被额外施加了 scoped write/delete reason
...
5. fork 会为新 session 生成新 slug，避免旧 plan 被互相 clobber
...
`sessionEnvironment.ts:15-22,61-143`
说明 session-env 首先服务：
