...
`src/services/mcp/useManageMCPConnections.ts:140-153`
...
`src/hooks/notifs/useMcpConnectivityStatus.tsx:25-80`
...
repo 已经在 `pending` 初始化上明确展示：

`reactivation`

之后仍有单独的 readiness 过渡层。

### 启示三

repo 已经在 notification、UI 与 CLI health 上明确展示：

`not-ready truth`

必须被继续显化，而不是被 activation success 遮蔽。

### 启示四

repo 已经在 `ReadMcpResourceTool` 上明确展示：

`consumer gate`

只接受 readiness truth，不接受 mere active truth。
...
本身也是会被运行时证据持续改写的动态主权。

## 11. 一条硬结论

这组源码真正说明的不是：

`stronger-request cleanup 只要补出 reactivation governance，就已经足够成熟。`

而是：

`Claude Code 在 MCP connection state machine、pending re-entry、surface grammar、consumer hard gate 与 runtime revocation grammar 上已经明确展示了 readiness governance 的存在；因此 artifact-family cleanup stronger-request cleanup-reactivation-governor signer 仍不能越级冒充 artifact-family cleanup stronger-request cleanup-readiness-governor signer。`

再压成一句：

`cleanup 线真正缺的不是“谁来把 returned truth 接回当前世界”，而是“谁来决定它接回来以后此刻到底能不能继续被使用”。`
