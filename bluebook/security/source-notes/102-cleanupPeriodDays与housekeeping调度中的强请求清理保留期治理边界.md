...
`src/utils/cleanup.ts:23-30,396-428,575-593` 给出了很硬的正控制面。
...
所以 `251` 的真正技术启示不是“retention horizon 已经统一”，
而是：

`retention governance is partially explicit and partially coverage-limited`

## 8. 为什么这一层不等于 `101` 的 irreversible-erasure gate

这里必须单独讲清楚，
否则最容易把 `102` 误读成 `101` 的尾注。
...
1. `101` 的典型形态是 `truncate`、`unlink(filePath)`、old debug log delete
2. `102` 的典型形态是 `cleanupPeriodDays`、validation skip、delay/activity gate、carrier coverage gap
