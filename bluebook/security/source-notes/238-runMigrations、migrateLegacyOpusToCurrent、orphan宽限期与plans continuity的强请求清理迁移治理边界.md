...
| 主题 | 关键源码 | 这里回答的问题 |
| --- | --- | --- |
| startup migration chain | `src/main.tsx:323-346` | 为什么 repo 已把迁移治理当成显式 startup governance |
...
| orphan grace-window governance | `src/utils/plugins/cacheUtils.ts:23-24,69-105` | 为什么旧 plugin world 要进入 orphan + grace window，而不是立即消失 |
| plan continuity governance | `src/utils/plans.ts:164-255` | 为什么 plan artifact 的过渡需要 resume reuse、missing-file recovery 与 fork copy / new-slug policy |
| cleanup migration gap | `src/utils/cleanup.ts:33-44,300-303,575-598`; `src/utils/diagLogs.ts:14-60` | 为什么 stronger-request cleanup 线当前还没有 old-world exit grammar |
...
## 5. model migration 给出最强正例：成熟系统不会把“更正新规则”直接写成“粗暴覆盖所有旧值”
...
## 6. old-key/new-key handoff 再证明：迁移治理并不等于“把旧字段替换掉”，而是决定新旧字段怎样交接
...
## 10. 苏格拉底式自反诘问：我是不是又把“知道该怎么修”误认成了“知道怎样让旧世界退场”
