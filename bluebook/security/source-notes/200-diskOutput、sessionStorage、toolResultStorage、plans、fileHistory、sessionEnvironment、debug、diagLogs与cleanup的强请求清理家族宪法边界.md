...
`为什么 stronger-request old cleanup carriers 当前明显活在不同 cleanup constitution 里。`
...
把 repaired temp world、project-session sweep world、home-root plural worlds 与 host-external diagnostics world 并排，
逼出一句更硬的结论：

`Claude Code 已经明确展示：stronger-request old cleanup 并不生活在单一 cleanup law 里；因此 noninterference proof 仍不能替代 family constitution proof。`
...
`Claude Code 已经在 stronger-request old cleanup surface 上公开展示至少四种不同 cleanup world：repaired session temp、project-session sweep、home-root plural worlds 与 host-external diagnostics world。`

## 3. 关键源码锚点

| 主题 | 关键源码 | 这里回答的问题 |
| --- | --- | --- |
| repaired temp world | `src/utils/task/diskOutput.ts:33-55`; `src/utils/permissions/filesystem.ts:1688-1700` | 为什么 live runtime task output 现在服从 session-scoped temp constitution |
| project-session sweep world | `src/utils/sessionStorage.ts:198-224`; `src/utils/toolResultStorage.ts:94-118`; `src/utils/cleanup.ts:155-257` | 为什么 transcripts 与 persisted tool-results 仍服从 project-rooted retention sweep |
| home-root plan/debug/backup/env worlds | `src/utils/plans.ts:79-106`; `src/utils/fileHistory.ts:951-957`; `src/utils/sessionEnvironment.ts:15-21`; `src/utils/debug.ts:216-230`; `src/utils/cleanup.ts:300-428` | 为什么离开 project tree 后，home-root cleanup 仍继续分裂成不同子宪法 |
| host-external diagnostics world | `src/utils/diagLogs.ts:27-53` | 为什么 diagnostics carrier 甚至不生活在 repo-local visible cleanup plane |
| family-by-family dispatcher | `src/utils/cleanup.ts:575-596` | 为什么 visible cleanup plane 更像 stitched plural laws，而不是一个显式 constitution registry |
...
3. 而是“read world 可以共享，write/delete boundary 仍要单独受治理”
...
这说明 stronger-request old cleanup 一旦被持久化进 transcript/tool-results，
它面对的已经不是 task-output family 的那条家法。

## 6. `plans`、`fileHistory`、`sessionEnvironment` 与 `debug` 再把问题推进：home-root 也不是一个统一世界
...
其中 plans world 还暴露出一条更深的制度张力：

1. write world 允许 `plansDirectory` project-root override
2. visible cleanup world 却仍主要瞄准默认 `~/.claude/plans`

这说明 stronger-request cleanup constitution 在这里不只是“多元”，
而且还包含：

`storage rationale and cleanup rationale may drift unless the law is explicitly restated`

## 7. `diagLogs.ts` 最后给出最强边界：有些 stronger-request old cleanup carrier 根本不在 repo-local cleanup constitution 内
