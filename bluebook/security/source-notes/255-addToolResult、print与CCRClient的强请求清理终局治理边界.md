...
## 7. 为什么这层不等于 `254` 的 current completion gate

这里必须单独讲清楚，
否则容易把 `255` 误读成 `254` 的尾注。
...
1. `254` 的典型形态是 `tool_result`、`completed progress`、`completed successfully`
2. `255` 的典型形态是 `files_persisted`、`idle`、`flush()` disclaimer、`state_restored`
...
## 8. 苏格拉底式自反诘问：我是不是又把“它这次已经完成”误认成了“它以后回来时仍然成立”
...
5. 如果 idle 已经说明 everything is over，为什么 `files_persisted` 与 `idle` 还要各自单独建 schema？
   因为 turn-over 与 persistence 是两层不同真相。
6. 如果 cleanup 线还没正式长出 finality grammar，是不是说明这层只是事件顺序细节？
