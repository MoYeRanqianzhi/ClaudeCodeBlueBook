# onConnectionAttempt、dynamicMcpState 与 reregisterChannelHandlerAfterReconnect 的重新并入治理边界
...
`cleanup 线未来谁来决定对象是否已经恢复，`

而是：

`cleanup 线一旦已经拿到 recovered truth，谁来决定这份真相何时真正重新进入当前 tool/status/event world。`
...
`Claude Code 已经在 MCP 线上明确展示：恢复成立，并不自动回答当前世界是否已经重新接回它；cleanup 线当前缺的不是这种思想，而是这套 reintegration governance 还没被正式接到旧 path、旧 promise 与旧 receipt 世界上。`
...
`Claude Code 已经在 MCP 线上明确把“恢复是否成立”和“恢复后的真相何时重新成为当前世界的一部分”拆成两层；cleanup 线当前缺的不是文化，而是这套 reintegration governance 还没被正式接到旧 cleanup artifact family 上。`

## 3. 关键源码锚点

| 主题 | 关键源码 | 这里回答的问题 |
| --- | --- | --- |
| raw recovery result | `src/services/mcp/client.ts:2137-2163` | 为什么 reconnect 成功本身还不等于 current-world membership |
...
如果 cleanup 线未来没有这类 live interaction reintegration grammar，
那它就仍然回答不了：
...
这条证据把 reintegration 从 “state bookkeeping” 提升成了真正的 model-visible world re-entry。
...
`recovered result != current-world membership`

### 启示二

repo 已经在 `dynamicMcpState` 上明确展示：

`recovered truth != next-turn tool visibility`

### 启示三

repo 已经在 handler re-registration 上明确展示：

`recovered connection != live interaction re-entry`

### 启示四

repo 已经在 pseudo-tool replacement 上明确展示：

`recovered proof != real tool-world reintegration`

这四句合起来，
正好说明为什么 cleanup 线未来不能把 recovery governance 直接偷写成 complete reintegration。
...
`repo 在 reconnectMcpServerImpl 的 raw result、useManageMCPConnections.onConnectionAttempt 的 state/event projection、print 的 appState/dynamicMcpState reinsertion、buildAllTools/buildMcpServerStatuses 的 consumer read model，以及 McpAuthTool 的 pseudo-tool replacement 上，已经明确展示了 reintegration governance 的存在；因此 artifact-family cleanup recovery-governor signer 仍不能越级冒充 artifact-family cleanup reintegration-governor signer。`

因此：

`cleanup 线真正缺的不是“谁来宣布它已经恢复”，而是“谁来决定恢复后的它何时重新成为当前世界的一部分”。`
