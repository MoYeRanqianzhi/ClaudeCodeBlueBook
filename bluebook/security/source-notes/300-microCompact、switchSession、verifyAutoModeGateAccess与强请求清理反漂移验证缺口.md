...
| stronger-request cleanup conformance cluster | `src/utils/sessionStorage.ts:953-969`; `src/utils/backgroundHousekeeping.ts:28-80`; `src/utils/cleanup.ts:33-45,300-303,575-598`; `src/utils/plans.ts:79-110,164-255`; `src/utils/diagLogs.ts:14-60` | 为什么 cleanup 线当前仍主要是在表达 conformance symptom，而不是 verifier grammar |
...
`cleanup 线真正缺的，不只是“这次是否 conform”，还包括“谁来保证下次再偏离时系统会自己发现，而且发现得足够响”。`
