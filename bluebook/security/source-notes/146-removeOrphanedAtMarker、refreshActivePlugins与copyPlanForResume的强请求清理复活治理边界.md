...
repo 已经在 plugin marker clearing 上明确展示：

`撤销“你已退役”的历史判断`

不等于

`重新接纳你进入 current world`

### 启示二

repo 已经在 Layer-3 refresh 与 `/reload-plugins` 上明确展示：

`resurrection` 需要显式 re-entry gate，
而不是 marker 清除后的自然副作用。
...
repo 已经在 plan 恢复线上明确展示：

`历史对象回来时仍需要 lineage、evidence 与 identity policy。`

### 启示五

repo 通过 tombstone deletion path 的克制设计反向证明：
...
`Claude Code 在 authoritative marker clearing、Layer-3 refresh、needsRefresh、lineage-based plan recovery 与 deletion-only tombstone path 上已经明确展示了 resurrection governance 的存在；因此 artifact-family cleanup stronger-request cleanup-tombstone-governor signer 仍不能越级冒充 artifact-family cleanup stronger-request cleanup-resurrection-governor signer。`

再压成一句：

`cleanup 线真正缺的不是“谁来留墓碑”，而是“谁来决定带着这些墓碑的旧对象凭什么回来”。`
