...
## 7. 为什么这层不等于 `63` 的 current completion gate
...
1. `63` 的典型形态是 `tool_result`、`completed progress`、`completed successfully`
2. `64` 的典型形态是 `files_persisted`、`idle`、`flush()` disclaimer、`state_restored`
...
这四句合起来，
正好说明为什么 stronger-request cleanup 线未来不能把 continued stronger-request completion 直接偷写成 continued stronger-request finality。
