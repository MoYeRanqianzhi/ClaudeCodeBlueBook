...
| effect persistence emit | `src/cli/print.ts:2252-2272` | 为什么 effect persistence 要单独建成 `files_persisted` |
| idle choreography | `src/cli/print.ts:2455-2468` | 为什么 going idle 前要先 flush internal events |
| finality schemas | `src/entrypoints/sdk/coreSchemas.ts:1672-1690,1735-1746` | 为什么 `files_persisted` 与 `idle` 是不同强度的 system signals |
...
而 `coreSchemas.ts:1735-1746`
与 `sdkEventQueue.ts:60-90`
又把：
...
## 7. 为什么这层不等于 `127` 的 current completion gate

这里必须单独讲清楚，
否则容易把 `128` 误读成 `127` 的尾注。
...
1. `127` 的典型形态是 `tool_result`、`completed progress`、`completed successfully`
2. `128` 的典型形态是 `files_persisted`、`idle`、`flush()` disclaimer、`state_restored`
