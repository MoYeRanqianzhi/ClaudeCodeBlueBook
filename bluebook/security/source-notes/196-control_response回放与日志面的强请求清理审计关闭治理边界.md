...
| duplicate trace logging | `src/cli/structuredIO.ts:391-398` | 为什么 duplicate old echo 仍留下 debug trace |
...
## 7. 为什么这层不等于 `195` 的 archive-close gate

这里必须单独讲清楚，
否则容易把 `196` 误读成 `195` 的尾注。
...
1. `195` 的典型形态是 lifecycle close、stale prompt teardown、queue non-admission、inline completed contract
2. `196` 的典型形态是 replay propagation、replay enqueue、debug trace、diagnostics parse
