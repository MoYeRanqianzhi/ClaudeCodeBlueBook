...
把 stronger-request family truth 当前散落在哪些 helper、哪些 schema、哪些 permission rule、哪些 comment、哪些 recovery path、哪些 dispatcher 与哪些 env contract 里，以及为什么这仍不等于 metadata plane，单独钉死。
...
`Claude Code 已经有足够多 stronger-request family truth，但这些 truth 仍主要以 helper composition、local comments、局部 schema、permission projection 与硬编码 dispatcher 的形式存在，还没有长成统一的 cleanup metadata grammar。`
...
| local explicit truth | `src/utils/task/diskOutput.ts:33-49`; `src/utils/diagLogs.ts:11-20,27-60` | 为什么有些 family 的理由虽清楚，却仍主要停留在 local comments / helpers |
...
| permission-side projection | `src/utils/permissions/filesystem.ts:245-255,1487-1494,1644-1652` | 为什么 permission plane 当前只消费 plan-family truth 的局部投影 |
| propagation failure | `src/utils/plans.ts:79-106,164-233`; `src/utils/settings/types.ts:824-830`; `src/utils/cleanup.ts:300-303` | 为什么 plans family 最能暴露没有 metadata plane 时的传播失灵 |
...
`diskOutput.ts:33-49` 已经把 task-output family 的 truth 写得非常清楚：
...
`plansDirectory` 已经能改写 storage / permission world，cleanup 仍只清默认 home-root world。
...
## 8. file-history、session-env、debug 与 permission plane 再证明：系统当然知道 family 有别，但知道有别不等于把“有别”保存成统一 grammar

`fileHistory.ts:951-957`、`sessionEnvironment.ts:15-21,60-140` 与 `debug.ts:30-68,91-111,230-249` 已经分别把：
...
`permissions/filesystem.ts:245-255,1487-1494,1644-1652` 也明确知道 plans family 的 current-session 读写应该被允许。
...
1. 高级安全设计不会只问“系统有没有理由”，还会继续追问“这份理由被系统自己保存在哪里”
2. local explicit truth、cross-file puzzle truth 与 propagation failure 是一条连续工程谱系，而不是三个彼此无关的问题
3. permission plane 是否在消费同一份 truth，是判断 metadata 是否真实存在的试金石
...
`Claude Code 已经拥有大量 stronger-request family-level truth，但这些 truth 仍主要寄居在 cleanup、settings、path helpers、permission rules、env contract 与 comments 的分散组合里；因此 artifact-family cleanup stronger-request cleanup-metadata-governor signer 仍必须独立于 artifact-family cleanup stronger-request cleanup-rationale-governor signer 被追问。`
