# callMCPToolWithUrlElicitationRetry、callMCPTool 与 tool_result 映射的强请求清理完成治理边界
...
## 8. 为什么这层不等于 `94` 的 continuation 剖面
...
1. `94` 的典型形态是 retry loop、accept/decline/cancel、retry budget、future readiness ceiling
2. `95` 的典型形态是 could-not-complete、completed progress、completed successfully、normalized result、success tool_result
