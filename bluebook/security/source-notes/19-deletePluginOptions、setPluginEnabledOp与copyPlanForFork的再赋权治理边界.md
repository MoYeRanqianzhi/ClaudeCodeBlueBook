# deletePluginOptions、setPluginEnabledOp 与 copyPlanForFork 的再赋权治理边界
...
`cleanup 线即便让旧对象回来，谁来决定它回来后还拿不拿得回原来的资格。`
...
`Claude Code 已经在别处明确展示：对象回来，并不自动回答它是否恢复旧配置、旧 secrets、旧 scope、旧 identity 与旧可用资格；cleanup 线当前缺的不是这种思想，而是这套再赋权治理还没被正式接到旧 path、旧 promise 与旧 receipt 世界上。`
...
`Claude Code 在 plugin 与 plan 线上已经明确把“对象回来”和“回来后恢复哪些旧资格”拆成两层；cleanup 线当前缺的不是文化，而是这套 re-entitlement governance 还没被正式接到旧 cleanup artifact family 上。`
...
## 7. `installed_plugins.json` 与 `enabledPlugins` 的 divergence guard 再证明：current entitlement 不是单看 object 在不在
...
`cleanup 线未来只要补出 resurrection governance，就已经足够成熟。`
...
`cleanup 线真正缺的不是“谁来让旧对象回来”，而是“谁来决定它回来后还能拿回多少原来的资格”。`
