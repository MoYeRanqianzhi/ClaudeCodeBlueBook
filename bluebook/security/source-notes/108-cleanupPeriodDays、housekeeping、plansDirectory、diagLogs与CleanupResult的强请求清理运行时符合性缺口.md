...
| delayed admission | `src/screens/REPL.tsx:3903-3906`; `src/main.tsx:2811-2818`; `src/utils/backgroundHousekeeping.ts:28-58,77-80` | 为什么 cleanup 进入 runtime 的方式本身就是延迟、可重排的 |
| intentional skip | `src/utils/cleanup.ts:575-584` | 为什么 runtime 会为了 fail-safe 原则而直接不执行 cleanup |
| propagation failure | `src/utils/plans.ts:79-106`; `src/utils/settings/types.ts:824-830`; `src/utils/permissions/filesystem.ts:245-255,1487-1494,1644-1652`; `src/utils/cleanup.ts:300-303` | 为什么 metadata 即使存在，也不自动保证 executor 跟随 |
...
`main.tsx:2811-2818` 则在非 bare headless world 中后台导入 housekeeping。

`backgroundHousekeeping.ts:28-58,77-80` 更进一步把时间结构钉死：
...
## 8. `CleanupResult`、stale-worktree telemetry 与 diagnostics uncovered gap 最后说明：repo 已经拥有局部 outcome vocabulary，却还没有 stronger-request family 级 conformance receipt

`cleanup.ts:33-44` 定义了 `CleanupResult`。

这说明作者知道 cleanup outcome 值得计数。

但 `cleanup.ts:575-597` 的后台入口没有把这些 result 汇总、保存或投影；
反而只有 stale worktrees 获得了单独 telemetry。

与此同时，
`diagLogs.ts:11-20,27-60` 仍只定义 diagnostics writer 与 env-selected logfile，
却没有进入 `cleanupOldMessageFilesInBackground()` 的 visible family list。
...
1. 成熟安全系统不会把声明 truth 当作运行 truth，而会把时间、调度、skip 与 receipt 单独拆开
2. suppression、admission、execution、coverage 与 surfaced receipt 是五个不同问题，任何一句宣传语都不该把它们糊成一句
3. propagation gap 是 runtime-conformance 的硬证据，因为它直接暴露出“规则被记住”与“规则被执行”之间的断层
4. local outcome vocabulary 的存在说明系统已经知道自己需要 receipt，只是还没有把 receipt 升格成 stronger-request family plane
