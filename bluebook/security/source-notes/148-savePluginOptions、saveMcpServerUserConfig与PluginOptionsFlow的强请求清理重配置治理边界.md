...
`Claude Code 在 split-save、needs-config、configured/skipped 与 take-effect grammar 上已经明确展示了 reconfiguration governance 的存在；因此 artifact-family cleanup stronger-request cleanup-re-entitlement-governor signer 仍不能越级冒充 artifact-family cleanup stronger-request cleanup-reconfiguration-governor signer。`

再压成一句：

`cleanup 线真正缺的不是“谁来让旧对象拿回 seat”，而是“谁来决定它拿回 seat 后按哪组 current truth 重新工作”。`
