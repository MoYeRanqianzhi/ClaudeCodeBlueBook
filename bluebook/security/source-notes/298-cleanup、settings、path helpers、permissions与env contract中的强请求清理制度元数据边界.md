...
| path-level grammar | `src/utils/toolResultStorage.ts:26-31,94-105`; `src/utils/sessionStorage.ts:202-258` | 为什么 tool-results / transcripts 已经拥有 naming grammar，却仍不是 shared descriptor |
| cross-plane propagation stress test | `src/utils/plans.ts:79-110,119-129,164-255`; `src/utils/permissions/filesystem.ts:244-255,1487-1495,1644-1652`; `src/utils/cleanup.ts:300-303` | 为什么 plans family 最能暴露 canonical carrier 缺位时的传播失灵 |
| stable but still local grammar | `src/utils/fileHistory.ts:951-957`; `src/utils/sessionEnvironment.ts:15-31,72-100`; `src/utils/debug.ts:44-56,91-101,230-249` | 为什么有些 family 即使暂时不失灵，也仍主要依靠 local microgrammar 生存 |
...
`toolResultStorage.ts:26-31,94-105`
又把 tool-results 编成：
...
5. fork 时为新 session 生成新 slug，避免旧新会话互相 clobber
...
`sessionEnvironment.ts:15-31,72-100`
把 replay family 编成：
...
`cleanup 线真正缺的，不只是“谁来解释这些 law 为什么合理”，还包括“谁来把这些 why 登记成 settings、storage、permissions、recovery、cleanup executor 与 host contract 可以共读的制度记忆”。`
