...
1. 旧 continuity line 的延长不能自动等于恢复 verdict
2. recovery 必须绑定新的 credential evidence、新的 connect result 与新的 tool world
3. control plane 必须把 retry、auth、callback 与 recovered 分开显化，否则弱事件会偷签强事实
4. consumer plane 必须只在真正 recovered 后才重新交回 real tools，否则会把恢复中的世界冒充成已恢复世界
5. silent path 与 interactive path 可以不同，但 recovery grammar 必须要求同一种 fresh-proof discipline

## 10. 第一性原理：恢复治理真正保护的不是“努力”，而是“新真相的可签字性”

如果从第一性原理追问，
recovery governance 保护的不是：

`系统有没有表现出想把它救回来`

而是：

`系统有没有拿到足以重新签字的 fresh truth`
...
1. 把 retry effort 误写成 recovered fact
2. 把 auth progress 误写成 service truth
3. 把 consumer hope 误写成 consumer-safe reality

Claude Code 在这组代码里先进的地方，
恰恰在于它没有把“意图恢复”偷写成“恢复成立”。

## 11. 苏格拉底式自反诘问：我是不是又把“系统正在恢复”误认成了“系统已经恢复”

如果对这组代码做更严格的自我审查，
至少要追问六句：

1. 如果 continuity grammar 已经足够强，为什么还要再拆 recovery？
   因为旧线还要不要继续，不等于什么新证据足以宣布恢复成立。
2. 如果 reconnect 已经发起，是不是就说明 recovered？
   不是。`reconnectMcpServerImpl()` 只接受新的 `connected` 结果。
3. 如果 auth URL 已经给出，是不是就说明恢复路径已经完成？
   不是。auth-start、callback 与 reconnect-complete 被显式拆开。
4. 如果 silent re-auth 能跑，是不是就说明 recovery verdict 可以提前签？
   也不对。silent 与 interactive 都只是证明收集路径，仍需新的 credential proof 闭环。
5. 如果 consumer 看到 auth tool 了，是不是就说明真实工具世界已经回来？
   恰恰相反。auth tool 正是在表达“真实工具世界还没回来”。
6. 如果 cleanup 线现在还没有显式 recovery 代码，是不是说明这层还不值得写？
   恰恰相反。越是缺这层明确 grammar，越容易把“仍在恢复中”偷写成“已经恢复”。`

这一串反问最终逼出一句更稳的判断：

`recovery 的关键，不在系统会不会继续抢救旧线，而在系统能不能正式决定什么新的证据足以宣布 recovered truth 重新成立。`
...
`Claude Code 在 needs-auth demotion、fresh reconnect verdict、新凭据收集、control-plane success grammar 与 consumer-side real-tool return discipline 上已经明确展示了 recovery governance 的存在；因此 artifact-family cleanup stronger-request cleanup-continuity-governor signer 仍不能越级冒充 artifact-family cleanup stronger-request cleanup-recovery-governor signer。`
