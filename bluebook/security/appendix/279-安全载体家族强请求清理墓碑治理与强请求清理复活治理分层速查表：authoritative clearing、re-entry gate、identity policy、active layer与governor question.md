...
这一页服务于 [295-安全载体家族强请求清理墓碑治理与强请求清理复活治理分层：为什么artifact-family cleanup stronger-request cleanup-tombstone-governor signer不能越级冒充artifact-family cleanup stronger-request cleanup-resurrection-governor signer](../295-安全载体家族强请求清理墓碑治理与强请求清理复活治理分层.md)。
...
| --- | --- | --- | --- | --- | --- |
| plugin orphan marker | old version remains marked by `.orphaned_at` | only authoritative installed set can clear marker | marker clear still != active components return | who decides which retired cleanup object may actually re-enter current world | `cacheUtils.ts:67-72,82-92,122-130` |
| plugin active world | object may exist again on disk | `/reload-plugins` + `refreshActivePlugins()` is explicit Layer-3 re-entry gate | active world is runtime admission, not automatic side effect | who decides when marker-cleared object becomes active truth in current session | `refresh.ts:1-18,72-80,123-137`; `reload-plugins.ts:37-56` |
| plugin update path | install/update already happened | updates only stop at `needsRefresh`; some new installs may auto-refresh, failure falls back to pending-like state | repo distinguishes auto-admit, manual admit, and not-yet-admitted worlds | who decides which resurrection path may auto-apply and which must remain explicit | `PluginInstallationManager.ts:56-59,135-179`; `useManagePlugins.ts:28-34,287-303` |
| plan recovery path | history / snapshot still exists | `copyPlanForResume()` requires lineage + evidence and is called from formal recovery flow | `copyPlanForFork()` creates new slug instead of restoring old identity | who decides whether cleanup object returns as old identity, new identity, or not at all | `plans.ts:164-231,233-257`; `conversationRecovery.ts:539-547` |
| tombstone deletion path | tombstone removes old transcript object | no re-entry logic here | delete path is not restore path | who distinguishes deletion grammar from resurrection grammar in cleanup world | `sessionStorage.ts:871-945,1468-1474` |
| stronger-request cleanup current gap | tombstone question is now visible | no explicit resurrection gate yet | old path / old promise / old receipt world still lack formal re-entry policy | who decides whether retired cleanup objects can come back, and on what identity / admission terms | `utils/cleanup.ts:33-45,300-303,575-597`; `utils/plans.ts:79-110` |
