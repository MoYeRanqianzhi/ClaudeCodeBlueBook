...
这一页服务于 [432-安全载体家族强请求清理重新担保治理与强请求清理用时重验证治理分层](../432-安全载体家族强请求清理重新担保治理与强请求清理用时重验证治理分层.md)。
...
| runtime contradiction | decide old positive wording may still stand for now | decide whether 401 / expired session must immediately revoke old reassurance | `McpAuthError` / `clearServerCache()` / `McpSessionExpiredError` | who decides when live use has disproved the last reassuring sentence | `src/services/mcp/client.ts:3200-3230` |
| tool roster freshness | decide some tool list once looked good enough to reassure | decide whether current roster must be refreshed when the server says tools changed | `tools/list_changed` invalidates `fetchToolsForClient` cache and re-fetches tools | who decides whether old capability rosters may keep standing in for current world | `src/services/mcp/useManageMCPConnections.ts:620-656` |
