...
这一页服务于 [437-安全载体家族强请求清理终局治理与强请求清理遗忘治理分层](../437-安全载体家族强请求清理终局治理与强请求清理遗忘治理分层.md)。
...
| “transcript 已 resolved，所以别处就不必再保留防重记忆” | transcript truth != anti-replay sufficiency |
...
`continued stronger cleanup request is final -> keep retained anti-duplicate memory -> block duplicate/orphan replay -> evict only under a bounded forgetting policy -> only then allow forgetting`
