...
这一页服务于 [425-安全载体家族强请求清理重配置治理与强请求清理重新激活治理分层](../425-安全载体家族强请求清理重配置治理与强请求清理重新激活治理分层.md)。
...
| install / update policy split | same disk-change family exists | new install may auto-refresh, update may not | scenario-sensitive reactivation rule | stronger-request cleanup has no scenario-sensitive takeover policy | who decides whether same cleanup truth change auto-activates or waits for explicit trigger | `PluginInstallationManager.ts:52-59,135-180` |
| headless path | config truth available before first ask | `refreshPluginState()` auto-consumes via `refreshActivePlugins()` | mode-specific auto-activation | stronger-request cleanup has no headless auto-consume rule | who decides that headless mode may auto-consume while interactive mode may not | `print.ts:1749-1769,3071-3080`; `AppStateStore.ts:209-215` |

## 3. 三个最重要的判断问题

判断一句

`既然 returned object 的配置已经保存好了，所以它现在也已经真正生效了`
