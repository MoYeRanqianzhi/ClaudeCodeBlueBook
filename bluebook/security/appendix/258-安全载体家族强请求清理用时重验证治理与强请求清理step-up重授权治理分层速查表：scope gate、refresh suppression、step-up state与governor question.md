...
这一页服务于 [274-安全载体家族强请求清理用时重验证治理与强请求清理step-up重授权治理分层](../274-安全载体家族强请求清理用时重验证治理与强请求清理step-up重授权治理分层.md)。
...
| token issuance path | decide whether current tokens are still fresh enough to use | decide whether refresh token must be suppressed because current scope is insufficient | `tokens()` + `needsStepUp` + refresh-token omission | who decides when refresh is forbidden because the problem is authority level, not freshness | `auth.ts:1625-1690` |
| step-up pending state | decide current use path needs a fresh re-auth run | decide what stronger-scope intent must persist across revoke / re-auth | `markStepUpPending(scope)` + preserved step-up scope | who decides how stronger cleanup authorization survives intermediate revocation and resumes on the next flow | `auth.ts:1468-1470,579-618,909-940,1887-1900` |
| runtime request authority | decide current consumer has a fresh enough current-use proof | decide whether the request still needs stronger reauthorization rather than plain reconnect/refresh | step-up pending + PKCE fallthrough | who decides which stronger cleanup requests need elevation even after live-use revalidation succeeded | `auth.ts:1468-1470,1625-1690` |
