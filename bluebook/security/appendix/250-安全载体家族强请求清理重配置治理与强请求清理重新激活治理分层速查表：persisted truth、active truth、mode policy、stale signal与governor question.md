...
这一页服务于 [266-安全载体家族强请求清理重配置治理与强请求清理重新激活治理分层：为什么artifact-family cleanup stronger-request cleanup-reconfiguration-governor signer不能越级冒充artifact-family cleanup stronger-request cleanup-reactivation-governor signer](../266-安全载体家族强请求清理重配置治理与强请求清理重新激活治理分层.md)。
...
| --- | --- | --- | --- | --- | --- |
| plugin config save surface | `Configuration saved` is visible | `take effect` is still pending | user must explicitly run `/reload-plugins` in interactive flows | who decides when saved cleanup truth becomes running-session truth | `ManagePlugins.tsx:1645-1668,1700-1707` |
| Layer-3 refresh primitive | new config already exists on disk | `refreshActivePlugins()` swaps active components and consumes `needsRefresh` | reactivation is a distinct runtime choreography | who decides when persisted cleanup truth may replace stale active truth | `refresh.ts:1-18,59-71,72-160` |
| explicit reload command | pending plugin changes already exist | `/reload-plugins` converts them into `Reloaded: ...` active state | interactive user-initiated activation path | who decides when a user command is required to activate returned cleanup truth | `reload-plugins/index.ts:1-13`; `reload-plugins.ts:34-56` |
| background install/update path | plugin state changed on disk | new install may auto-refresh; updates stop at `needsRefresh` | scenario-sensitive rollout policy | who decides which changed cleanup truth may auto-apply and which must stay pending | `PluginInstallationManager.ts:56-59,135-180` |
...
| headless auto-consume path | config/plugin truth is ready before first ask | `refreshPluginState()` auto-consumes via `refreshActivePlugins()` | headless mode uses a different activation authority | who decides when non-interactive cleanup truth may auto-takeover the session | `AppStateStore.ts:206-214`; `print.ts:1733-1765,1881-1912` |
...
| “看到了通知，所以已经完成接管” | notification != activation |
