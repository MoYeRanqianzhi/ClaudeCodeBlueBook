...
这一页服务于 [357-安全载体家族强请求清理退役治理与强请求清理墓碑治理分层](../357-安全载体家族强请求清理退役治理与强请求清理墓碑治理分层.md)。
...
| --- | --- | --- | --- | --- | --- | --- |
| orphaned assistant messages after streaming fallback | explicit `tombstone` message | stream handler + `QueryEngine` | old partial assistant state stops counting and is routed to deletion instead of silent drift | stronger-request cleanup has no explicit post-retirement control message for old carrier objects | who signs the minimal machine signal that says an old cleanup carrier must now be removed, not re-read | `query.ts:712-719`; `utils/messages.ts:2954-2957`; `QueryEngine.ts:757-760` |
