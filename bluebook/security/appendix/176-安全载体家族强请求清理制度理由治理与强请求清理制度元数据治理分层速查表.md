...
这一页服务于 [192-安全载体家族强请求清理制度理由治理与强请求清理制度元数据治理分层：为什么artifact-family cleanup stronger-request cleanup-rationale-governor signer不能越级冒充artifact-family cleanup stronger-request cleanup-metadata-governor signer](../192-安全载体家族强请求清理制度理由治理与强请求清理制度元数据治理分层.md)。
...
| plans | settings schema + storage helper + permission rule + resume helper + cleanup helper | settings, storage, read access, resume recovery, cleanup | strongest cross-plane propagation gap | `plansDirectory` changes storage world while cleanup still targets default root | who preserves plan-family truth across all planes | `settings/types.ts:824-830`; `plans.ts:79-106,164-233`; `cleanup.ts:300-303`; `permissions/filesystem.ts:245-254,1645-1652` |
| file-history / session-env | module-local path helpers + cleanup helpers | restore/replay flows + per-session cleanup | stable local truth, but no shared restore/replay family grammar | lower now, but still not objectified | who turns restore/replay local truth into explicit metadata | `fileHistory.ts:951-957`; `sessionEnvironment.ts:14-21,60-140`; `cleanup.ts:305-387` |
| debug logs | argv/env checks + comments + debug path helper + cleanup helper | operational debugging, `/share`, bug-report support | operational truth not centralized with other retained carriers | medium; truth explicit but stays operationally local | who encodes operational-log duty as metadata instead of comments/argv conventions | `debug.ts:30-68,97-108,229-249`; `cleanup.ts:391-428` |
