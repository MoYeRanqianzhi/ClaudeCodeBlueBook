...
这一页服务于 [258-安全载体家族强请求清理运行时符合性治理与强请求清理反漂移验证治理分层：为什么artifact-family cleanup stronger-request cleanup-runtime-conformance-governor signer不能越级冒充artifact-family cleanup stronger-request cleanup-anti-drift-verifier signer](../258-安全载体家族强请求清理运行时符合性治理与强请求清理反漂移验证治理分层.md)。
...
| diagnostics family | writer/env contract exists but cleanup coverage absent | covered-family set is still implicit and incomplete | uncovered-carrier drift can stay silent | who signs cleanup coverage set and alarms on omitted carriers | `diagLogs.ts:11-20,27-60`; `cleanup.ts:575-595` |
...
| “diagnostics 不在 cleanup 里只是范围问题，不是 drift 问题” | uncovered family set 本身就是 verifier 问题 |
