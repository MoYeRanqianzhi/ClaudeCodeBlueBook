...
这一页服务于 [224-安全载体家族强请求清理制度理由治理与强请求清理制度元数据治理分层：为什么artifact-family cleanup stronger-request cleanup-rationale-governor signer不能越级冒充artifact-family cleanup stronger-request cleanup-metadata-governor signer](../224-安全载体家族强请求清理制度理由治理与强请求清理制度元数据治理分层.md)。
...
| --- | --- | --- | --- | --- | --- | --- |
| task outputs | `diskOutput.ts` comment + path helper | task-output writer/reader paths | no shared family descriptor across cleanup / permission / drift-check planes | currently low; truth is explicit but local | who turns explicit local truth into reusable metadata | `diskOutput.ts:31-49` |
...
| transcripts / casts | transcript path helper + session persistence helpers | transcript writes + project-session cleanup | continuity/replay reason not centralized with tool-results despite same constitution world | medium; same law, different rationale, no explicit metadata split | who separates continuity metadata from inspection metadata | `sessionStorage.ts:198-220`; `cleanup.ts:155-195` |
...
| file-history / session-env | module-local path helpers + cleanup helpers + recovery/replay helpers | restore/replay flows + per-session cleanup | stable local truth, but no shared restore/replay family grammar | lower now, but still not objectified | who turns restore/replay local truth into explicit metadata | `fileHistory.ts:951-957`; `sessionEnvironment.ts:14-21,60-140`; `cleanup.ts:305-387` |
| debug logs | argv/env checks + comments + debug path helper + cleanup helper | operational debugging, `/share`, bug-report support | operational truth not centralized with other retained carriers | medium; truth explicit but stays operationally local | who encodes operational-log duty as metadata instead of comments/argv conventions | `debug.ts:30-68,91-111,229-249`; `cleanup.ts:391-428` |
