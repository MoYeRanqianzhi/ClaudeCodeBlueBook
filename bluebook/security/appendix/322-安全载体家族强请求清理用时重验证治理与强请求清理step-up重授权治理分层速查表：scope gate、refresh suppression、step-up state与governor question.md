...
这一页服务于
[338-安全载体家族强请求清理用时重验证治理与强请求清理step-up重授权治理分层](../338-安全载体家族强请求清理用时重验证治理与强请求清理step-up重授权治理分层.md)。
...
| cached higher-scope continuation | decide current use path is live enough to reach re-auth | decide whether next auth flow should inherit stronger-scope target and discovery hints | cached `stepUpScope` + persisted stronger-scope state | who decides what higher-authority target the next cleanup auth flow should continue toward | `auth.ts:909,1896` |
| higher-authority completion | decide current use path has returned usable tokens | decide when higher-authority pending state is actually resolved | `_pendingStepUpScope = undefined` on `saveTokens()` | who decides when stronger-scope repair is complete rather than merely pending | `auth.ts:1705` |
