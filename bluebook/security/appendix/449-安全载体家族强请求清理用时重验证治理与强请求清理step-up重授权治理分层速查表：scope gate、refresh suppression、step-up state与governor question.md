...
这一页服务于 [465-安全载体家族强请求清理用时重验证治理与强请求清理step-up重授权治理分层](../465-安全载体家族强请求清理用时重验证治理与强请求清理step-up重授权治理分层.md)。
...
| --- | --- | --- | --- | --- | --- |
| transport fetch | decide whether current request is still live enough to proceed | decide whether `403 insufficient_scope` must bypass ordinary refresh thinking and enter step-up pending | `wrapFetchWithStepUpDetection()` + live transport hook | who decides a fresh cleanup use still lacks sufficient authority level for the stronger request | `src/services/mcp/auth.ts:1345-1370`; `src/services/mcp/client.ts:620-634,821-828` |
| token issuance path | decide whether current tokens are still fresh enough to use | decide whether refresh token must be suppressed because current scope is insufficient | `tokens()` + `needsStepUp` + refresh-token omission | who decides when refresh is forbidden because the problem is authority level rather than freshness | `src/services/mcp/auth.ts:1625-1692` |
| refresh path law | decide whether ordinary expiry handling may continue | decide whether RFC 6749 §6 blocks refresh from impersonating scope elevation | proactive refresh skip when `needsStepUp` | who decides that current cleanup proof must leave the refresh path and enter a stronger authorization path | `src/services/mcp/auth.ts:1461-1470,1645-1650` |
| step-up pending persistence | decide current use path needs a fresh re-auth run | decide what stronger-scope intent must persist across revoke / re-auth | `markStepUpPending(scope)` + preserved `stepUpScope` / `resourceMetadataUrl` | who decides how stronger cleanup authorization survives intermediate revocation and resumes on the next flow | `src/services/mcp/auth.ts:578-616,903-935,1461-1470,1884-1899` |
| auth result semantics | decide auth flow has produced a fresh usable auth result | decide whether nominal `AUTHORIZED` is still insufficient when requested scope has not actually been elevated | `sdkAuth(...)` result + upscoping-loop commentary | who decides when a nominally authorized cleanup state still lacks requested higher authority | `src/services/mcp/auth.ts:1220-1228,1345-1352` |
...
1. 这里回答的只是当前连接 / 当前使用资格是否 fresh enough，还是已经回答请求所需更高 authority level 是否足够
2. 当前问题是 freshness 问题，还是 `insufficient_scope` 这类 authority-level 问题
3. 当前系统是在 refresh 一个已有等级的 token，还是已经为更高等级请求进入 step-up reauthorization 并具备完成语义
