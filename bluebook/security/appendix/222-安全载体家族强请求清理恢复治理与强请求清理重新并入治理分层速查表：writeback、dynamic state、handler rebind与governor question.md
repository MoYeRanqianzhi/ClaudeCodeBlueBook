...
这一页服务于
[238-安全载体家族强请求清理恢复治理与强请求清理重新并入治理分层](../238-安全载体家族强请求清理恢复治理与强请求清理重新并入治理分层.md)。
...
| consumer read model | decide whether recovered truth exists | decide whether tool / status consumers can actually see it now | `buildAllTools()` + `buildMcpServerStatuses()` read `dynamicMcpState` and `appState` | who decides whether restored cleanup truth is only true in recovery logic or already visible to downstream consumers | `print.ts:1468-1490,1608-1630` |
| event / channel plane | decide whether recovery has reached `connected` | decide whether event handlers and channel notifications are actually back on the live connection | `registerElicitationHandlers()` + `reregisterChannelHandlerAfterReconnect()` | who decides when restored cleanup truth is once again allowed to participate in live interactive current world | `print.ts:1248-1285,4786-4845` |
