...
这一页服务于 [206-安全载体家族强请求清理恢复治理与强请求清理重新并入治理分层](../206-安全载体家族强请求清理恢复治理与强请求清理重新并入治理分层.md)。
...
| event/channel plane | decide whether recovery has reached `connected` | decide whether event handlers and channel notifications are actually back on the live connection | `registerElicitationHandlers()` + `reregisterChannelHandlerAfterReconnect()` | who decides when restored cleanup truth is once again allowed to participate in live interactive current world | `print.ts:1253-1280,4786-4835` |
