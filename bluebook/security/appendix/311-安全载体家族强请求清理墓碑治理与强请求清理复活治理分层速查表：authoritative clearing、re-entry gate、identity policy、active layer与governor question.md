...
这一页服务于 [327-安全载体家族强请求清理墓碑治理与强请求清理复活治理分层](../327-安全载体家族强请求清理墓碑治理与强请求清理复活治理分层.md)。
...
| positive control / cleanup surface | authoritative clearing | re-entry gate / active layer | identity or continuity policy | governor question | 关键证据 |
| --- | --- | --- | --- | --- | --- |
| plugin orphan marker | installed set clears `.orphaned_at` | marker clear itself does not reload active components | same version can be re-admitted only after later Layer-3 refresh | who decides that a cleared cleanup marker means “eligible for return” rather than “already returned” | `cacheUtils.ts:67-72,82-92,122-130` |
| active plugin return | `/reload-plugins` and `refreshActivePlugins()` perform Layer-3 swap | explicit active-world re-entry signal | current session state rebuilt and `needsRefresh` consumed | who decides when old cleanup objects are actually readmitted into current runtime world | `refresh.ts:1-18,72-80,123-137`; `reload-plugins.ts:37-56` |
| mode-sensitive plugin comeback | auto-refresh only for some install paths; others stop at `needsRefresh` | auto-admit vs explicit-admit vs pending-admit split | pending world preserved until user or headless path consumes it | who decides which cleanup comeback paths may auto-admit and which must remain explicit | `PluginInstallationManager.ts:56-59,135-179`; `useManagePlugins.ts:28-34,287-303` |
| plan recovery | slug/evidence checked before return | recovery only on ENOENT + remote-session path | resume keeps lineage; fork mints new slug | who decides whether returned cleanup objects preserve identity or only preserve content | `plans.ts:164-231,233-257`; `conversationRecovery.ts:539-547` |
| tombstone deletion path | none; deletion helper only removes old object | no re-entry gate present | no identity return semantics | stronger-request cleanup likewise lacks a dedicated resurrection plane today | who keeps deletion semantics from being mistaken for comeback semantics | `sessionStorage.ts:871-945,1468-1474` |
| stronger-request cleanup current world | marker/residue grammar is emerging, but no explicit re-entry policy | unresolved | unresolved | who decides which retired cleanup laws/promises/receipts may ever return to current world | `cleanup.ts:33-45,300-303,575-598`; `diagLogs.ts:14-60` |
