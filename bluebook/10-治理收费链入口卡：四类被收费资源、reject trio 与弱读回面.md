...
它也只补一件最容易被写浅的事实：安全与省 token 不是两套并列 feature，而是同一条收费链在对 model-reachable world 的危险扩张与昂贵扩张同时定价；进入诊断时先找 `earliest unpaid expansion`，只有在需要判责时才回看被收费对象类型。

## 边界卡

- `能合法说`
  - 只压缩 canonical chain、`earliest unpaid expansion -> repricing proof -> lease checkpoint -> reject -> escalation` 这条最短诊断环、reject trio、弱读回面声明与最短二跳。
- `不能改判`
...
- 治理不是先认资源类目或更会拦截，而是先固定 `governance key -> externalized truth chain -> typed ask -> decision window -> continuation pricing -> durable-transient cleanup`，并要求每次继续都先回答：最早 unpaid expansion 是什么、谁 repriced 了它、谁在 cleanup 后结算。
...
后面的 `canonical chain / 被收费对象 / reject trio / 弱读回面` 都只是这条诊断环的支持卡片，不是新的分类主题。
