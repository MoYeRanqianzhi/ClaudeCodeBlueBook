# 权威面与反僵尸图谱：single-writer surfaces、409 adoption、bridge pointer freshness与release shaping

这一章回答五个问题：

1. 为什么 Claude Code 的源码先进性最终要回到 authority surface，而不是停在模块美学。
2. 为什么 startup 顺序、release shaping、状态写回与恢复资产必须放在同一张结构图上。
3. 为什么 anti-zombie 不是局部并发技巧，而是一类跨系统不变量。
4. 为什么后来维护者最需要的不是更多 helper，而是更清楚的 single-writer surface。
...
- Claude Code 的源码先进性，不在于仓库“看起来更像教科书”，而在于系统先把单一权威面、陈旧写回、恢复资产命运与发布面边界都写进了结构。
...
2. single-writer authority surface atlas
...
1. 当前哪条 authority surface 负责宣布真相
...
## 4. Authority Surface Atlas：当前谁配写当前真相

Claude Code 的高级点，在于它不断把单一写入面做显式化。
...
- recovery asset 很重要，但它不配变成 authority surface
...
如果一个仓库没有 single-writer surface，也没有 anti-zombie catalog，那么后来维护者面对问题时只能：
...
1. policy-ordered bootstrap
2. single-writer authority surface
3. anti-zombie invariant catalog
4. recovery asset non-sovereignty
5. compile-time / runtime / artifact 三层 release shaping

如果这五件事不先成立，再漂亮的模块图也只是当前快照更好看。

## 10. 苏格拉底式追问

在你准备宣布“我已经理解了 Claude Code 的源码先进性”前，先问自己：

1. 我找到了 authority surface，还是只找到了大文件。
...
Claude Code 的源码先进性，首先不是目录更漂亮，而是它把 authority surface、anti-zombie、recovery asset 命运与 release shaping 一起写成了结构真相，先替后来者保住了“现在不能被过去写坏”的权利。
