...
这张 atlas 默认只收 `102` 已 handoff 的 promotion-passed objects；若当前 worktree 仍是 `mirror absent / public-evidence only`，下表也只表示“本页按 `102` handoff 应如何读”，不等于本地镜像已重新核验这些对象 truth。

| current-truth object | sole writer after `102` promotion | writeback seam | per-host width | forbidden substitute writer | local veto cue | first retreat layer |
|---|---|---|---|---|---|---|
| `permission_mode` | `onChangeAppState` | app-state writeback / externalized metadata | CLI / SDK / external metadata projection | 任意 UI 本地猜测 mode | mode 只能靠 UI 体感推断，而非正式回写 | 回 `onChangeAppState` 的 fail-closed externalization |
| `pending_action` | `sessionState + external_metadata` | session-state + metadata writeback chain | host-facing status width | transcript replay / tool log prose | ask 状态开始只剩弹窗感受或日志 prose | 回 `sessionState / external_metadata` 的 stale-field cleanup seam |
| `worker_status` | `reportState/reportMetadata` | uploader flush / status report chain | CCR / SDK status width | event timeline / print log | timeline 先于正式状态对象发号施令 | 回 `WorkerStateUploader` 的 coalesced flush discipline |
| `bridge pointer vs session head` | server/session head | server-head adoption / bridge pointer refresh | bridge freshness pointer | pointer 直接宣布 current truth | pointer 越过 session head 自己签当前真相 | 回 TTL + head adoption，把 pointer 降回 recovery asset |

按 landing card 语法再压一层，这张表现在已经把 `sole writer / writeback seam / local veto cue / first retreat layer` 全部摊开：`forbidden substitute writer` 解释谁在冒充 writer，`local veto cue` 解释 first no 先落在哪里，`first retreat layer` 则解释 first no 之后先把谁降回 `no-authority / no-write`。更短地说，这张 atlas 不再只是对象名录，而是 later maintainer 拿来直接拒收第二真相的 worked pattern。
