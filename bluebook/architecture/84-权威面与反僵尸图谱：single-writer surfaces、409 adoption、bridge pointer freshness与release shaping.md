...
按 landing card 语法再压一层，这张表里的 `forbidden substitute writer` 就是各 current-truth object 的第一条 `local veto cue`，而离它最近、能重新取回 writer claim 合法性的 `freshness / recovery / eviction` seam，就是 `first retreat layer`。
