...
- `102` 已经先锁定这是 object-level claim；本页只继续展开 object-level writer claim、writeback seam 与 anti-zombie invariants，不重开 ladder 或 promotion。
...
这张 atlas 也只配收 promotion-passed objects；若某个对象仍缺 writer promotion 所需证明，就应继续停在 `writer claim state / unresolved-authority note`，而不是先在这里占一行让 atlas 代行 promotion。
