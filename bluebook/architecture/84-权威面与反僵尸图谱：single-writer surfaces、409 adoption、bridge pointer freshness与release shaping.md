...
- `102` 已经先锁定这是 object-level claim；本页只继续展开 promotion-passed object 的 sole writer、writeback seam 与 anti-zombie invariants，不重开 ladder、promotion 或 provisional admission。
...
Claude Code 的高级点，在于它不断把 `current-truth claim / surface` 做显式化。公开证据还没 promotion 通过前，对象先写成 claim-state，而不是提前冒充 landed writer truth；未完成 promotion 的对象继续留在 `102` 的 `writer claim state / unresolved-authority note`，不在本页 atlas 里占行。
更硬一点说，`84` 不是第二张 admissibility 表；它不替 `102` 解释“为什么这对象够格进入 atlas”，只负责消费已经 promotion-passed 的 object-level result。
...
这张 atlas 也只配收 promotion-passed objects；若某个对象仍缺 writer promotion 所需证明，就应继续停在 `102` 的 `writer claim state / unresolved-authority note`，而不是先在这里占一行让 atlas 代行 promotion。更硬一点说，provisional object 不该在本页以半填表格的方式出现；它仍属于 `102` 的 claim-state 记录，而不是 `84` 的 atlas row。
若当前 worktree 仍是 `mirror absent / public-evidence only`，下表里的 `evidence status` 也只表示“本页按 `102` handoff 应如何读”，不等于本地镜像已重新核验这些对象 truth。

| current-truth object | evidence status | sole writer after `102` promotion | per-host width | forbidden substitute writer | freshness / eviction guard |
|---|---|---|---|---|---|
| `permission_mode` | `promotion-passed in 102` | `onChangeAppState` | CLI / SDK / external metadata projection | 任意 UI 本地猜测 mode | runtime externalization only |
| `pending_action` | `promotion-passed in 102` | `sessionState + external_metadata` | host-facing status width | transcript replay / tool log prose | stale field cleanup before reuse |
| `worker_status` | `promotion-passed in 102` | `reportState/reportMetadata` | CCR / SDK status width | event timeline / print log | coalesced uploader + flush discipline |
| `bridge pointer vs session head` | `promotion-passed in 102` | server/session head | bridge freshness pointer | pointer 直接宣布 current truth | TTL + head adoption |

按 landing card 语法再压一层，这张表里的 `forbidden substitute writer` 就是各 current-truth object 的第一条 `local veto cue`，而离它最近、能重新取回 writer claim 合法性的 `freshness / recovery / eviction` seam，就是 `first retreat layer`。若对象仍未 promotion，就根本不进入这张表，直接退回 `102` 继续保留 `writer claim state / unresolved-authority note`。

### 4.1 `operator artifact` 先按 `surface candidate` 入场，再决定是否退回 `102`

`CLAUDE.md`、hooks、skills、settings、permission schemas` 这类 operator artifacts 最容易被 later maintainer 一眼写成“这里就是 writer”。更稳的对象层动作反而更保守：

1. 先只把它记成 `surface candidate`
2. 再问它能否同时点名 `signer / effect ceiling / local veto cue / first retreat layer`
3. 还要确认 `102` 是否已经把它 typing 成 object-level claim，而不是仍停在 `public artifact ceiling / provisional claim / consumer subset`

只要这三步里有一步答不上，它就不进 atlas，而是直接退回 `102` 继续保留 `writer claim state / unresolved-authority note`。从第一性原理看，这不是对 operator artifact 不信任，而是拒绝把“公开可见、改动风险高、离操作者很近”误读成“因此天然拥有 sole writer authority”。
...
- 发布面边界被当成正式结构控制面
- 更硬一点说，release-surface shaping 就是 later maintainer 的可见性宪法：compile-time gate、runtime gate 与 artifact surface 若不分开，公开图谱就会重新把未签出的 capability 误读成 current truth。
- 若某个 capability 说不清是没被编进 build、没在当前 runtime 点亮，还是只没进入公开 artifact，先把结论退回这三层拆分。分不清 `compile-time gate / runtime gate / artifact surface` 之前，不把可见性差异写成 current truth、consumer subset 或 capability 缺失。
- 更稳一点说，release-surface shaping 不只是可见性说明，也是源码质量线的变更前协议：later maintainer 若还答不出某能力在 `compile-time gate / runtime gate / artifact surface` 三层里的 `first local veto cue` 与 `first retreat layer`，就还不配宣称自己已经看清这次改动的 blast radius。
