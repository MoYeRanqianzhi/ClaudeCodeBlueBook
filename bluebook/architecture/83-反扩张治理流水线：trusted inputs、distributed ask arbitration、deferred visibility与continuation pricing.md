...
- `governance key` 才是整张控制面的实现主键；source slot 只是最前面的来源位点，后面的动作授权、工具可见性、上下文占位和 continuation 都只是这张主键向下游派生出的受价结果
