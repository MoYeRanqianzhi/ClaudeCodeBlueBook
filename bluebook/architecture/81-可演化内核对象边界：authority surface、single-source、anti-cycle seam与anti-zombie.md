# 可演化内核对象边界：current-truth surface、single-source、anti-cycle seam与anti-zombie

这一章回答五个问题：

1. 为什么 Claude Code 的源码先进性最终要回到可演化内核的对象边界，而不是停在结构美学。
2. 为什么 current-truth surface、single-source、anti-cycle seam 与 anti-zombie 本质上在保护同一类长期不变量。
3. 为什么 `config / deps / QueryGuard / task framework / bridge pointer` 这些看似分散的实现，其实都在共同塑形内核。
4. 为什么没有这层对象边界，未来维护者就会重新落回半真相、陈旧 current-truth writeback 与第二套恢复面。
...
## 2. current-truth surface：先固定谁宣布对象当前真相

`onChangeAppState.ts` 与 `QueryGuard.ts` 说明：

1. mode 这类关键状态不能散落同步
2. query lifecycle 不能交给多个 flag 分头维护
3. 当前对象真相必须有单一 current-truth surface 入口
...
一旦 current-truth surface 漂移，后续所有 consumer subset、host projection、CI gate 都会一起漂移。
...
1. await 之后只补 offset patch，不回写整对象，避免陈旧 current-truth writeback
...
1. 恢复资产只能帮助恢复，不应长成第二 current-truth surface
...
- 过去的快照、旧 finally、陈旧指针是否会抢占 current-truth writeback，把自己重新写回现在

## 6. 为什么这四层本质上在保护同一件事

current-truth surface 保护：
...
- 过去不要通过陈旧 current-truth writeback 重新覆盖现在
...
1. 关键对象是否有 current-truth surface 入口
2. 共享真相是否仍能指回单一来源
3. 脏边是否已被 seam 圈住
...
Claude Code 的可演化内核之所以成熟，不在于结构更整齐，而在于它把 current-truth surface、single-source、anti-cycle seam 与 anti-zombie 一起写成了对象边界，先替未来维护者保住了不变量。
