...
- 本页只给 `adapter subset / status projection` 分级，不代签底层 `current-truth surface`
...
3. 凡改 adapter 宽度、subtype 支持面或 status projection，先按 `guides/102` 记一行 `change-risk record`，至少写出 `<surface> / replay obligation / gap note>`。
