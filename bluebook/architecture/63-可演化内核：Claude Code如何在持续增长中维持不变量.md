...
2. 为什么 `query/config.ts`、`query/deps.ts`、`QueryGuard`、leaf modules 这些细节透露出“可演化内核”的意识。
...
Claude Code 的源码先进性，真正值得学的不是某一次漂亮分层，而是它在持续增长中不断把 config、deps、state machine、leaf module 和权威面收成内核，让不变量始终有地方可守。
