...
更硬一点说，这套定价秩序实际在管理同一条 authority lease：每次放大都要回答 `signer / scope delta / durability class / cleanup trigger`，否则动作、能力、席位与时间只是在被重复占用，而不是被正式续租。
...
## 3. `bypass` 不是豁免一切，而是在同租约内压缩剩余 ask
...
- 在同一条 authority lease 已签出的 scope 内压缩剩余 ask

而不是：

- 新增 signer、scope delta、durability class 或 cleanup trigger

它减少的是摩擦，不单独形成新的治理事实；若一段 ask 需要新的 signer 或新的 cleanup 责任，`bypass` 也不能替它补签。
...
更稳一点说，审批效率不是更快成交，而是更少出现没有新增 `decision delta` 的 ask；若批准更顺滑，却仍在为同一条边界重复付费，这套系统就还没有真正省下 token。
...
这三步其实已经把 `durability class / cleanup trigger` 写进去了：结果从主上下文席位迁出后，才配被视作 externalized durable asset；而 `tool_use_id` 被替换就是 replay / cleanup 的直接触发器，不只是字节稳定性的细节。
...
再压一层，continue 是否划算也不只看 token 比例，而要看这次续租有没有新增 `decision delta`；而且这份 gain 只有在同一条 authority lease 仍然闭合住当前对象时才算数。若只是重复保留旧边界、旧结果与旧 readback，或者已经退回 requalification / cleanup，这就不叫 continuation gain，只叫时间被免费烧掉。
