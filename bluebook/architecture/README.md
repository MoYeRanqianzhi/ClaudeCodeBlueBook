...
这四项不代替 `102` 的 promotion gate；它们只是在 owner README 层把 handoff 压成 later maintainer 一眼可见的最小摘要。若 reader 还要翻两段 prose 才看出对象现在只是 `provisional claim`，还是已经 `promotion-passed`，对象 landing 就还太依赖作者解释。
