...
1. 把“rewrite correction 写得更完整”降为次级信号。
2. 把 `restored_request_object_id + compiled_request_hash + section_registry_snapshot + dynamic_boundary_attested` 提升为前提。
3. 任何先看 prose、后看对象的 Prompt rewrite correction execution 都判为 drift。
...
1. 把目录清晰度与 attachment 说明降为评审材料。
2. 把 `section_registry_generation + registry_drift_cleared + coordinator_synthesis_owner + late_bound_attachment_set + attachment_binding_order + dynamic_boundary_attested` 提升为正式对象。
3. 任何“静态目录清楚即视为 registry 已恢复”的 Prompt rewrite correction execution 都判为 drift。
...
3. 任何阈值已失忆却仍继续放行的 Prompt rewrite correction execution 都判为 drift。
