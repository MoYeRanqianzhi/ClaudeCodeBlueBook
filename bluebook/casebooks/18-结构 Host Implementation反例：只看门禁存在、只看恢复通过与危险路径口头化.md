...
1. 为什么结构 implementation 明明已经有 authority surface、recovery asset、anti-zombie gate、rollback boundary 与 retained assets，团队仍然会退回存在性合规与作者记忆。
...
- gate 已经存在，却只剩“规则写了”“恢复过了”“作者知道危险路径”，没人再围绕同一 authority surface、同一 recovery asset 与同一 rollback object 判断
...
## 1. 只看门禁存在 vs authoritative path truth

### 坏解法

- 宿主或评审只要看到 authority surface 已被点名、规则写在文档里，就默认结构 implementation 成立，不再追问当前读写路径是否真的只剩这一条权威路径。

### 为什么坏

- authority 被点名，不等于 split-brain 已经消失。
...
- 交接时作者口头说明几条 danger paths，团队就默认结构 implementation 可以接手，不再核验 rollback object、retained assets 与 projection set 是否结构化交付。

### 为什么坏

- 口头说明会把 shared implementation 再次退回作者记忆。
- 如果 rollback object 与 retained assets 不在包里，后来者仍然不知道真正该回退什么、该保留什么。
- 结构交接会从对象级真相再次退回经验传授。
...
- 这四项都可能局部正确，却仍然没有共享同一 authority / recovery / rollback object。
...
4. rollback object 与 retained assets 是否已经比作者口头说明更先被交付。
