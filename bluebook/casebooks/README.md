...
- 下一层入口见 `../navigation/77 -> ../guides/78-80`，继续回答“怎样把这些 steady-state correction execution 失真重新压回固定顺序、拒收升级路径与改写模板骨架”。
- 再往下一层，应由 `../navigation/78 -> ../api/75-77` 继续回答“怎样把这些 steady-state correction-of-correction 继续压成宿主可消费的修正对象、拒收升级语义与长期 reopen 责任面”。
