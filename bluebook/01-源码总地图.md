...
兼容地图也继续继承 `问题分型 -> 工作对象 -> 控制面 -> 入口`；它只保留 repo-wide map projection，不额外重签 canonical ladder、current-truth verdict 或产品承诺。
...
- `continuity` 在地图层也不是第四类源码主题；它只是 request assembly、continuation pricing 与 recovery non-sovereignty 在源码地图上的时间轴交叉。
