# 治理宿主修复稳态纠偏再纠偏改写纠偏执行手册：rewrite correction card、classifier pricing、writeback seam与threshold rebinding drill

这一章不再解释治理宿主修复稳态纠偏再纠偏改写纠偏协议该消费哪些字段，而是把 Claude Code 式治理 steady-state correction-of-correction rewrite correction protocol 压成一张可持续执行的 `rewrite correction card`。

它主要回答五个问题：

1. 为什么安全设计与省 token 设计在再纠偏改写纠偏执行里运行的不是“两套系统互相权衡”，而是同一个治理控制面持续拒绝免费扩张。
2. 宿主、CI、评审与交接怎样共享同一张治理 `rewrite correction card`，而不是各自宣布不同版本的“现在可以继续”。
3. 应该按什么固定顺序执行 authority、ledger、decision window、continuation pricing、classifier pricing、writeback seam、capability liability 与 threshold，才能不让免费继续、免费扩窗与免费放权重新进场。
...
- governance key、ledger、window、continuation pricing、classifier pricing、writeback seam、capability liability 与 threshold 仍围绕同一个治理对象正式宣布：现在什么能继续、什么值得继续、什么必须被拒绝继续
...
1. 当前 rewrite correction object 是否仍围绕同一个 governance object。
...
4. 当前 `classifier pricing attestation` 是否真的让安全系统也接受被定价。
5. 当前 `writeback seam reseal` 是否真的把 `requires_action -> pending_action -> session_state_changed` 写回同一宿主真相。
6. 当前 `capability liability recustody` 是否仍明确 durable assets 与 transient authority 的分界。

## 2. 共享 rewrite correction card 最小字段

每次治理宿主修复稳态纠偏再纠偏改写纠偏巡检，宿主、CI、评审与交接系统至少应共享：
...
4. 交接看 later 团队能否只凭 `rewrite correction card` 重建同一判定与责任边界。

## 3. 固定 reject verdict 顺序

### 3.1 先验 `rewrite_correction_session_object`

先看当前准备宣布 rewrite correction 完成的，到底是不是同一个 `governance_object_id`，而不是一张更正式的稳定说明。
...
2. `externalized_truth_chain` 是否仍把 authority 外化成唯一真相链。
...
3. 当前 rewrite correction 是否仍阻止旧窗口与旧上下文投影复活。

### 3.6 再验 `continuation pricing rebinding` 与 `classifier pricing attestation`

再看：

1. `continuation_gate`、`budget_policy_generation` 与 `settled_price` 是否仍把未来继续重新定价。
2. `classifier_cost_priced` 是否仍明确 classifier 不在对象外。
...
### 3.7 再验 `writeback seam reseal` 与 `capability liability recustody`

再看：

1. `requires_action_ref -> pending_action_ref -> session_state_changed_ref` 是否仍围绕同一条 writeback seam。
2. `capability_release_scope`、`custody_owner` 与 `liability_owner` 是否仍明确。
...
出现下面情况时，应直接阻断当前治理 rewrite correction execution：
...
7. `writeback_seam_missing`
8. `externalized_truth_chain_missing`
9. `transient_authority_resumed`
10. `free_continuation_detected`
...
1. 先冻结新的 capability expansion，直到 `rewrite correction card` 补全。
...
4. 先按固定顺序重验 governance key、ledger、window、pricing、classifier、writeback 与 liability，不允许跳过 `decision window refreeze`。
...
每轮至少跑下面七个治理宿主稳态纠偏再纠偏改写纠偏执行演练：

1. `authority_chain_restitution_recheck`
2. `ledger_reseal_replay`
3. `decision_window_refreeze_replay`
4. `context_usage_window_reconsume`
5. `classifier_pricing_replay`
6. `writeback_seam_reseal_replay`
7. `capability_liability_threshold_replay`

## 7. 复盘记录最少字段

每次治理宿主稳态纠偏再纠偏改写纠偏失败、再入场或 reopen，至少记录：
...
11. `durable_assets_after`
12. `transient_authority_cleared`
...
在你准备宣布“治理已完成稳态纠偏再纠偏改写纠偏执行”前，先问自己：

1. 我现在救回的是同一个治理对象，还是一套更严格的 rewrite correction 说明书。
2. 我现在重申的是 governance key 真相链，还是 mode 与 dashboard 的平静感。
3. 我现在冻结的是 formal `decision window`，还是只是在看 token 百分比是否回落。
4. 我现在恢复的是 continuation 与 classifier 的正式定价，还是给默认继续换了个更体面的名字。
5. 我恢复的是 durable assets，还是把 transient authority 也一并续租了。
6. 我现在保留的是未来推翻当前 steady 的正式 threshold，还是一句“以后有问题再看”。

## 9. 一句话总结

真正成熟的治理宿主修复稳态纠偏再纠偏改写纠偏执行，不是把治理面板运行得更熟练，而是持续证明 governance key、ledger、window、pricing、classifier、writeback seam、capability liability、durable-vs-transient 分界与 threshold 仍在共同拒绝免费扩张。
