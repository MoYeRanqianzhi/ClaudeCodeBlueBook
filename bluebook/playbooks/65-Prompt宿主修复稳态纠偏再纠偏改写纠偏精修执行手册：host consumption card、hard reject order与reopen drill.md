# Prompt宿主修复稳态纠偏再纠偏改写纠偏精修执行手册：message lineage witness、shared reject order与reopen drill
...
3. 应该按什么固定顺序执行 `refinement session`、`false projection demotion`、`compiled request lineage`、`registry-boundary rewrite`、`coordinator synthesis custody`、`protocol-prefix custody`、`forgetting-continuation covenant`、`hard reject semantics`、`shared-consumer attestation` 与 `reopen liability ledger`，才能不让 refinement protocol 重新退回解释稿。
...
`host consumption card` 只是 Explainability 末端 carrier，它自己不是新的 Prompt 主语；它必须显式携带 `message_lineage_ref -> section_registry_ref -> stable_prefix_ref -> protocol_transcript_ref -> continuation_object_ref -> continue_qualification_verdict` 这条 witness order，才能继续承载 shared reject / reopen drill。
...
### 3.8 再验 `hard_reject_semantics_abi` 与 `shared_consumer_surface_attestation_packet`
