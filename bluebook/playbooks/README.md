...
- 下一层入口见 `../navigation/77 -> ../guides/78-80`，继续回答“怎样把这些稳态纠偏执行失真重新压回固定顺序、拒收升级路径与改写模板骨架”。
- 再往下一层，应由 `../navigation/78 -> ../api/75-77` 继续回答“怎样把这些稳态纠偏再纠偏继续压成宿主可消费的修正对象、拒收升级语义与长期 reopen 责任面”。
