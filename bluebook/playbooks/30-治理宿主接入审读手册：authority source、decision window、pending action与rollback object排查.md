...
1. 团队怎样知道宿主当前仍在围绕同一个治理对象消费 `sources -> effective -> applied -> externalized`、decision window、pending action、continuation gate 与 rollback object。
...
- 宿主当前仍在围绕同一个 governance object 消费 governance key、decision window、continuation gate 与 rollback object
...
2. pending action 是否仍是 externalized object，而不是提示语。
...
1. 宿主只看 `permission_mode`，不看 `sources / effective / applied / externalized`。
2. `pending_action` 被降格成“当前卡住了”的一段文案，或靠事件流反推。
...
2. `pending_action` 只剩文案，或靠事件流 / spinner 反推，不剩对象字段。
...
2. 先把 `pending_action` 拉回 externalized object。
...
2. `pending_action` 在我的系统里还是 externalized object，还是已经退化成文案。
...
真正成熟的治理宿主接入审读，不是看控制请求能不能跑通，而是持续证明宿主仍围绕同一个治理对象消费 governance key、decision window、continuation gate 与 rollback object。
