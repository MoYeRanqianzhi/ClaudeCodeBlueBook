# 治理宿主修复稳态纠偏再纠偏改写执行手册：rewrite card、reject verdict order、window repricing drill、capability liability与threshold rebinding drill
...
### 3.3 再验 `authority source restitution`
...
真正成熟的治理宿主修复稳态纠偏再纠偏改写执行，不是把治理面板运行得更熟练，而是持续证明 authority、ledger、window、pricing、capability liability 与 threshold 仍在共同拒绝免费扩张。
