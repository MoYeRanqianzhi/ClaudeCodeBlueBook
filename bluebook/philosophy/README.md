...
`philosophy/` 只回答三件事：世界如何合法进入模型，扩张如何先被定价，过去如何不得越权写回现在。这里不重写长链，只保留 why frontdoor：Prompt 先到 `84`，治理先经 `10` 固定 canonical chain 再回 `85 / 61`，源码质量先经 `guides/102` 再回 `86 / 87`。
