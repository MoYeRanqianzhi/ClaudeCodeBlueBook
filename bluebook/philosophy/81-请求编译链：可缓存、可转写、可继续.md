...
如果只想先看一张最小 signer / carrier 表，也只先记下面这张：

| 层 | 更像 signer / witness 的对象 | 更像 carrier / projection 的对象 |
|---|---|---|
| 世界准入 | `message lineage / section registry / stable prefix / protocol transcript` | UI transcript、display history、作者解释 |
| 继续资格 | `continuation object / continue qualification verdict` | summary、session memory、handoff prose、sticky prompt |
| 解释与观测 | cache break reason、qualification failure reason | status 文案、usage 面板、阅读友好摘要 |

更硬一点说，carrier 可以搬运上下文、方向和 receipt，却不能单独代签世界定义与继续资格；一旦 later consumer 只能拿到右栏对象，Prompt 效力就应先降格，而不是被继续包装成“还能用”。

如果要把上面这张表再压成一个 end-to-end worked example，也只先看下面这组：

| handoff package | 保住了什么 | 丢了什么 | 更稳的 verdict | 为什么 |
|---|---|---|---|---|
| `carrier-only package` | summary、session memory、handoff prose、最后一步建议 | `protocol transcript / stable prefix / continuation object` 里的 guard 与 qualification binding | `reject` | later consumer 仍得重开 `world-definition / tool-legality / next-action search`，所以它只配算 belonging carrier |
| `witness-rebound package` | `current work / next-step guard / required assets / rollback boundary / continue qualification / threshold liability` | narrative richness 可以缺一些 | `pass` | later consumer 不必重谈 authority、boundary 与已排除分支，就能复现同一条 `continue-or-reject verdict` |
| `mixed package` | summary + 部分 object guard | `continue qualification` 或 branch-pruning reason 缺失 | `provisional` | 还带着继续味道，但 witness 链未闭合；先回 `protocol transcript / stable prefix / continuation object` 做 rebind，不直接写成 same-world 成立 |

更硬一点说，Prompt 的“魔力”在这个例子里一点也不神秘：它只是把 later consumer 原本要重付的一轮世界重谈、动作重搜与分支重判税，提前编译进了 witness package；一旦只剩 carrier，没有这笔预付税，继续资格就会重新退回冷启动。
...
更稳地说，`continue qualification` 不是对摘要质量的感受分，而是 later consumer 重新读取这组 `lawful forgetting witness ABI` 时，仍能在不重谈 `authority / boundary / lineage` 的前提下复现同一条 `continue-or-reject verdict` 的判定。
...
第一条 `first retreat layer` 也因此不是补写更顺的摘要，而是先回 `protocol transcript / stable prefix / continuation object`，补齐哪一项 `required witness` 已经断掉；若退不回这层，lawful forgetting 就已经退回 narrative compression。
更快一点的拒收线也只剩一句：只要 `narrative-only` 或 `forbidden-as-sole-witness` 被迫独自承担 continuation witness，就直接判 `continue qualification` 失效，不得再把摘要包装成 lawful forgetting。
...
更硬一点说，`next-step guard` 不只是在说“接下来做什么”，还必须保住“哪些看似可做的分支已经被当前 `authority / boundary` 排除”；若 compact 之后只剩正向建议而没有负向剪枝，later consumer 仍会重新打开全量动作搜索，`continuation object` 也就退回成叙事提示。
更严格地说，若 compact 之后连“这些分支为什么仍该被排除”都要重新生成，later consumer 消耗掉的就不只是 token，而是一轮没有新增决策增益的 branch re-adjudication；这时失稳的不是摘要长度，而是 `continue qualification` 已退回冷启动。
