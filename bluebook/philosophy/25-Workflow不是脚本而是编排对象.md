...
- `claude-code-source-code/src/Task.ts:6-12` 把 `local_workflow` 提升为正式 `TaskType`
- `claude-code-source-code/src/Task.ts:78-85` 为 workflow 分配独立 task id 前缀 `w`
- `claude-code-source-code/src/tasks/types.ts:12-29` 把 `LocalWorkflowTaskState` 纳入统一 task / background task 联合类型
- `claude-code-source-code/src/utils/task/sdkProgress.ts:10-35` 允许 `task_progress` 附带 `workflow_progress`
- `claude-code-source-code/src/utils/sessionStorage.ts:232-258` 为 workflow transcript 预留 `subagents/workflows/<runId>/`
- `claude-code-source-code/src/utils/worktree.ts:1023-1045` 为 workflow run 保留专门的临时 worktree 命名与清理语义

这些锚点共同说明，workflow 已经穿透 task、event、transcript 与 worktree 四层边界。
...
后续所有 workflow 相关章节都应先回答三件事：

1. 它进入了哪一种正式对象联合类型。
2. 它在事件流里暴露了哪些可观测进度。
3. 它在哪一层拥有独立 transcript / worktree / cleanup 边界。

如果这三件事没有回答，就不该把它写成“已经理解 workflow”。
