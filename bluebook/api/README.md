...
- `31-45`: 失败语义、Evidence Envelope、Artifact Contract 与 Harness Runner，回答共享工件怎样挂回治理链与 `contract -> registry -> current-truth surface -> consumer subset -> hotspot kernel -> mirror gap discipline` 这条 canonical ladder，而不是长出第二套壳层主语。
...
- 想看目录级地图、结构工件与规则包怎样回到 canonical ladder：从 `30 -> 39 -> 42 -> 46-50`
