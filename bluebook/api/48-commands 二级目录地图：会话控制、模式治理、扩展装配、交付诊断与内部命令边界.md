...
- `current-truth claim state`：
  - 当前被装进公开命令面的 session / context / resume 命令子集只先构成 command-surface claim-state
- `consumer subset`：
...
- `current-truth claim state`：
  - 当前被准入的治理命令子集只先构成 governance-command claim-state
- `consumer subset`：
...
- `current-truth claim state`：
  - 当前被准入的 connection / plugin 控制命令子集只先构成 connection-command claim-state
- `consumer subset`：
...
- `current-truth claim state`：
  - 当前被准入的 diagnostic / review 命令子集只先构成 diagnostic-command claim-state
- `consumer subset`：
...
- `current-truth claim state`：
  - 当前被装进公开命令面的 collaboration 命令子集只先构成 collaboration-command claim-state
- `consumer subset`：
...
- `current-truth claim state`：
  - 当前真正对外公开的命令子集只先构成 public-command claim-state
- `consumer subset`：
...
这页真正值钱的，不是把 `commands/` 二级目录再排成一张更细的地图，而是把 later maintainer 拉回：哪些显式控制动作沿 `contract -> registry -> current-truth claim state -> consumer subset -> hotspot kernel -> mirror gap discipline` 被公开承诺、被裁切或被拒收。
