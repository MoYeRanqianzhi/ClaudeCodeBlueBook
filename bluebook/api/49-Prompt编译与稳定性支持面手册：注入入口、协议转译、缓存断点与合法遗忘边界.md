# Prompt编译与稳定性支持面手册：注入入口、协议转译、缓存断点与合法遗忘边界
...
真正把 Prompt 变成 compiled request truth 的关键支持面仍然是内部机制：
...
2. 不要用原文 prompt 代替编译后的请求真相。
3. 不要把 cache 稳定性问题理解成偶发性能问题。

## 7. 一句话总结

Claude Code 的 Prompt 支持面，不是一个 prompt 字符串 API，而是“输入入口 + 观测表面 + 内部编译机制”共同组成的分层支持面。
