...
4. `continuation pricing + durable-transient cleanup`
   - 继续资格、cleanup carrier 与回退边界都必须按正式对象消费。
...
因此宿主在 Claude Code 里不是被动接收答案的前端，而是正式参与治理主键与 cleanup 主键的控制面消费者。
...
- `continuation pricing verdict` 只是 host-facing 继续判词。
- `rollback object` 只是 cleanup / handoff 的 carrier，不是治理根对象。
