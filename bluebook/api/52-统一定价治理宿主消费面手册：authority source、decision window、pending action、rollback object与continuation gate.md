...
Claude Code 当前没有公开一份名为 `governance_key` 的单独公共对象；在 `public-evidence only` 下，更稳的写法是：宿主当前只按四类已外化 surface 来消费治理链，再在这些 surface 之上做解释：

1. 宿主可写的 control requests
2. 宿主可读的 settings / state / pending-action / usage surfaces
3. 宿主可见的 ask / window / continuation projections
4. 宿主不应直接绑定的 internal decision machinery

如果把统一定价治理宿主消费面前门继续压成最短读法，也只剩四句：

1. 先看宿主到底能写哪些 `control requests`
   - 不先把它们写成已公开的治理根对象。
2. 再看宿主到底能读哪些 `settings / state / pending_action / usage` surface
   - host 只消费 runtime 已外化的真相，不自己补完。
3. 再用这些 surface 解释 ask / decision window / continuation pricing
   - `pending_action` 只是 ask / window 的证据字段，不是治理节点本身。
4. 最后才把 `governance key / externalized truth chain / continuation pricing` 当解释词使用
   - 它们帮助压缩读法，但当前还不配替 concrete surface 抢首句。
...
- 宿主已经拿到了权限、设置与回退相关的正式写入口

在这些 surface 之上，你当然可以继续用 `governance key / typed ask / cleanup` 这些词来解释它们的共同语义；但更稳的默认写法仍是：先承认“这些是已外化 control requests”，再谈它们可能共同指向的治理母题，而不是反过来把 surface 写成已公开的治理根对象。
...
## 5.1 最短 public-surface 对照

如果要把这一页压成最短宿主实现表，可以先钉死下面这张 `surface first / interpretation second` 对照：

| concrete surface | 可用什么解释词压缩阅读 | 宿主不该怎么偷懒 |
|---|---|---|
| `get_settings.sources / effective / applied` | `governance key` 的解释词 | 不要只看 `permission_mode` |
| `permission_mode + session_state_changed + worker_status + pending_action + get_context_usage` | `externalized truth chain` 的解释词 | 不要从 UI 局部状态反推整体 |
| `can_use_tool` request/response + request correlation | `typed ask` 的解释词 | 不要把 ask 退回弹窗存在性 |
| `get_context_usage + pending_action + current state` | `decision window` 的解释词 | 不要把 usage 百分比当窗口本身 |
| `context usage / compact 阈值 / state transition` | `continuation pricing` 的解释词 | 不要把 continue 当默认免费按钮 |
| `rewind / cleared fields / post-resume state` | `durable-transient cleanup` 的解释词 | 不要把 rollback carrier 当治理根对象 |
...
Claude Code 的统一定价治理支持面，不是若干零散 control API，而是“宿主可写 control requests + 宿主可读 settings / state / pending-action / usage surfaces + 在这些 surface 上可解释出的 ask / decision window / continuation pricing 投影 + internal-only machinery”共同组成的分层宿主消费面；在 `public-evidence only` 下，这些治理 nouns 默认先按解释词读取，而不是 landed object。
