...
更稳的收口不是继续把这十类对象并列平铺，而是把它们压成四段执行主语：`pricing-right covenant -> truth-surface attestation packet -> asset-rollback ABI -> shared reject / reopen semantics`；`authority_ledger_covenant`、`window_pricing_covenant`、`classifier_writeback_custody` 与 `capability_liability_ledger` 都只是这四段之下的 evidence slot。
...
- 围绕这十类对象继续消费同一个 authority、同一个 ledger、同一个 decision window、同一个 `settled_price` runtime witness、同一份 classifier 成本、同一条 writeback seam、同一个 capability liability、同一个 `shared_consumer_surface` 与同一个 threshold
...
- `free_continuation_blocked` 不是 UI 提醒，而是 `settled_price` 尚未成立时拒绝继续资格外借的 guard
...
如果继续把这组对象再压成 later maintainer 可直接复用的一条 `asset / rollback object ABI`，也只该剩：
...
更稳的读法不是把 compaction、handoff、rollback、reopen 当四套对象语言，而是把它们都读成同一组 asset/rollback object 在不同 consumer 前的 custody 与 liability 变体。
...
### 5.3 truth surface attestation packet
...
如果继续把跨消费者证明再压成 `truth surface attestation packet`，更稳的最小对象也只该剩：

1. `display_truth_ref`
2. `event_truth_ref`
3. `current_truth_ref`
4. `protocol_truth_ref`
5. `surface_lossiness_digest`
6. `forbidden_promotion_set`
7. `event_replay_not_current_truth`

这组对象真正值钱的地方，不是再多发一层 attestation，而是把 display / event / current / protocol 之间哪些可以互证、哪些绝不能升格，写成 later consumer 可复演的边界。
`shared_consumer_surface` 也因此不是四端共用一段文案，而是 host / CI / reviewer / handoff 以不同宽度只读消费同一个 verdict object 的 carrier。
...
- `asset / rollback object ABI` 若没有被同一组 consumer 共享，宿主就会把“回退什么”“保留什么”“何时重入”“何时重开”重新拆成四套壳层说法
- `truth surface attestation packet` 若没有写清 `forbidden_promotion_set`，display / event truth 迟早会再次越权冒充 current / protocol truth
...
7. 再验 `truth_surface_attestation_packet`
