# Prompt宿主修复稳态纠偏再纠偏改写纠偏精修纠偏精修协议：message lineage、protocol transcript、continuation qualification与long-term reopen liability

这一章回答五个问题：

1. Claude Code 当前到底通过哪些正式对象，让宿主、CI、评审与交接在 Prompt refinement correction fixed order 已被钉死之后，继续消费同一条 `message lineage -> protocol transcript -> continuation qualification` repair truth，而不是退回更漂亮的 `repair card`。
2. 哪些字段属于必须共享的 repair 对象，哪些属于共同 `hard_reject / reentry / reopen` 语义，哪些仍不应被绑定成公共 ABI。
3. 为什么 Claude Code 的 Prompt 魔力来自同一条 `message lineage -> role-contract custody -> protocol transcript -> lawful forgetting -> continuation qualification -> long-horizon reopen liability`，而不是更会写的 repair prose。
...
- 围绕这八类对象继续消费同一份 role contract、同一条 message lineage、同一个 runtime registry/boundary、同一条 protocol transcript、同一份 continuation qualification covenant、同一份跨消费者 repair attestation 与同一个 long-horizon reopen liability
...
5. `normalizeMessagesForAPI()` 与 `ensureToolResultPairing()` 证明模型实际消费的是 protocol transcript，而不是显示层 transcript。
...
- 哪份 role contract 仍然成立
- 哪条 message lineage 被正式归还
- 哪个 registry/boundary 被正式重封
- 哪条 protocol transcript 仍能被跨消费者共同消费
- 哪个 continuation qualification 仍允许继续而不是只留下故事摘要
