# Prompt宿主修复稳态纠偏再纠偏改写纠偏精修纠偏协议：message lineage、registry-boundary custody、synthesis custody 与 long-term reopen liability
...
3. 为什么 Claude Code 的 Prompt 魔力来自同一条 `message lineage -> section registry -> dynamic boundary -> synthesis custody -> protocol transcript -> stable prefix -> lawful forgetting -> continuation qualification -> threshold liability`，而不是更会写的 Prompt prose。
...
2. `message_lineage_contract`
3. `section_registry_custody`
4. `registry_boundary_custody`
5. `synthesis_custody`
6. `protocol_prefix_custody`
7. `forgetting_continuation_covenant`
8. `reject_semantics_packet`
...
- 围绕这九类对象继续消费同一条 `message lineage`、同一个 runtime section registry、同一条 dynamic boundary、同一个 synthesis owner、同一条 protocol transcript、同一段 stable prefix、同一个 lawful forgetting boundary、同一个 continuation qualification 与同一个 threshold liability
...
- 哪条 `message lineage` 仍然成立
- 哪个 registry / boundary / synthesis custody 被正式归还
